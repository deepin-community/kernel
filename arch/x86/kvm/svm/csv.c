...
static int csv_vm_attestation(struct kvm *kvm, unsigned long gpa, unsigned long len)
{
	struct kvm_sev_info *sev = &to_kvm_svm(kvm)->sev_info;
...
int csv_get_msr(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	...
	case MSR_AMD64_SEV_ES_GHCB:
		...
		if (!msr_info->host_initiated || !____sev_es_guest(vcpu->kvm))
			return 1;
