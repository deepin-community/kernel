...
static int page_inc_gen(struct lruvec *lruvec, struct page *page, bool reclaiming)
{
	...
	lru_gen_balance_size(lruvec, page, old_gen, new_gen);
			     ...
			     unsigned long *min_seq, bool can_swap, bool *need_aging)
{
	...
	if (min_seq[TYPE_FILE] + MIN_NR_GENS > max_seq)
		*need_aging = true;
	else if (min_seq[TYPE_FILE] + MIN_NR_GENS < max_seq)
		*need_aging = false;
...
static bool sort_page(struct lruvec *lruvec, struct page *page, int tier_idx)
{
	...
	if (type && PageAnon(page) && PageDirty(page)) {
		success = lru_gen_del_page(lruvec, page, true);
		VM_BUG_ON_PAGE(!success, page);
	...
	if (PageLocked(page) || PageWriteback(page) || (type && PageDirty(page))) {
		       ...
		       bool *swapped)
{
	...
	list_for_each_entry(page, &list, lru) {
		if ((page_is_file_lru(page) || PageSwapCache(page)) &&
...
static long get_nr_to_scan(struct lruvec *lruvec, struct scan_control *sc, bool can_swap)
{
	...
	return min_seq[TYPE_FILE] + MIN_NR_GENS <= max_seq ? nr_to_scan : 0;
}
