	...
	unsigned int mclk_fs)
{
	...
	ret = snd_soc_dai_set_sysclk(snd_soc_rtd_to_cpu(rtd, 0), mclk_div, mclk,
			SND_SOC_CLOCK_OUT);
	if (ret && ret != -ENOTSUPP)
		return ret;
...
static const struct snd_soc_ops cix_dailink_ops = {
	.startup = dai_startup,
...
int cix_card_parse_of(struct cix_asoc_card *priv)
{
	...
	for_each_child_of_node(dev->of_node, np) {
		...
		if (of_property_read_bool(np, "jack-det,dpout")) {
			link_info->jack_pin[JACK_DPOUT].pin = np->name ? np->name : "jack-dpout";
