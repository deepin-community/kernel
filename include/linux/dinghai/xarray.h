...
#ifndef DINGHAI_XARRAY_H
#define DINGHAI_XARRAY_H
/*
 * eXtensible Arrays
...
/*
 * Define struct xa_limit and struct xarray only when NEED_XARRAY is defined
 * to avoid redefinition conflicts with kernel's native xarray.h
 */
struct xa_limit {
	u32 max;
	u32 min;
...
/* struct xa_limit already defined above */
 ...
 */
#define XA_FLAGS_LOCK_IRQ	((__force gfp_t)DH_XA_LOCK_IRQ)
#define XA_FLAGS_LOCK_BH	((__force gfp_t)DH_XA_LOCK_BH)
...
/* struct xarray already defined above */
...
void __init dh_radix_tree_init(void);
