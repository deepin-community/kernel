...
static int gpiochip_match_name(struct gpio_chip *chip, const void *data)
{
...
static int get_gpio_irq_from_acpi_table(int gpio)
{
	...
	gdev = gpio_device_find("LOON0007:00", gpiochip_match_name);
	if (!gdev)
		return -ENOENT;
...
#endif
MODULE_DESCRIPTION("Loongson LS2K500 frame buffer driver");
MODULE_LICENSE("GPL");
