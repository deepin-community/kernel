...
static int sky1_udphy_init(struct cix_udphy *udphy)
{
	...
	dev_info(udphy->dev, "%s: typec dir= 0x%x\n", __func__, value);
	...
	if (udphy->next_mode == UDPHY_MODE_DP) {
		dev_info(udphy->dev, "%s: sky1_udphy_dp_conf\n", __func__);
				      ...
				      struct phy_configure_opts_dp *dp)
{
	...
	dev_dbg(udphy->dev, "%s: lane %d, vs %d, pe %d\n", __func__,
