...
void setDPMS(DPMS_t state)
{
    ...
    if(ddk750_getChipType() >= SM750LE)
    {
        ...
        {
        case DPMS_ON:
            value = SMI_FIELD_SET(value, SECONDARY_DISPLAY_CTRL, DPMS, 0);
            break;

        case DPMS_STANDBY:
            value = SMI_FIELD_SET(value, SECONDARY_DISPLAY_CTRL, DPMS, 1);
            break;

        case DPMS_SUSPEND:
            value = SMI_FIELD_SET(value, SECONDARY_DISPLAY_CTRL, DPMS, 2);
            break;

        case DPMS_OFF:
            value = SMI_FIELD_SET(value, SECONDARY_DISPLAY_CTRL, DPMS, 3);
            break;
        }
    ...
    {
       case DPMS_ON:
        value = SMI_FIELD_SET(value, SYSTEM_CTRL, DPMS, VPHP);
        break;

       case DPMS_STANDBY:
        value = SMI_FIELD_SET(value, SYSTEM_CTRL, DPMS, VPHN);
        break;

       case DPMS_SUSPEND:
        value = SMI_FIELD_SET(value, SYSTEM_CTRL, DPMS, VNHP);
        break;

       case DPMS_OFF:
        value = SMI_FIELD_SET(value, SYSTEM_CTRL, DPMS, VNHN);
        break;
    }
...
unsigned long getPowerMode()
{
    ...
    return (SMI_FIELD_GET(peekRegisterDWord(POWER_MODE_CTRL), POWER_MODE_CTRL, MODE));
}
...
void setPowerMode(unsigned long powerMode)
{
    ...
    previousPowerState = SMI_FIELD_GET(control_value, POWER_MODE_CTRL, MODE);

    /* Set the power mode to the requested power mode. */
    switch (powerMode)
    {
        case 0:
            control_value = SMI_FIELD_SET(control_value, POWER_MODE_CTRL, MODE, MODE0);
            break;

        case 1:
            control_value = SMI_FIELD_SET(control_value, POWER_MODE_CTRL, MODE, MODE1);
            break;

        case 2:
            control_value = SMI_FIELD_SET(control_value, POWER_MODE_CTRL, MODE, SLEEP);
            break;
    ...
    if (powerMode == POWER_MODE_CTRL_MODE_SLEEP)
        control_value = SMI_FIELD_SET(control_value, POWER_MODE_CTRL, OSC_INPUT, OFF);
    else
        control_value = SMI_FIELD_SET(control_value, POWER_MODE_CTRL, OSC_INPUT, ON);
#endif
    ...
    if ((powerMode != 2) && (previousPowerState != SMI_FIELD_GET(control_value, POWER_MODE_CTRL, MODE)))
    {
...
void enableBusMaster(unsigned long enable)
{
    ...
    if (enable)
    {
        if ((ddk750_getChipType() == SM718) && (busMasterCounter == 0))        
            pokeRegisterDWord(SYSTEM_CTRL, SMI_FIELD_SET(peekRegisterDWord(SYSTEM_CTRL), SYSTEM_CTRL, PCI_MASTER, ON));
    ...
    else
    {
        ...
        if ((ddk750_getChipType() == SM718) && (busMasterCounter == 0))
            pokeRegisterDWord(SYSTEM_CTRL, SMI_FIELD_SET(peekRegisterDWord(SYSTEM_CTRL), SYSTEM_CTRL, PCI_MASTER, OFF));
    }
...
)
{
    ...
    if (ddk750_getChipType() == SM750)
    {
        ...
        pokeRegisterDWord(PCI_MASTER_BASE, SMI_FIELD_VALUE(0, PCI_MASTER_BASE, ADDRESS, pciMasterBaseAddress));
    ...
    else
    {
        ...
        pokeRegisterDWord(SM718_PCI_MASTER_BASE, SMI_FIELD_VALUE(0, SM718_PCI_MASTER_BASE, ADDRESS, pciMasterBaseAddress));
...
)
{
    ...
    if (enable != 0)
    {
		...
		{
			/* Enable PCI Slave Burst Write. */
			value = SMI_FIELD_SET(peekRegisterDWord(SYSTEM_CTRL), SYSTEM_CTRL, PCI_BURST, ON); 
			pokeRegisterDWord(SYSTEM_CTRL, value);
        }
    ...
    else
    {
        ...
        if ((ddk750_getChipType() == SM718) && (pciSlaveBurstWriteCounter == 0))
		{
			...
			value = SMI_FIELD_SET(peekRegisterDWord(SYSTEM_CTRL), SYSTEM_CTRL, PCI_BURST, OFF); 
			pokeRegisterDWord(SYSTEM_CTRL, value);
		}
    }
...
)
{
    ...
    if (enable != 0)
    {
        if ((ddk750_getChipType() == SM718) && (pciSlaveBurstReadCounter == 0))
		{
			...
			value = SMI_FIELD_SET(value, SYSTEM_CTRL, PCI_BURST_READ, ON); 

			/* Set the Read Size */
			switch(burstReadSize)
			{
				case 1:
					value = SMI_FIELD_SET(value, SYSTEM_CTRL, PCI_SLAVE_BURST_READ_SIZE, 1);
					break;
				case 2:
					value = SMI_FIELD_SET(value, SYSTEM_CTRL, PCI_SLAVE_BURST_READ_SIZE, 2);
					break;
				case 4:
					value = SMI_FIELD_SET(value, SYSTEM_CTRL, PCI_SLAVE_BURST_READ_SIZE, 4);
					break;
				default:
				case 8:
					value = SMI_FIELD_SET(value, SYSTEM_CTRL, PCI_SLAVE_BURST_READ_SIZE, 8);
					break;
			}
			pokeRegisterDWord(SYSTEM_CTRL, value);
        }
    ...
    else
    {
        ...
        if ((ddk750_getChipType() == SM718) && (pciSlaveBurstReadCounter == 0))
		{
			...
			value = SMI_FIELD_SET(peekRegisterDWord(SYSTEM_CTRL), SYSTEM_CTRL, PCI_BURST_READ, OFF); 
			pokeRegisterDWord(SYSTEM_CTRL, value);
		}
    }
...
void enable2DEngine(unsigned long enable)
{
    ...
    if (enable)
    {
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, DE,  ON);
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, CSC, ON);
    }
    else
    {
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, DE,  OFF);
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, CSC, OFF);
    }
...
void enableZVPort(unsigned long enable)
{
    ...
    if (enable)
    {
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, ZVPORT, ON);
#if 1
        /* Using Software I2C */
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, GPIO, ON);
#endif
    }
    else
    {
        ...
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, ZVPORT, OFF);
    }
...
void enableSSP(unsigned long enable)
{
    ...
    gate = peekRegisterDWord(CURRENT_GATE);
    if (enable)
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, SSP, ON);        
    else
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, SSP, OFF);
...
void enableDMA(unsigned long enable)
{
    ...
    gate = peekRegisterDWord(CURRENT_GATE);
    if (enable)
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, DMA, ON);
    else
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, DMA, OFF);
...
void enableGPIO(unsigned long enable)
{
    ...
    if (enable)
        ...
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, GPIO, OFF);
...
void enablePWM(unsigned long enable)
{
    ...
    gate = peekRegisterDWord(CURRENT_GATE);
    if (enable)
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, PWM, ON);        
    else
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, PWM, OFF);
...
void enableI2C(unsigned long enable)
{
    ...
    gate = peekRegisterDWord(CURRENT_GATE);
    if (enable)
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, I2C, ON);        
    else
        gate = SMI_FIELD_SET(gate, CURRENT_GATE, I2C, OFF);
