...
byte I2CReadEDID(struct i2c_client *client, byte RegAddr, byte NBytes, byte * Data )
{
	byte i = 0;
	while(i<NBytes){
		Data[i] = i2cReadReg(0xA0,RegAddr+i);
		i++;
	}
	return IIC_OK;
}
...
#define ReadBlockEDID(a,b,c)            		I2CReadEDID(siiEDID, a, b, c)
...
byte ReleaseDDC (byte SysCtrlRegVal)
{
	...
	printk("failed to release DDC bus control\n");
