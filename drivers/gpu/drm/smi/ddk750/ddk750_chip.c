...
unsigned long ddk750_getFrameBufSize(void)
{
	...
	sizeSymbol = SMI_FIELD_GET(peekRegisterDWord(MISC_CTRL), MISC_CTRL, LOCALMEM_SIZE);
	switch(sizeSymbol)
	{
...
void ddk750_resetFrameBufferMemory()
{
	...
	{        
    	...
    	ulReg = SMI_FIELD_SET(ulReg, MISC_CTRL, LOCALMEM_RESET, RESET);
    	pokeRegisterDWord(MISC_CTRL, ulReg);

    	ulReg = SMI_FIELD_SET(ulReg, MISC_CTRL, LOCALMEM_RESET, NORMAL);
    	pokeRegisterDWord(MISC_CTRL, ulReg);
	}
}
...
long ddk750_initChipParamEx(initchip_param_t * pInitParam)
{
    ...
    if (pInitParam->setAllEngOff == 1)
    {
        ...
        ulReg = SMI_FIELD_SET(ulReg, PRIMARY_HWC_ADDRESS, ENABLE, DISABLE); 
        ...
        ulReg = SMI_FIELD_SET(ulReg, SECONDARY_HWC_ADDRESS, ENABLE, DISABLE); 
        ...
        ulReg = SMI_FIELD_SET(ulReg, ZV0_CAPTURE_CTRL, CAP, DISABLE); 
        ...
        ulReg = SMI_FIELD_SET(ulReg, ZV1_CAPTURE_CTRL, CAP, DISABLE); 
        ...
        ulReg = SMI_FIELD_SET(ulReg, DMA_ABORT_INTERRUPT, ABORT_1, ABORT);
