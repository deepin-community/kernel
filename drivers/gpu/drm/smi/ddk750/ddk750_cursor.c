...
)
{
    ...
    pokeRegisterDWord(
        ...
        SMI_FIELD_SET(0, PRIMARY_HWC_ADDRESS, EXT, LOCAL) |
        SMI_FIELD_VALUE(0, PRIMARY_HWC_ADDRESS, ADDRESS, base));
    ...
    pokeRegisterDWord(
        ...
        SMI_FIELD_VALUE(0, PRIMARY_HWC_COLOR_12, 1_RGB565, color1) |
        SMI_FIELD_VALUE(0, PRIMARY_HWC_COLOR_12, 2_RGB565, color2) );

    pokeRegisterDWord(
        (dispControl == PRIMARY_CTRL) ? PRIMARY_HWC_COLOR_3 : SECONDARY_HWC_COLOR_3,
        SMI_FIELD_VALUE(0, PRIMARY_HWC_COLOR_3, RGB565, color3));
}
...
{  
    ...
    value = SMI_FIELD_VALUE(0, PRIMARY_HWC_LOCATION, X, dx) |
            SMI_FIELD_VALUE(0, PRIMARY_HWC_LOCATION, Y, dy);
    
    /* Set the top boundary select either partially outside the top boundary
       screen or inside */
    if (topOutside)
        value = SMI_FIELD_SET(value, PRIMARY_HWC_LOCATION, TOP, OUTSIDE);
    else         
        value = SMI_FIELD_SET(value, PRIMARY_HWC_LOCATION, TOP, INSIDE);

    /* Set the left boundary select either partially outside the left boundary
       screen or inside */
    if (leftOutside)
        value = SMI_FIELD_SET(value, PRIMARY_HWC_LOCATION, LEFT, OUTSIDE);
    else        
        value = SMI_FIELD_SET(value, PRIMARY_HWC_LOCATION, LEFT, INSIDE);
...
)
{
    ...
    if (enable) 
        value = SMI_FIELD_SET(value, PRIMARY_HWC_ADDRESS, ENABLE, ENABLE);
    else
        value = SMI_FIELD_SET(value, PRIMARY_HWC_ADDRESS, ENABLE, DISABLE);
