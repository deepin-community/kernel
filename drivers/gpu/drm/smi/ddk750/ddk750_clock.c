...
unsigned long formatPllReg(pll_value_t *pPLL)
{
    ...
    */
    ulPllReg =
        SMI_FIELD_SET(  0, PRIMARY_PLL_CTRL, BYPASS, OFF)
      | SMI_FIELD_SET(  0, PRIMARY_PLL_CTRL, POWER,  ON)
      | SMI_FIELD_SET(  0, PRIMARY_PLL_CTRL, INPUT,  OSC)
      | SMI_FIELD_VALUE(0, PRIMARY_PLL_CTRL, POD,    pPLL->POD)      
      | SMI_FIELD_VALUE(0, PRIMARY_PLL_CTRL, OD,     pPLL->OD)
      | SMI_FIELD_VALUE(0, PRIMARY_PLL_CTRL, N,      pPLL->N)
      | SMI_FIELD_VALUE(0, PRIMARY_PLL_CTRL, M,      pPLL->M);
...
unsigned long getPllValue(clock_type_t clockType, pll_value_t *pPLL)
{
    ...
    pPLL->M = SMI_FIELD_GET(ulPllReg, PRIMARY_PLL_CTRL, M);
    pPLL->N = SMI_FIELD_GET(ulPllReg, PRIMARY_PLL_CTRL, N);
    pPLL->OD = SMI_FIELD_GET(ulPllReg, PRIMARY_PLL_CTRL, OD);
    pPLL->POD = SMI_FIELD_GET(ulPllReg, PRIMARY_PLL_CTRL, POD);    
...
void setMemoryClock(unsigned long frequency)
{
    ...
    if (frequency != 0)        
    {
        ...
        {   
            default:
            case 1:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, M2XCLK, DIV_1);
                break;
            case 2:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, M2XCLK, DIV_2);
                break;
            case 3:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, M2XCLK, DIV_3);
                break;
            case 4:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, M2XCLK, DIV_4);
                break;
        }
...
void setMasterClock(unsigned long frequency)
{
    ...
    if (frequency != 0)
    {
        ...
        switch(divisor)
        {
            default:
            case 3:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, MCLK, DIV_3);
                break;
            case 4:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, MCLK, DIV_4);
                break;
            case 6:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, MCLK, DIV_6);
                break;
            case 8:
                ulReg = SMI_FIELD_SET(ulReg, CURRENT_GATE, MCLK, DIV_8);
                break;
        }
...
unsigned long getMasterClock()
{
    ...
    switch (SMI_FIELD_GET(value, CURRENT_GATE, MCLK))
    {
...
unsigned long getMemoryClock()
{
    ...
    /* Get the divisor */
    value = peekRegisterDWord(CURRENT_GATE);
    switch (SMI_FIELD_GET(value, CURRENT_GATE, M2XCLK))
    {
...
)
{
    ...
    pokeRegisterDWord(MISC_CTRL, SMI_FIELD_SET(peekRegisterDWord(MISC_CTRL), MISC_CTRL, PLL_CLK_COUNT, ON));
    ...
    {
        startCount = endCount;
        value = SMI_FIELD_GET(peekRegisterDWord(PLL_CLK_COUNT), PLL_CLK_COUNT, COUNTER);
        endCount = value;
    ...
    pokeRegisterDWord(MISC_CTRL, SMI_FIELD_SET(peekRegisterDWord(MISC_CTRL), MISC_CTRL, PLL_CLK_COUNT, OFF));
}
