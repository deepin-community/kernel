...
unsigned char isDualPanelEnable(void)
{
    ...
    value = SMI_FIELD_GET(peekRegisterDWord(PRIMARY_DISPLAY_CTRL), PRIMARY_DISPLAY_CTRL, DUAL_DISPLAY);
...
{
    if (SMI_FIELD_GET(peekRegisterDWord(PRIMARY_DISPLAY_CTRL), PRIMARY_DISPLAY_CTRL, DOUBLE_PIXEL) == 
...
)
{
    ...
    {
        case TFT_18BIT:
            value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, DUAL_DISPLAY, ENABLE);
            value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, DOUBLE_PIXEL, DISABLE);
            break;
        case TFT_24BIT:
            value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, DUAL_DISPLAY, DISABLE);
            value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, DOUBLE_PIXEL, DISABLE);
            break;
        case TFT_36BIT:
            value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, DUAL_DISPLAY, DISABLE);
            value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, DOUBLE_PIXEL, ENABLE);
            break;
        default:
            return (-1);
    }
...
void waitNextVerticalSync(disp_control_t dispControl, unsigned long vsync_count)
{
    ...
    if (dispControl == PRIMARY_CTRL)
    {
        ...
        if ((SMI_FIELD_GET(peekRegisterDWord(PRIMARY_PLL_CTRL), PRIMARY_PLL_CTRL, POWER) ==
             PRIMARY_PLL_CTRL_POWER_OFF) ||
            (SMI_FIELD_GET(peekRegisterDWord(PRIMARY_DISPLAY_CTRL), PRIMARY_DISPLAY_CTRL, TIMING) ==
        ...
        while (vsync_count-- > 0)
        {
            ...
            do
            {
                status = SMI_FIELD_GET(peekRegisterDWord(SYSTEM_CTRL),
                                   SYSTEM_CTRL,
                                   PRIMARY_VSYNC);
    ...
    else
    {
        ...
        if ((SMI_FIELD_GET(peekRegisterDWord(SECONDARY_PLL_CTRL), SECONDARY_PLL_CTRL, POWER) ==
             SECONDARY_PLL_CTRL_POWER_OFF) ||
            (SMI_FIELD_GET(peekRegisterDWord(SECONDARY_DISPLAY_CTRL), SECONDARY_DISPLAY_CTRL, TIMING) ==
...
void waitVSyncLine(disp_control_t dispControl)
{
    ...
    {
        if (dispControl == PRIMARY_CTRL)
            value = SMI_FIELD_GET(peekRegisterDWord(PRIMARY_CURRENT_LINE), PRIMARY_CURRENT_LINE, LINE);
        else
            value = SMI_FIELD_GET(peekRegisterDWord(SECONDARY_CURRENT_LINE), SECONDARY_CURRENT_LINE, LINE);
    }
...
void swPanelPowerSequence_SM750LE(disp_state_t dispState, unsigned long vsync_delay)
{
    ...
    {
        ulMask = SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, EN,   HIGH) |
                 SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, BIAS, HIGH) |
                 SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, DATA, ENABLE) |
                 SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, VDD,  HIGH);
        
        pokeRegisterDWord(DISPLAY_CONTROL_750LE, (ulDispControl | ulMask));
    }
    else
    {
        ulMask = SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, EN,   LOW) |
                 SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, BIAS, LOW) |
                 SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, DATA, DISABLE) |
                 SMI_FIELD_SET(0, DISPLAY_CONTROL_750LE, VDD,  LOW);
...
void swPanelPowerSequence(disp_state_t dispState, unsigned long vsync_delay)
{
    ...
    {
        /* Turn on FPVDDEN. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, FPVDDEN, HIGH);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
        primaryWaitVerticalSync(vsync_delay);

        /* Turn on FPDATA. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, DATA, ENABLE);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
        primaryWaitVerticalSync(vsync_delay);

        /* Turn on FPVBIAS. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, VBIASEN, HIGH);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
        primaryWaitVerticalSync(vsync_delay);

        /* Turn on FPEN. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, FPEN, HIGH);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
    }
    ...
    {
        /* Turn off FPEN. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, FPEN, LOW);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
        primaryWaitVerticalSync(vsync_delay);

        /* Turn off FPVBIASEN. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, VBIASEN, LOW);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
        primaryWaitVerticalSync(vsync_delay);

        /* Turn off FPDATA. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, DATA, DISABLE);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
        primaryWaitVerticalSync(vsync_delay);

        /* Turn off FPVDDEN. */
        primaryControl = SMI_FIELD_SET(primaryControl, PRIMARY_DISPLAY_CTRL, FPVDDEN, LOW);
        pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, primaryControl);
    }
...
void setDAC(disp_state_t state)
{
    ...
    if (state == DISP_ON)
    {
        pokeRegisterDWord(MISC_CTRL, SMI_FIELD_SET(peekRegisterDWord(MISC_CTRL),
                                               MISC_CTRL,
                                               DAC_POWER,
                                               ON));
    }
...
void setDisplayControl(disp_control_t dispControl, disp_state_t dispState)
{
    ...
    if (dispControl == PRIMARY_CTRL)
    {
        ...
        if (dispState == DISP_ON)
        {
             ...
             */
            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, PRIMARY_DISPLAY_CTRL, TIMING, ENABLE);
            pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, ulDisplayCtrlReg);
            
            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, PRIMARY_DISPLAY_CTRL, PLANE, ENABLE);
        ...
        else
        {
            /* When turning off, there is no rule on the programming sequence since whenever the
               ...
               Note: Modifying the plane bit will take effect on the next vertical sync. Need to
                     ...
                     enable bit. 
             */
            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, PRIMARY_DISPLAY_CTRL, PLANE, DISABLE);
            pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, ulDisplayCtrlReg);

            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, PRIMARY_DISPLAY_CTRL, TIMING, DISABLE);
            pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, ulDisplayCtrlReg);
        }
    ...
    else if(dispControl == SECONDARY_CTRL)
    {
        ...
        if (dispState == DISP_ON)
        {
            /* Timing should be enabled first before enabling the plane because changing at the
               same time does not guarantee that the plane will also enabled or disabled. 
             */
            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, SECONDARY_DISPLAY_CTRL, TIMING, ENABLE);
            pokeRegisterDWord(SECONDARY_DISPLAY_CTRL, ulDisplayCtrlReg);
                        
            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, SECONDARY_DISPLAY_CTRL, PLANE, ENABLE);
             ...
             * reserved bits are needed to be masked out.
             */
            ulReservedBits = SMI_FIELD_SET(0, SECONDARY_DISPLAY_CTRL, RESERVED_1_MASK, ENABLE) |
                             SMI_FIELD_SET(0, SECONDARY_DISPLAY_CTRL, RESERVED_2_MASK, ENABLE) |
                             SMI_FIELD_SET(0, SECONDARY_DISPLAY_CTRL, RESERVED_3_MASK, ENABLE) |
                             SMI_FIELD_SET(0, SECONDARY_DISPLAY_CTRL, RESERVED_4_MASK, ENABLE);
        ...
        else
        {
            /* When turning off, there is no rule on the programming sequence since whenever the
               ...
               Note: Modifying the plane bit will take effect on the next vertical sync. Need to
                     ...
                     enable bit.
             */
            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, SECONDARY_DISPLAY_CTRL, PLANE, DISABLE);
            pokeRegisterDWord(SECONDARY_DISPLAY_CTRL, ulDisplayCtrlReg);

            ulDisplayCtrlReg = SMI_FIELD_SET(ulDisplayCtrlReg, SECONDARY_DISPLAY_CTRL, TIMING, DISABLE);
            pokeRegisterDWord(SECONDARY_DISPLAY_CTRL, ulDisplayCtrlReg);
        }
...
)
{
    ...
    if (dispPath == PANEL_PATH)
    {
        ...
        if (dispState == DISP_ON)
        {
            ...
            if (modeParam.clock_phase_polarity == POS)
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_HIGH);
            else
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_LOW);
                        
            /* Adjust the VSync polarity */
            if (modeParam.vertical_sync_polarity == POS)
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, VSYNC_PHASE, ACTIVE_HIGH);
            else
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, VSYNC_PHASE, ACTIVE_LOW);

            /* Adjust the HSync polarity */
            if (modeParam.horizontal_sync_polarity == POS)
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, HSYNC_PHASE, ACTIVE_HIGH);
            else
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, HSYNC_PHASE, ACTIVE_LOW);

            /* Display control is not swapped, so use the normal display data flow */
            if (dispControl == PRIMARY_CTRL)
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, SELECT, PRIMARY);                    
            else    /* Secondary Control */
                control = SMI_FIELD_SET(control, PRIMARY_DISPLAY_CTRL, SELECT, SECONDARY);
        }
        ...
        //    hwDeviceExtension1[1]->ChipID==0x718)
        {
            ...
            pokeRegisterDWord(MODE0_GATE, SMI_FIELD_SET(clock0, MODE0_GATE, MCLK, 42MHZ)); 
            clock1 = peekRegisterDWord(MODE1_GATE);
            pokeRegisterDWord(MODE1_GATE, SMI_FIELD_SET(clock1, MODE1_GATE, MCLK, 42MHZ));                 
        }
#endif
        ...
        //    hwDeviceExtension1[1]->ChipID==0x718)
        {
            ...
            MiscControl= SMI_FIELD_SET(MiscControl, MISC_CTRL, LOCALMEM_RESET, RESET);
            pokeRegisterDWord(MISC_CTRL, MiscControl);
            MiscControl= SMI_FIELD_SET(MiscControl, MISC_CTRL, LOCALMEM_RESET, NORMAL);
            pokeRegisterDWord(MISC_CTRL, MiscControl);            
        }
#endif
    ...
    else if(dispPath == CRT_PATH)   /* CRT Path */
    {
        ...
        if (dispState == DISP_ON)
        {
            /* Adjust the Clock polarity */
            if (modeParam.clock_phase_polarity == POS)
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_HIGH);
            else
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_LOW);
                
            /* Adjust the VSync polarity */
            if (modeParam.vertical_sync_polarity == POS)
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, VSYNC_PHASE, ACTIVE_HIGH);
            else
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, VSYNC_PHASE, ACTIVE_LOW);
    
            /* Adjust the HSync polarity */
            if (modeParam.horizontal_sync_polarity == POS)
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, HSYNC_PHASE, ACTIVE_HIGH);
            else
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, HSYNC_PHASE, ACTIVE_LOW);

            /* Display control is not swapped, so use the normal display data flow */
            if (dispControl == PRIMARY_CTRL)
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, SELECT, PRIMARY);
            else
                control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, SELECT, SECONDARY);
            
            /* Enable the CRT Pixel */
            control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, BLANK, OFF);
        }
        else
        {
            /* Disable the CRT Pixel */
            control = SMI_FIELD_SET(control, SECONDARY_DISPLAY_CTRL, BLANK, ON);
        }    
...
)
{
    ...
    if (pRedValue != (unsigned char *)0)
        *pRedValue = (unsigned char)SMI_FIELD_GET(value, SECONDARY_MONITOR_DETECT, RED);
        
    if (pGreenValue != (unsigned char *)0)
        *pGreenValue = (unsigned char)SMI_FIELD_GET(value, SECONDARY_MONITOR_DETECT, GREEN);
        
    if (pBlueValue != (unsigned char *)0)
        *pBlueValue = (unsigned char)SMI_FIELD_GET(value, SECONDARY_MONITOR_DETECT, BLUE);
}
...
)
{
    ...
    value = ((unsigned long)red << 16) |
            ...
            ((unsigned long)blue);
    value = SMI_FIELD_SET(value, SECONDARY_MONITOR_DETECT, ENABLE, ENABLE);
    ...
    if (SMI_FIELD_GET(peekRegisterDWord(SECONDARY_MONITOR_DETECT), SECONDARY_MONITOR_DETECT, VALUE) ==
    ...
    value = peekRegisterDWord(SECONDARY_MONITOR_DETECT);
    value = SMI_FIELD_SET(value, SECONDARY_MONITOR_DETECT, ENABLE, DISABLE);
    pokeRegisterDWord(SECONDARY_MONITOR_DETECT, value);
...
)
{
    ...
    if (dispCtrl == PRIMARY_CTRL)
    {
        ...
        if (SMI_FIELD_GET(value, SECONDARY_DISPLAY_CTRL, EXPANSION) == SECONDARY_DISPLAY_CTRL_EXPANSION_ENABLE)
            return 1;
    }
    else
    {
        value = peekRegisterDWord(SECONDARY_SCALE);
        if ((SMI_FIELD_GET(value, SECONDARY_SCALE, VERTICAL_SCALE) != 0) ||
            (SMI_FIELD_GET(value, SECONDARY_SCALE, HORIZONTAL_SCALE) != 0))
            return 1;
    }
