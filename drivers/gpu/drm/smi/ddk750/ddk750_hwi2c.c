...
)
{
    ...
    value = SMI_FIELD_SET(value, GPIO_MUX, 30, I2C) |
			SMI_FIELD_SET(0, GPIO_MUX, 31, I2C);
    ...
    if (busSpeedMode == 0)
        value = SMI_FIELD_SET(value, I2C_CTRL, MODE, STANDARD);
    else
        value = SMI_FIELD_SET(value, I2C_CTRL, MODE, FAST);        
    value = SMI_FIELD_SET(value, I2C_CTRL, EN, ENABLE);
...
void ddk750_hwI2CClose()
{
    ...
    value = peekRegisterByte(I2C_CTRL);
    value = SMI_FIELD_SET(value, I2C_CTRL, EN, DISABLE);
    pokeRegisterByte(I2C_CTRL, value);
    ...
    value = peekRegisterDWord(GPIO_MUX);
    value = SMI_FIELD_SET(value, GPIO_MUX, 30, GPIO);
    value = SMI_FIELD_SET(value, GPIO_MUX, 31, GPIO);
    pokeRegisterDWord(GPIO_MUX, value);
}
...
static long hwI2CWaitTXDone(void)
{
	...
	while ((SMI_FIELD_GET(peekRegisterByte(I2C_STATUS), I2C_STATUS, TX) != I2C_STATUS_TX_COMPLETED) &&
           (timeout != 0))
		timeout--;
...
)
{
    ...
    do
    {
        ...
        /* Start the I2C */
        pokeRegisterByte(I2C_CTRL, SMI_FIELD_SET(peekRegisterByte(I2C_CTRL), I2C_CTRL, CTRL, START));
