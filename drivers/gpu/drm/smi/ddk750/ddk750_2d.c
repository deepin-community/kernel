...
void ddk750_deInit(void)
{
    ...
    */
    POKE_32(DE_STRETCH_FORMAT,
        SMI_FIELD_SET  (0, DE_STRETCH_FORMAT, PATTERN_XY,   NORMAL)  |
        SMI_FIELD_VALUE(0, DE_STRETCH_FORMAT, PATTERN_Y,    0)       |
        SMI_FIELD_VALUE(0, DE_STRETCH_FORMAT, PATTERN_X,    0)       |
        SMI_FIELD_SET  (0, DE_STRETCH_FORMAT, ADDRESSING,   XY)      |
        SMI_FIELD_VALUE(0, DE_STRETCH_FORMAT, SOURCE_HEIGHT,3));
...
void deReset()
{
	...
	if (chipType == SM750 || chipType == SM718)
	{
    	...
    	sysCtrl = SMI_FIELD_SET(sysCtrl, SYSTEM_CTRL, DE_ABORT, ON);
    	POKE_32(SYSTEM_CTRL, sysCtrl);

    	/* Re-enable 2D engine to normal state */
    	sysCtrl = PEEK_32(SYSTEM_CTRL);
    	sysCtrl = SMI_FIELD_SET(sysCtrl, SYSTEM_CTRL, DE_ABORT, OFF);
    	POKE_32(SYSTEM_CTRL, sysCtrl);
	}
	else /* For SM750LE and SM750HS series */
	{
		...
		sysCtrl = SMI_FIELD_SET(sysCtrl, DE_STATE1, DE_ABORT, ON);
		POKE_32(DE_STATE1, sysCtrl);

		/* Re-enable 2D engine to normal state */
        sysCtrl = PEEK_32(DE_STATE1);
		sysCtrl = SMI_FIELD_SET(sysCtrl, DE_STATE1, DE_ABORT, OFF);
		POKE_32(DE_STATE1, sysCtrl);
	}
}
...
long deWaitForNotBusy(void)
{
	...
	if (chipType == SM750 || chipType == SM718)
	{
    	while (i--)
    	{
    	    ...
    	    if ((SMI_FIELD_GET(dwVal, SYSTEM_CTRL, DE_STATUS)      == SYSTEM_CTRL_DE_STATUS_IDLE) &&
    	        (SMI_FIELD_GET(dwVal, SYSTEM_CTRL, DE_FIFO)        == SYSTEM_CTRL_DE_FIFO_EMPTY) &&
    	        (SMI_FIELD_GET(dwVal, SYSTEM_CTRL, CSC_STATUS)     == SYSTEM_CTRL_CSC_STATUS_IDLE) &&
    	        (SMI_FIELD_GET(dwVal, SYSTEM_CTRL, DE_MEM_FIFO)    == SYSTEM_CTRL_DE_MEM_FIFO_EMPTY))
    	    {
    ...
    else /* For SM750LE & SM750HS */
    {
        while (i--)
        {
            ...
            if ((SMI_FIELD_GET(dwVal, DE_STATE2, DE_STATUS)      == DE_STATE2_DE_STATUS_IDLE) &&
                (SMI_FIELD_GET(dwVal, DE_STATE2, DE_FIFO)        == DE_STATE2_DE_FIFO_EMPTY) &&
                (SMI_FIELD_GET(dwVal, DE_STATE2, DE_MEM_FIFO)    == DE_STATE2_DE_MEM_FIFO_EMPTY))
            {
                return 0; /* Return because engine idle */
			}
        }
...
long deWaitIdle(unsigned long i)
{
    ...
    if (ddk750_getChipType() == SM750LE)
    {
		...
		while (i--)
		{
			...
			if (SMI_FIELD_GET(dwVal, DE_CONTROL, STATUS) == DE_CONTROL_STATUS_STOP)
				return 0;
		}
		return -1;
#else
...
unsigned long y2)     /* Note that the region will not include x2 and y2 */
{
     ...
     */
    POKE_32(DE_CLIP_TL,
        SMI_FIELD_VALUE(0, DE_CLIP_TL, TOP, y1) |
        ((enable)?
          SMI_FIELD_SET(0, DE_CLIP_TL, STATUS, ENABLE)
        : SMI_FIELD_SET(0, DE_CLIP_TL, STATUS, DISABLE))|
        SMI_FIELD_SET  (0, DE_CLIP_TL, INHIBIT,OUTSIDE) |
        SMI_FIELD_VALUE(0, DE_CLIP_TL, LEFT, x1));

    /* Lower right corner */
    POKE_32(DE_CLIP_BR,
        SMI_FIELD_VALUE(0, DE_CLIP_BR, BOTTOM,y2) |
        SMI_FIELD_VALUE(0, DE_CLIP_BR, RIGHT, x2));
...
unsigned long ulColor)    /* Color to compare. */
{
    ...
    */
    de_ctrl = PEEK_32(DE_CONTROL)
              & SMI_FIELD_CLEAR(DE_CONTROL, TRANSPARENCY)
              & SMI_FIELD_CLEAR(DE_CONTROL, TRANSPARENCY_MATCH)
              & SMI_FIELD_CLEAR(DE_CONTROL, TRANSPARENCY_SELECT);
    ...
    */
    de_ctrl |=
    ((enable)?
      SMI_FIELD_SET(0, DE_CONTROL, TRANSPARENCY, ENABLE)
    : SMI_FIELD_SET(0, DE_CONTROL, TRANSPARENCY, DISABLE))        |
    ((tMatch)?
      SMI_FIELD_SET(0, DE_CONTROL, TRANSPARENCY_MATCH, TRANSPARENT)
    : SMI_FIELD_SET(0, DE_CONTROL, TRANSPARENCY_MATCH, OPAQUE)) |
    ((tSelect)?
      SMI_FIELD_SET(0, DE_CONTROL, TRANSPARENCY_SELECT, DESTINATION)
    : SMI_FIELD_SET(0, DE_CONTROL, TRANSPARENCY_SELECT, SOURCE));
...
unsigned long deGetTransparency(void)
{
    ...
    de_ctrl &= 
        SMI_FIELD_MASK(DE_CONTROL_TRANSPARENCY_MATCH) | 
        SMI_FIELD_MASK(DE_CONTROL_TRANSPARENCY_SELECT)| 
        SMI_FIELD_MASK(DE_CONTROL_TRANSPARENCY);
...
)
{
    ...
    {
        case 8:
            de_format = SMI_FIELD_SET(de_format, DE_STRETCH_FORMAT, PIXEL_FORMAT, 8);
            break;
        default:
        case 16:
            de_format = SMI_FIELD_SET(de_format, DE_STRETCH_FORMAT, PIXEL_FORMAT, 16);
            break;
        case 32:
            de_format = SMI_FIELD_SET(de_format, DE_STRETCH_FORMAT, PIXEL_FORMAT, 32);
            break;
    }
...
unsigned long rop2)   /* ROP value */
{
    ...
    */
    POKE_32(DE_PITCH,
        SMI_FIELD_VALUE(0, DE_PITCH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_PITCH, SOURCE,      (dPitch/bytePerPixel)));

    /* Screen Window width in Pixels.
       2D engine uses this value to calculate the linear address in frame buffer for a given point.
    */
    POKE_32(DE_WINDOW_WIDTH,
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, SOURCE,      (dPitch/bytePerPixel)));
...
//        DDKDEBUGPRINT((DE_LEVEL, "ROP != ROP_COPY, width * bytePerPixel = %x (> 192 bytes)\n", width * bytePerPixel));
        ...
        while (1)
        {
            ...
            POKE_32(DE_DESTINATION,
                SMI_FIELD_SET  (0, DE_DESTINATION, WRAP, DISABLE) |
                SMI_FIELD_VALUE(0, DE_DESTINATION, X,    x)  |
                SMI_FIELD_VALUE(0, DE_DESTINATION, Y,    y));
                
            POKE_32(DE_DIMENSION,
                SMI_FIELD_VALUE(0, DE_DIMENSION, X,    xChunk) |
                SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, height));

            de_ctrl = 
                SMI_FIELD_SET  (0, DE_CONTROL,  STATUS,     START)          |
                SMI_FIELD_SET  (0, DE_CONTROL,  DIRECTION,  LEFT_TO_RIGHT)  |
                //SMI_FIELD_SET  (0, DE_CONTROL,LAST_PIXEL, OFF)            |
                SMI_FIELD_SET  (0, DE_CONTROL,  COMMAND,    RECTANGLE_FILL) |
                SMI_FIELD_SET  (0, DE_CONTROL,  ROP_SELECT, ROP2)           |
                SMI_FIELD_VALUE(0, DE_CONTROL,  ROP,        rop2);
...
#endif
    {
        ...
        POKE_32(DE_DIMENSION,
            SMI_FIELD_VALUE(0, DE_DIMENSION, X,    width) |
            SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, height));
...
)
{
    unsigned long de_ctrl =
        SMI_FIELD_SET  (0, DE_CONTROL, STATUS,      START)           |
        SMI_FIELD_SET  (0, DE_CONTROL, QUICK_START, ENABLE)          |
        SMI_FIELD_SET  (0, DE_CONTROL, LAST_PIXEL,  OFF)             |
        SMI_FIELD_SET  (0, DE_CONTROL, DIRECTION,   LEFT_TO_RIGHT)   |
        SMI_FIELD_SET  (0, DE_CONTROL, COMMAND,     TRAPEZOID_FILL)  |
        SMI_FIELD_SET  (0, DE_CONTROL, ROP_SELECT,  ROP2)            |
        SMI_FIELD_VALUE(0, DE_CONTROL, ROP,         rop2);
    ...
    */
    POKE_32(DE_PITCH,
        SMI_FIELD_VALUE(0, DE_PITCH, DESTINATION, dPitch / BYTE_PER_PIXEL(bpp)) |
        SMI_FIELD_VALUE(0, DE_PITCH, SOURCE, dPitch / BYTE_PER_PIXEL(bpp)));

    /* Screen Window width in Pixels.
       2D engine uses this value to calculate the linear address in frame buffer for a given point.
    */
    POKE_32(DE_WINDOW_WIDTH,
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, DESTINATION, dPitch / BYTE_PER_PIXEL(bpp)) |
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, SOURCE,      dPitch / BYTE_PER_PIXEL(bpp)));
    ...
    POKE_32(DE_DIMENSION,
        SMI_FIELD_VALUE(0, DE_DIMENSION, X,    length) |
        SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, 0));
...
)
{
    ...
    POKE_32(DE_DESTINATION,
        SMI_FIELD_VALUE(PEEK_32(DE_DESTINATION), DE_DESTINATION, X,    x));
        
    /* Set the line length */
    POKE_32(DE_DIMENSION,
        SMI_FIELD_VALUE(PEEK_32(DE_DIMENSION), DE_DIMENSION, X,    length));
...
long deStopTrapezoidFill(void)
{
    ...
    POKE_32(DE_CONTROL, SMI_FIELD_SET(PEEK_32(DE_CONTROL), DE_CONTROL, QUICK_START, DISABLE));
...
unsigned long rop2)   /* ROP value */
{
    ...
    */
    POKE_32(DE_PITCH,
        SMI_FIELD_VALUE(0, DE_PITCH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_PITCH, SOURCE,      (sPitch/bytePerPixel)));
    ...
    */
    POKE_32(DE_WINDOW_WIDTH,
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, SOURCE,      (sPitch/bytePerPixel)));
    ...
    if ((rop2 != ROP2_COPY) && ((width * bytePerPixel) > 192))
    {
        ...
        while (1)
        {
            deWaitForNotBusy();
            POKE_32(DE_SOURCE,
                SMI_FIELD_SET  (0, DE_SOURCE, WRAP, DISABLE) |
                SMI_FIELD_VALUE(0, DE_SOURCE, X_K1, sx)   |
                SMI_FIELD_VALUE(0, DE_SOURCE, Y_K2, sy));
            POKE_32(DE_DESTINATION,
                SMI_FIELD_SET  (0, DE_DESTINATION, WRAP, DISABLE) |
                SMI_FIELD_VALUE(0, DE_DESTINATION, X,    dx)  |
                SMI_FIELD_VALUE(0, DE_DESTINATION, Y,    dy));
            POKE_32(DE_DIMENSION,
                SMI_FIELD_VALUE(0, DE_DIMENSION, X,    xChunk) |
                SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, height));
            ...
            de_ctrl = 
                SMI_FIELD_VALUE(0, DE_CONTROL, ROP, rop2) |
                SMI_FIELD_SET(0, DE_CONTROL, ROP_SELECT, ROP2) |
                SMI_FIELD_SET(0, DE_CONTROL, COMMAND, BITBLT) |
                ((nDirection == RIGHT_TO_LEFT) ? 
                SMI_FIELD_SET(0, DE_CONTROL, DIRECTION, RIGHT_TO_LEFT)
                : SMI_FIELD_SET(0, DE_CONTROL, DIRECTION, LEFT_TO_RIGHT)) |
                SMI_FIELD_SET(0, DE_CONTROL, STATUS, START);
...
)
{
    ...
    */
    ...
    value = SMI_FIELD_VALUE(0, DE_WINDOW_SOURCE_BASE, ADDRESS, (unsigned long)pSBase);        
    pciMasterBaseAddress = ((unsigned long)pSBase & 0xFC000000) >> 24;
    POKE_32(PCI_MASTER_BASE, SMI_FIELD_VALUE(0, PCI_MASTER_BASE, ADDRESS, pciMasterBaseAddress));
    value = SMI_FIELD_SET(value, DE_WINDOW_SOURCE_BASE, EXT, EXTERNAL);
...
)
{
    ...
    */
    ...
    POKE_32(DE_SOURCE,
        SMI_FIELD_SET  (0, DE_SOURCE, WRAP, DISABLE) |
        SMI_FIELD_VALUE(0, DE_SOURCE, X_K1, 0)       |
        SMI_FIELD_VALUE(0, DE_SOURCE, Y_K2, 0));
    ...
    de_ctrl = 
        SMI_FIELD_VALUE(0, DE_CONTROL, ROP, rop2)         |
        SMI_FIELD_SET(0, DE_CONTROL, ROP_SELECT, ROP2)    |
        SMI_FIELD_SET(0, DE_CONTROL, COMMAND, HOST_WRITE) |
        SMI_FIELD_SET(0, DE_CONTROL, HOST, COLOR)         |
        SMI_FIELD_SET(0, DE_CONTROL, STATUS, START);
...
unsigned long rop2)     /* ROP value */
{
    ...
    */
    ...
    POKE_32(DE_SOURCE,
        SMI_FIELD_SET  (0, DE_SOURCE, WRAP, DISABLE)       |
        SMI_FIELD_VALUE(0, DE_SOURCE, X_K1_MONO, startBit));
    ...
    de_ctrl = SMI_FIELD_VALUE(0, DE_CONTROL, ROP, rop2)         |
              SMI_FIELD_SET(0, DE_CONTROL, ROP_SELECT, ROP2)    |
              SMI_FIELD_SET(0, DE_CONTROL, COMMAND, HOST_WRITE) |
              SMI_FIELD_SET(0, DE_CONTROL, HOST, MONO)          |
              SMI_FIELD_SET(0, DE_CONTROL, STATUS, START);
...
)
{
    ...
    */
    POKE_32(DE_PITCH,
        SMI_FIELD_VALUE(0, DE_PITCH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_PITCH, SOURCE,      1));
    ...
    */
    POKE_32(DE_WINDOW_WIDTH,
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, SOURCE,      0)); /* Source window width is don't care */

    POKE_32(DE_SOURCE,
        SMI_FIELD_SET  (0, DE_SOURCE, WRAP, DISABLE) |
        SMI_FIELD_VALUE(0, DE_SOURCE, X_K1, startBit)   |
        SMI_FIELD_VALUE(0, DE_SOURCE, Y_K2, 0));
    ...
    de_ctrl = 
        SMI_FIELD_VALUE(0, DE_CONTROL, ROP, rop2) |
        SMI_FIELD_SET(0, DE_CONTROL, ROP_SELECT, ROP2) |
        SMI_FIELD_SET(0, DE_CONTROL, COMMAND, FONT) |
        SMI_FIELD_SET(0, DE_CONTROL, HOST, MONO) |
        SMI_FIELD_VALUE(0, DE_CONTROL, MONO_DATA, packed) |
        SMI_FIELD_SET(0, DE_CONTROL, STATUS, START);
...
unsigned long rop2)   /* ROP value */
{
    ...
    */
    POKE_32(DE_PITCH,
        SMI_FIELD_VALUE(0, DE_PITCH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_PITCH, SOURCE,      0));      /* Source pitch is don't care */
    ...
    */
    ...
    POKE_32(DE_SOURCE,
        SMI_FIELD_SET  (0, DE_SOURCE, WRAP, DISABLE) |
        SMI_FIELD_VALUE(0, DE_SOURCE, X_K1, 0)   |
        SMI_FIELD_VALUE(0, DE_SOURCE, Y_K2, 0));    /* Source data starts at location (0, 0) */
...
unsigned long rop2)       /* ROP value */
{
    ...
    */
    POKE_32(DE_WINDOW_WIDTH,
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, DESTINATION, (dPitch/bytePerPixel)) |
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, SOURCE,      distBetweenFont));
    ...
    */
    POKE_32(DE_SOURCE,
        SMI_FIELD_SET  (0, DE_SOURCE, WRAP, DISABLE) |
        SMI_FIELD_VALUE(0, DE_SOURCE, X_K1, 0)   |
        SMI_FIELD_VALUE(0, DE_SOURCE, Y_K2, fontNumber));
...
)
{
    ...
    */
    POKE_32(DE_PITCH,
        SMI_FIELD_VALUE(0, DE_PITCH, DESTINATION, dPitch / BYTE_PER_PIXEL(bpp)) |
        SMI_FIELD_VALUE(0, DE_PITCH, SOURCE, sPitch / BYTE_PER_PIXEL(bpp)));
    ...
    */
    POKE_32(DE_WINDOW_WIDTH,
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, DESTINATION, dPitch / BYTE_PER_PIXEL(bpp)) |
        SMI_FIELD_VALUE(0, DE_WINDOW_WIDTH, SOURCE,      sPitch / BYTE_PER_PIXEL(bpp)));
        
    /* Set the pixel format of the destination */
    deSetPixelFormat(bpp);
       
    /* Setup Control Register */
    de_ctrl = SMI_FIELD_SET(0, DE_CONTROL, STATUS, START)    |
              SMI_FIELD_SET(0, DE_CONTROL, COMMAND, ROTATE)  |
              SMI_FIELD_SET(0, DE_CONTROL, ROP_SELECT, ROP2) |
              SMI_FIELD_VALUE(0, DE_CONTROL, ROP, rop2)      |
              ((repeatEnable == 1) ? 
                    SMI_FIELD_SET(0, DE_CONTROL, REPEAT_ROTATE, ENABLE) :
                    SMI_FIELD_SET(0, DE_CONTROL, REPEAT_ROTATE, DISABLE)) |
              deGetTransparency();
    ...
    switch (rotateDirection)
    {
        case ROTATE_NORMAL:
            ...
            de_ctrl |= (SMI_FIELD_SET(0, DE_CONTROL, STEP_X, POSITIVE) |
                        SMI_FIELD_SET(0, DE_CONTROL, STEP_Y, POSITIVE));
        ...
        case ROTATE_180_DEGREE:
            ...
            de_ctrl |= (SMI_FIELD_SET(0, DE_CONTROL, STEP_X, NEGATIVE) |
                        SMI_FIELD_SET(0, DE_CONTROL, STEP_Y, NEGATIVE));
...
)
{
    ...
    */
    ...
    /* Set the line length and width */
    POKE_32(DE_DIMENSION,
        SMI_FIELD_VALUE(0, DE_DIMENSION, X,    1) |
        SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, length));
    ...
    de_ctrl = 
        SMI_FIELD_SET  (0, DE_CONTROL,  STATUS,     START)         |
        SMI_FIELD_SET  (0, DE_CONTROL,  DIRECTION,  LEFT_TO_RIGHT) |
        SMI_FIELD_SET  (0, DE_CONTROL,  MAJOR,      Y)             |
        SMI_FIELD_SET  (0, DE_CONTROL,  STEP_X,     NEGATIVE)      |
        SMI_FIELD_SET  (0, DE_CONTROL,  STEP_Y,     NEGATIVE)      |
        SMI_FIELD_SET  (0, DE_CONTROL,  LAST_PIXEL, OFF)           |
        SMI_FIELD_SET  (0, DE_CONTROL,  COMMAND,    SHORT_STROKE)  |
        SMI_FIELD_SET  (0, DE_CONTROL,  ROP_SELECT, ROP2)          |
        SMI_FIELD_VALUE(0, DE_CONTROL,  ROP,        rop2);
...
)
{
    ...
    */
    ...
    POKE_32(DE_DIMENSION,
        SMI_FIELD_VALUE(0, DE_DIMENSION, X,    length) |
        SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, 1));
    ...
    de_ctrl = 
        SMI_FIELD_SET  (0, DE_CONTROL,  STATUS,     START)         |
        SMI_FIELD_SET  (0, DE_CONTROL,  DIRECTION,  RIGHT_TO_LEFT) |
        SMI_FIELD_SET  (0, DE_CONTROL,  MAJOR,      X)             |
        SMI_FIELD_SET  (0, DE_CONTROL,  STEP_X,     NEGATIVE)      |
        SMI_FIELD_SET  (0, DE_CONTROL,  STEP_Y,     NEGATIVE)      |
        SMI_FIELD_SET  (0, DE_CONTROL,  LAST_PIXEL, OFF)           |
        SMI_FIELD_SET  (0, DE_CONTROL,  COMMAND,    SHORT_STROKE)  |
...
)
{
    unsigned long de_ctrl =
        ...
        SMI_FIELD_SET  (0, DE_CONTROL, DIRECTION,  LEFT_TO_RIGHT) |
        SMI_FIELD_SET  (0, DE_CONTROL, MAJOR,      X)             |
        SMI_FIELD_SET  (0, DE_CONTROL, STEP_X,     POSITIVE)      |
        SMI_FIELD_SET  (0, DE_CONTROL, STEP_Y,     POSITIVE)      |
        SMI_FIELD_SET  (0, DE_CONTROL, LAST_PIXEL, ON)            |
        SMI_FIELD_SET  (0, DE_CONTROL, ROP_SELECT, ROP2)          |
        SMI_FIELD_VALUE(0, DE_CONTROL, ROP,        rop2);
    ...
    {
        dx = x1 - x0;
        de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, STEP_X, NEGATIVE);
    }
    else
        dx = x0 - x1;
    ...
    {
        dy = y1 - y0;
        de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, STEP_Y, NEGATIVE);
    }
    else
        dy = y0 - y1;
    ...
    if (dx < dy)
        de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, MAJOR, Y);        
    ...
    else 
    {
        ...
        */
        ...
        POKE_32(DE_DESTINATION,
            SMI_FIELD_SET  (0, DE_DESTINATION, WRAP, DISABLE) |
            SMI_FIELD_VALUE(0, DE_DESTINATION, X,    x0)      |
            SMI_FIELD_VALUE(0, DE_DESTINATION, Y,    y0));
        ...
        if (dx == dy)
        {
            ...
            POKE_32(DE_DIMENSION,
                SMI_FIELD_VALUE(0, DE_DIMENSION, X,    1) |
                SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, dx + 1));

            /* Set the command register. */
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, COMMAND, SHORT_STROKE);
        ...
        else
        {
            ...
            POKE_32(DE_SOURCE,
                SMI_FIELD_SET  (0, DE_SOURCE, WRAP, DISABLE) |
                SMI_FIELD_VALUE(0, DE_SOURCE, X_K1_LINE, k1) |
                SMI_FIELD_VALUE(0, DE_SOURCE, Y_K2_LINE, k2));

            /* Set the line length and width */
            POKE_32(DE_DIMENSION,
                SMI_FIELD_VALUE(0, DE_DIMENSION, X,    w) |
                SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, et));

            /* Set the control register. */
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, COMMAND, LINE_DRAW);
...
)
{
    ...
    unsigned long de_ctrl =
        ...
        SMI_FIELD_SET  (0, DE_CONTROL, QUICK_START, ENABLE)     |
        SMI_FIELD_SET  (0, DE_CONTROL, LAST_PIXEL,  OFF)        |
        SMI_FIELD_SET  (0, DE_CONTROL, COMMAND,     RLE_STRIP)  |
        SMI_FIELD_SET  (0, DE_CONTROL, ROP_SELECT,  ROP2)       |
        SMI_FIELD_VALUE(0, DE_CONTROL, ROP,         rop2);
    ...
    */
    ...
    {
        case 0:
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, DIRECTION, LEFT_TO_RIGHT);
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, MAJOR, X);
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, STEP_X, POSITIVE);
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, STEP_Y, POSITIVE);
            break;
        case 1:
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, DIRECTION, LEFT_TO_RIGHT);
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, MAJOR, X);
        ...
        case 7:
            ...
            break;
        case 8:
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, DIRECTION, RIGHT_TO_LEFT);
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, MAJOR, X);
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, STEP_X, POSITIVE);
            de_ctrl = SMI_FIELD_SET(de_ctrl, DE_CONTROL, STEP_Y, POSITIVE);
            break;
    ...
    /* Set the line length and width */
    POKE_32(DE_DIMENSION,
        SMI_FIELD_VALUE(0, DE_DIMENSION, X,    lineCounter) |
        SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, length));
#else
    ...
    switch (direction)
    {
        case 0:
            ...
            POKE_32(DE_DIMENSION,
                SMI_FIELD_VALUE(0, DE_DIMENSION, X,    length) |
                SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, lineCounter));
            break;
...
)
{
    ...
    if (SMI_FIELD_GET(de_ctrl, DE_CONTROL, MAJOR) == DE_CONTROL_MAJOR_X)
    {
        /* Set the line length and width */
        POKE_32(DE_DIMENSION,
            SMI_FIELD_VALUE(0, DE_DIMENSION, X,    length) |
...
)
{
    ...
    */
    ...
    POKE_32(DE_STRETCH_FORMAT, 
        SMI_FIELD_VALUE(PEEK_32(DE_STRETCH_FORMAT), DE_STRETCH_FORMAT, SOURCE_HEIGHT, sHeight));
        
    /* Set the alpha blend value */
    POKE_32(DE_ALPHA,
        SMI_FIELD_VALUE(0, DE_ALPHA, VALUE, alphaValue));
    ...
    if ((sWidth * bytePerPixel) > 192)
    {
        ...
        while (1)
        {
            ...
            POKE_32(DE_DIMENSION,
                SMI_FIELD_VALUE(0, DE_DIMENSION, X,    xChunk) |
                SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, dHeight));

            /* Setup Control Register */
            de_ctrl = SMI_FIELD_SET(0, DE_CONTROL, STATUS, START)    |
                      SMI_FIELD_SET(0, DE_CONTROL, COMMAND, ALPHA_BLEND)  |
                      SMI_FIELD_SET(0, DE_CONTROL, ROP_SELECT, ROP2) |
                      SMI_FIELD_VALUE(0, DE_CONTROL, ROP, rop2)      |
                      ((sHeight < dHeight) ?
                            SMI_FIELD_SET(0, DE_CONTROL, STRETCH, ENABLE) :
                            SMI_FIELD_SET(0, DE_CONTROL, STRETCH, DISABLE));
...
#endif
    {
        ...
        /* Set the Destination Dimension */    
        POKE_32(DE_DIMENSION,
            SMI_FIELD_VALUE(0, DE_DIMENSION, X,    sWidth) |
            SMI_FIELD_VALUE(0, DE_DIMENSION, Y_ET, dHeight));
        ...
        ((width * bytePerPixel) > 192))
    {
        ...
        while (1)
        {
            ...
            de_ctrl = 
                ...
                SMI_FIELD_SET  (0, DE_CONTROL,  COMMAND,    RECTANGLE_FILL) |
                SMI_FIELD_SET  (0, DE_CONTROL,  ROP_SELECT, ROP2)           |
                SMI_FIELD_VALUE(0, DE_CONTROL,  ROP,        rop3);
