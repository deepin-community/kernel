...
unsigned int ddk768_getCrystalType(void)
{
	...
	strapPin = SMI_FIELD_GET(peekRegisterDWord(STRAP_PINS), STRAP_PINS, CRYSTAL_CONFIG);
...
unsigned int ddk768_getPixelType(void)
{
	...
	strapPin = SMI_FIELD_GET(peekRegisterDWord(STRAP_PINS), STRAP_PINS, PIXEL_CONFIG);
...
unsigned long ddk768_getFrameBufSize()
{
    ...
    strapPin = SMI_FIELD_GET(peekRegisterDWord(STRAP_PINS), STRAP_PINS, MEM_SIZE);
    ddrController = peekRegisterDWord(DDR_CONTROL);

    switch(strapPin)
    {
        case STRAP_PINS_MEM_SIZE_512M:
        pokeRegisterDWord(DDR_CONTROL, ddrController | SMI_FIELD_SET(0, DDR_CONTROL, SIZE, 512M));
        rValue = MB(512);
        break;

        case STRAP_PINS_MEM_SIZE_1024M:
        pokeRegisterDWord(DDR_CONTROL, ddrController | SMI_FIELD_SET(0, DDR_CONTROL, SIZE, 1024M));
        rValue = MB(1024);
        break;
...
long ddk768_initChipParamEx(initchip_param_t * pInitParam)
{
    ...
    if (pInitParam->setAllEngOff == 1)
    {
        ...
        ulReg = SMI_FIELD_SET(ulReg, HWC_CONTROL, MODE, DISABLE); 
