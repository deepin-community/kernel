...
{  
    ...
    */
    ...
    ulTimerValue =
          SMI_FIELD_VALUE(0, TIMER_CONTROL, COUNTER, timerCounter)
        | SMI_FIELD_SET(0, TIMER_CONTROL, RAWINT_STATUS, RESET)    /* Reset the raw interrupt */
        | SMI_FIELD_SET(0, TIMER_CONTROL, RAWINT_ENABLE, ENABLE)   /* Enable raw interrupt to happen when time out */
        | SMI_FIELD_VALUE(0, TIMER_CONTROL, DIV16, div16Enable)
        | SMI_FIELD_SET(0, TIMER_CONTROL, ENABLE, ENABLE);           /* Start the timer */
...
{  
    ...
    */
    ulTimerAddr = TIMER_CONTROL + (timer << 2);
    rawIntStatus = SMI_FIELD_GET(peekRegisterDWord(ulTimerAddr), TIMER_CONTROL, RAWINT_STATUS);
...
{  
    ...
    */
    ...
    ulTimerValue = peekRegisterDWord(ulTimerAddr)
                 & SMI_FIELD_CLEAR(TIMER_CONTROL, COUNTER); /* We don't want the current counter value */

    pokeRegisterDWord(ulTimerAddr,
            ulTimerValue
          | SMI_FIELD_VALUE(0, TIMER_CONTROL, COUNTER, gTimerCounter[timer]) /* When clearing raw int, we don't want to erase the original counter value */
          | SMI_FIELD_SET(0, TIMER_CONTROL, RAWINT_STATUS, RESET));    /* Reset the raw interrupt */
}
...
{  
    ...
    */
    ...
    ulTimerValue =
          SMI_FIELD_SET(0, TIMER_CONTROL, RAWINT_STATUS, RESET)    /* Reset the raw interrupt */
        | SMI_FIELD_SET(0, TIMER_CONTROL, ENABLE, DISABLE);        /* Stop the timer */
...
{  
    ...
    */
    ulTimerAddr = TIMER_CONTROL + (timer << 2);
    ulCounter = SMI_FIELD_GET(peekRegisterDWord(ulTimerAddr), TIMER_CONTROL, COUNTER);
...
)
{
    ...
    {
        case 0:
            mask |= SMI_FIELD_SET(0, INT_MASK, TIMER0, ENABLE);
            break;
        case 1:
            mask |= SMI_FIELD_SET(0, INT_MASK, TIMER1, ENABLE);
            break;
        case 2:
            mask |= SMI_FIELD_SET(0, INT_MASK, TIMER2, ENABLE);
            break;
        case 3:
            mask |= SMI_FIELD_SET(0, INT_MASK, TIMER3, ENABLE);
            break;
        default:
            break;
    }
...
{
    if (SMI_FIELD_GET(status, INT_STATUS, TIMER0) == INT_STATUS_TIMER0_ACTIVE)
    {
    ...
    if (SMI_FIELD_GET(status, INT_STATUS, TIMER1) == INT_STATUS_TIMER1_ACTIVE)
    {
    ...
    if (SMI_FIELD_GET(status, INT_STATUS, TIMER2) == INT_STATUS_TIMER2_ACTIVE)
    {
    ...
    if (SMI_FIELD_GET(status, INT_STATUS, TIMER3) == INT_STATUS_TIMER3_ACTIVE)
    {
