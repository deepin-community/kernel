...
)
{
	...
	pokeRegisterDWord(regFB,
        ...
        | SMI_FIELD_VALUE(0, FB_ADDRESS, ADDRESS, ulBaseAddress));
}
...
)
{
    ...
    if (SMI_FIELD_GET(peekRegisterDWord(regFB), FB_ADDRESS, STATUS) == FB_ADDRESS_STATUS_PENDING)
        return 1;
...
)
{
    ...
    if (pLogicalMode->x == 3840)
        ...
        pokeRegisterDWord(VGA_CONFIGURATION,
                          SMI_FIELD_SET(0, VGA_CONFIGURATION, PLL, PANEL) | SMI_FIELD_SET(0, VGA_CONFIGURATION, MODE, GRAPHIC));
    ...
    pokeRegisterDWord((FB_ADDRESS+offset),
          SMI_FIELD_SET(0, FB_ADDRESS, STATUS, PENDING)
        | SMI_FIELD_VALUE(0, FB_ADDRESS, ADDRESS, pLogicalMode->baseAddress));
    ...
    pokeRegisterDWord((FB_WIDTH+offset),
          SMI_FIELD_VALUE(0, FB_WIDTH, WIDTH, pLogicalMode->pitch));

    pokeRegisterDWord((HORIZONTAL_TOTAL+offset),
          SMI_FIELD_VALUE(0, HORIZONTAL_TOTAL, TOTAL, pModeParam->horizontal_total - 1)
        | SMI_FIELD_VALUE(0, HORIZONTAL_TOTAL, DISPLAY_END, pModeParam->horizontal_display_end - 1));

    pokeRegisterDWord((HORIZONTAL_SYNC+offset),
          SMI_FIELD_VALUE(0, HORIZONTAL_SYNC, WIDTH, pModeParam->horizontal_sync_width)
        | SMI_FIELD_VALUE(0, HORIZONTAL_SYNC, START, pModeParam->horizontal_sync_start - 1));

    pokeRegisterDWord((VERTICAL_TOTAL+offset),
          SMI_FIELD_VALUE(0, VERTICAL_TOTAL, TOTAL, pModeParam->vertical_total - 1)
        | SMI_FIELD_VALUE(0, VERTICAL_TOTAL, DISPLAY_END, pModeParam->vertical_display_end - 1));

    pokeRegisterDWord((VERTICAL_SYNC+offset),
          SMI_FIELD_VALUE(0, VERTICAL_SYNC, HEIGHT, pModeParam->vertical_sync_height)
        | SMI_FIELD_VALUE(0, VERTICAL_SYNC, START, pModeParam->vertical_sync_start - 1));


    
    hdmi_channel = SMI_FIELD_GET(peekRegisterDWord(DISPLAY_CTRL+offset),
                                   DISPLAY_CTRL,
                                   HDMI_SELECT);    
    ...
    ulTmpValue =       
        ...
        ? SMI_FIELD_SET(0, DISPLAY_CTRL, VSYNC_PHASE, ACTIVE_HIGH)
        : SMI_FIELD_SET(0, DISPLAY_CTRL, VSYNC_PHASE, ACTIVE_LOW))
      | (pModeParam->horizontal_sync_polarity == POS
        ? SMI_FIELD_SET(0, DISPLAY_CTRL, HSYNC_PHASE, ACTIVE_HIGH)
        : SMI_FIELD_SET(0, DISPLAY_CTRL, HSYNC_PHASE, ACTIVE_LOW))
      | (pModeParam->clock_phase_polarity== POS
        ? SMI_FIELD_SET(0, DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_HIGH)
        : SMI_FIELD_SET(0, DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_LOW))
      | SMI_FIELD_SET(0, DISPLAY_CTRL, DATA_PATH, EXTENDED)
      | SMI_FIELD_SET(0, DISPLAY_CTRL, DIRECTION, INPUT)
      | SMI_FIELD_SET(0, DISPLAY_CTRL, TIMING, ENABLE)
      | SMI_FIELD_SET(0, DISPLAY_CTRL, PLANE, ENABLE) 
      | (pLogicalMode->bpp == 8
        ? SMI_FIELD_SET(0, DISPLAY_CTRL, FORMAT, 8)
        : (pLogicalMode->bpp == 16
        ? SMI_FIELD_SET(0, DISPLAY_CTRL, FORMAT, 16)
        : SMI_FIELD_SET(0, DISPLAY_CTRL, FORMAT, 32)));

	if(clk_phase == 0)
		ulTmpValue = SMI_FIELD_SET(ulTmpValue, DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_LOW);
	if(clk_phase == 1)
		ulTmpValue = SMI_FIELD_SET(ulTmpValue, DISPLAY_CTRL, CLOCK_PHASE, ACTIVE_HIGH);

	
	if( hdmi_channel == DISPLAY_CTRL_HDMI_SELECT_CHANNEL0)
		 ulTmpValue= SMI_FIELD_SET(ulTmpValue,DISPLAY_CTRL, HDMI_SELECT, CHANNEL0);
	else
		 ulTmpValue= SMI_FIELD_SET(ulTmpValue,DISPLAY_CTRL, HDMI_SELECT, CHANNEL1);
