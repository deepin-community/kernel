...
{
        return (SMI_FIELD_GET(peekRegisterDWord(VIDEO_FB_ADDRESS), VIDEO_FB_ADDRESS, STATUS));
}
...
{
    return (SMI_FIELD_GET(peekRegisterDWord(VIDEO_FB_WIDTH), VIDEO_FB_WIDTH, WIDTH));
}
...
{
    return (SMI_FIELD_GET(peekRegisterDWord(VIDEO_FB_WIDTH), VIDEO_FB_WIDTH, OFFSET));
}
...
)
{
    ...
    {
        value = (unsigned long)
            SMI_FIELD_GET(peekRegisterDWord(VIDEO_FB_ADDRESS), VIDEO_FB_ADDRESS, ADDRESS);
    }
...
{
        return (SMI_FIELD_GET(peekRegisterDWord(VIDEO_FB_ADDRESS), VIDEO_FB_ADDRESS, ADDRESS));
}
...
)
{
    ...
    if (bufferIndex == 0)
    {
        ...
        pokeRegisterDWord(VIDEO_FB_0_LAST_ADDRESS,
            SMI_FIELD_VALUE(0, VIDEO_FB_0_LAST_ADDRESS, ADDRESS, bufferStart + bufferSize));
    }
    else
    {   
        /* Set Video Buffer 1 Last Address */ 
        pokeRegisterDWord(VIDEO_FB_1_LAST_ADDRESS,
            SMI_FIELD_VALUE(0, VIDEO_FB_1_LAST_ADDRESS, ADDRESS, bufferStart + bufferSize));
    }
#endif
}
...
)
{
    ...
    if (bufferIndex == 0)
    {
        ...
        return (unsigned long) (SMI_FIELD_GET(peekRegisterDWord(VIDEO_FB_0_LAST_ADDRESS), 
                                          VIDEO_FB_0_LAST_ADDRESS, ADDRESS));
    }
    else
    {   
        /* Get Video Buffer 1 Last Address */ 
        return (unsigned long) (SMI_FIELD_GET(peekRegisterDWord(VIDEO_FB_1_LAST_ADDRESS), 
...
)
{
    ...
    if (bufferIndex == 0)
    {
	    ...
	    pokeRegisterDWord(regFB,
	            ...
	            SMI_FIELD_VALUE(0, VIDEO_FB_ADDRESS, ADDRESS, bufferStartAddress));
    }
}
/*
 * videoSetUVBuffer
...
)
{
    ...
    pokeRegisterDWord(regU,
        SMI_FIELD_VALUE(0, VIDEO_FB_ADDRESS_U, ADDRESS, bufferStartUAddress));
    pokeRegisterDWord(regV,
        SMI_FIELD_VALUE(0, VIDEO_FB_ADDRESS_V, ADDRESS, bufferStartVAddress));
}
...
)
{
	...
	pokeRegisterDWord(regWidth,
	    ...
	    SMI_FIELD_VALUE(0, VIDEO_FB_WIDTH, OFFSET, lineOffset));
}
/*
...
)
{
	...
	pokeRegisterDWord(regWidthU,
	    SMI_FIELD_VALUE(0, VIDEO_FB_WIDTH_U, WIDTH, pitch) |
	    SMI_FIELD_VALUE(0, VIDEO_FB_WIDTH_U, OFFSET, lineOffset));
	pokeRegisterDWord(regWidthV,
	    SMI_FIELD_VALUE(0, VIDEO_FB_WIDTH_V, WIDTH, pitch) |
	    SMI_FIELD_VALUE(0, VIDEO_FB_WIDTH_V, OFFSET, lineOffset));
}
/*
 *  videoSetLast 
...
)
{
    ...
    if (dispCtrl == CHANNEL0_CTRL)
	{
	    pokeRegisterDWord(CHANNEL0_VIDEO_LAST, 
	        SMI_FIELD_VALUE(0, CHANNEL0_VIDEO_LAST, COLUMN, width) |
	        SMI_FIELD_VALUE(0, CHANNEL0_VIDEO_LAST, ROW, height)); 
	}
	else
	{
	    pokeRegisterDWord(CHANNEL1_VIDEO_LAST, 
	        SMI_FIELD_VALUE(0, CHANNEL1_VIDEO_LAST, COLUMN, width) |
	        SMI_FIELD_VALUE(0, CHANNEL1_VIDEO_LAST, ROW, height)); 
	}													   
#endif
}
/*
...
)
{
	...
	startX = SMI_FIELD_GET(value, VIDEO_PLANE_TL, LEFT);
	startY = SMI_FIELD_GET(value, VIDEO_PLANE_TL, TOP);

	/* Set bottom and right position */
	pokeRegisterDWord(regBR,
	    SMI_FIELD_VALUE(0, VIDEO_PLANE_BR, BOTTOM, startY + height - 1 - gHeightAdjustment) |
	    SMI_FIELD_VALUE(0, VIDEO_PLANE_BR, RIGHT, startX + width - 1 - gWidthAdjustment)); 
}
...
)
{
	...
	videoWidth  = SMI_FIELD_GET(positionRightBottom, VIDEO_PLANE_BR, RIGHT) - 
	              SMI_FIELD_GET(positionTopLeft, VIDEO_PLANE_TL, LEFT) + 1 +
	              gWidthAdjustment;
	videoHeight = SMI_FIELD_GET(positionRightBottom, VIDEO_PLANE_BR, BOTTOM) - 
	              SMI_FIELD_GET(positionTopLeft, VIDEO_PLANE_TL, TOP) + 1 +
	              gHeightAdjustment;
...
)
{
	...
	pokeRegisterDWord(regTL,
	    SMI_FIELD_VALUE(0, VIDEO_PLANE_TL, TOP, startY) |
	    SMI_FIELD_VALUE(0, VIDEO_PLANE_TL, LEFT, startX));
...
)
{
	...
	pokeRegisterDWord(regYUV,
	    SMI_FIELD_VALUE(0, VIDEO_YUV_CONSTANTS, Y, yConstant) |
	    SMI_FIELD_VALUE(0, VIDEO_YUV_CONSTANTS, R, redConstant) |
	    SMI_FIELD_VALUE(0, VIDEO_YUV_CONSTANTS, G, greenConstant) |
	    SMI_FIELD_VALUE(0, VIDEO_YUV_CONSTANTS, B, blueConstant));
}
...
)
{
    ...
    value = SMI_FIELD_VALUE(value, VIDEO_INITIAL_SCALE, VERTICAL, InitScaleVertical);
    value = SMI_FIELD_VALUE(value, VIDEO_INITIAL_SCALE, HORIZONTAL, InitScaleHorizontal);
...
)
{
    ...
    *pBufferHInitScale = (unsigned short)
        SMI_FIELD_GET(peekRegisterDWord(regScale), VIDEO_INITIAL_SCALE, HORIZONTAL);
    *pBufferVInitScale = (unsigned short)
        SMI_FIELD_GET(peekRegisterDWord(regScale), VIDEO_INITIAL_SCALE, VERTICAL);
}
...
)
{
	...
	if (dstHeight >= srcHeight)
	{
	    ...
	    value = SMI_FIELD_VALUE(value, VIDEO_SCALE , VERTICAL_SCALE, scaleFactor);
	}
	...
	if (dstWidth >= srcWidth)
	{
	    ...
	    value = SMI_FIELD_VALUE(value, VIDEO_SCALE, HORIZONTAL_SCALE, scaleFactor);
	}
...
)
{
	...
	if (byteSwap == SWAP_BYTE)
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, BYTE_SWAP, ENABLE);
	else
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, BYTE_SWAP, DISABLE);
...
)
{
	...
	if (enableHorzInterpolation)
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, HORIZONTAL_MODE, INTERPOLATE);
	else
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, HORIZONTAL_MODE, REPLICATE);
	    
	if (enableVertInterpolation)
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, VERTICAL_MODE, INTERPOLATE);
	else
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, VERTICAL_MODE, REPLICATE);
...
)
{
	...
	{
		if (SMI_FIELD_GET(value, VIDEO_DISPLAY_CTRL, HORIZONTAL_MODE) == VIDEO_DISPLAY_CTRL_HORIZONTAL_MODE_INTERPOLATE)
    ...
    if (pHorzInterpolationStatus != (unsigned long *)0)
	{
		if (SMI_FIELD_GET(value, VIDEO_DISPLAY_CTRL, VERTICAL_MODE) == VIDEO_DISPLAY_CTRL_VERTICAL_MODE_INTERPOLATE)
...
)
{
    pokeRegisterDWord(VIDEO_DISPLAY_CTRL, 
                      ...
                      SMI_FIELD_VALUE(0, VIDEO_DISPLAY_CTRL, PIXEL, startPixel));    
}
...
)
{
	...
	if (enableGammaCtrl)
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, GAMMA, ENABLE);
	else
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, GAMMA, DISABLE);
...
unsigned char isVideoEnable()
{
    ...
    return ((SMI_FIELD_GET(value, VIDEO_DISPLAY_CTRL, PLANE) == VIDEO_DISPLAY_CTRL_PLANE_ENABLE) ? 1 : 0);
}
...
)
{
	...
	if (videoCtrl == VIDEO_ON)
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, PLANE, ENABLE);
	else
	    value = SMI_FIELD_SET(value, VIDEO_DISPLAY_CTRL, PLANE, DISABLE);
...
)
{
	...
	switch (videoFormat)
	{
	    default:
	    case FORMAT_RGB565:
	        value = SMI_FIELD_SET(value,  VIDEO_DISPLAY_CTRL , FORMAT, 16);
	        break;
	    case FORMAT_RGB888:
	        value = SMI_FIELD_SET(value,  VIDEO_DISPLAY_CTRL , FORMAT, 32);
	        break;
	    case FORMAT_YUYV:
	        value = SMI_FIELD_SET(value,  VIDEO_DISPLAY_CTRL , FORMAT, YUV422);
	        break;
	    case FORMAT_YUV420:
	        value = SMI_FIELD_SET(value,  VIDEO_DISPLAY_CTRL , FORMAT, YUV420);
	        break;
	}
...
)
{
    ...
    if (getChipType() == SM718)
    {
        ...
        {
            value = SMI_FIELD_SET(0, VIDEO_EDGE_DETECTION, DETECT, ENABLE) |
                    SMI_FIELD_VALUE(value, VIDEO_EDGE_DETECTION, VALUE, edgeDetectValue);
            pokeRegisterDWord(VIDEO_EDGE_DETECTION, value); 
        }
        else
        {
            value = SMI_FIELD_SET(peekRegisterDWord(VIDEO_EDGE_DETECTION), VIDEO_EDGE_DETECTION, DETECT, DISABLE);
            pokeRegisterDWord(VIDEO_EDGE_DETECTION, value);
        }
    }
#endif
...
)
{
    ...
    if (getChipType() == SM718)
    {
        ...
        if (pEdgeDetectValue != (unsigned long *)0)
            *pEdgeDetectValue = (unsigned long) SMI_FIELD_GET(value, VIDEO_EDGE_DETECTION, VALUE);
            
        if (SMI_FIELD_GET(value, VIDEO_EDGE_DETECTION, DETECT) == VIDEO_EDGE_DETECTION_DETECT_ENABLE)
            return 1;
        else
            return 0; 
    }
