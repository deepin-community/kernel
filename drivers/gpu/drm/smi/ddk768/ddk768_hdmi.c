...
)
{
    ...
    if (SMI_FIELD_GET(status, INT_STATUS, HDMI) == INT_STATUS_HDMI_ACTIVE)
    {
...
)
{
    ...
    if (g_pHdmiIntHandlers == ((hdmi_interrupt_t *)0))
        returnValue = registerHandler(hdmiISR, SMI_FIELD_SET(0, INT_MASK, HDMI, ENABLE));
...
void writeHDMIRegister(unsigned char addr, unsigned char value)
{
    ...
    pokeRegisterDWord(HDMI_CONFIG, 
        SMI_FIELD_SET(0, HDMI_CONFIG, WRITE, ENABLE) | 
        SMI_FIELD_VALUE(0, HDMI_CONFIG, DATA, value) | 
        SMI_FIELD_VALUE(0, HDMI_CONFIG, ADDRESS, addr));
...
unsigned char readHDMIRegister(unsigned char addr)
{
    ...
    pokeRegisterDWord(HDMI_CONFIG, 
        SMI_FIELD_SET(0, HDMI_CONFIG, READ, ENABLE) | 
        SMI_FIELD_VALUE(0, HDMI_CONFIG, ADDRESS, addr));
    
    DelayMs(1);
    
    pokeRegisterDWord(HDMI_CONFIG, SMI_FIELD_VALUE(0, HDMI_CONFIG, ADDRESS, addr));
...
unsigned char readHDMIControlRegister(void)
{
    ...
    pokeRegisterDWord(HDMI_CONFIG, 
        SMI_FIELD_SET(0, HDMI_CONFIG, READ, ENABLE) | 
        SMI_FIELD_VALUE(0, HDMI_CONFIG, ADDRESS, 0));
...
void setHDMIChannel(unsigned char Channel)
{
    ...
    if (Channel == 0)
    {
        pokeRegisterDWord(DISPLAY_CTRL, 
            SMI_FIELD_SET(value, DISPLAY_CTRL, HDMI_SELECT, CHANNEL0));
    }
    else
    {
        pokeRegisterDWord(DISPLAY_CTRL, 
            SMI_FIELD_SET(value, DISPLAY_CTRL, HDMI_SELECT, CHANNEL1));
    }
...
void enableHdmI2C(unsigned long enable)
{
    ...
    if (enable)
        value = SMI_FIELD_SET(value, TEST_CONTROL, I2C, HDMI);
    else
        value = SMI_FIELD_SET(value, TEST_CONTROL, I2C, I2C1);
...
void HDMI_Video_Setting (mode_parameter_t *pModeParam, bool isHDMI)
{
    ...
    regValue = 
        ...
        ? SMI_FIELD_SET(regValue, X30_EXT_VPARAMS, VSYNC_PHASE, POS)
        : SMI_FIELD_SET(regValue, X30_EXT_VPARAMS, VSYNC_PHASE, NEG))
        | (pModeParam->horizontal_sync_polarity == POS
        ? SMI_FIELD_SET(regValue, X30_EXT_VPARAMS, HSYNC_PHASE, POS)
        : SMI_FIELD_SET(regValue, X30_EXT_VPARAMS, HSYNC_PHASE, NEG))
        | SMI_FIELD_SET(regValue, X30_EXT_VPARAMS, USE, EXTERNAL);
...
unsigned char HDMI_Edid_ReadFirstByte(void)
{
    ...
    pokeRegisterDWord(HDMI_CONFIG, 
        SMI_FIELD_SET(0, HDMI_CONFIG, READ, LATCH) | 
        SMI_FIELD_VALUE(0, HDMI_CONFIG, ADDRESS, X80_EDID));
    
    DelayMs(1);
    
    pokeRegisterDWord(HDMI_CONFIG, SMI_FIELD_VALUE(0, HDMI_CONFIG, ADDRESS, X80_EDID));
...
BYTE HDMI_connector_detect(void)
{
	...
	value = SMI_FIELD_SET(peekRegisterDWord(GPIO_DATA_DIRECTION), GPIO_DATA_DIRECTION, 1, INPUT);
...
int hdmi_detect(void)
{
    ...
    if (SMI_FIELD_GET(intStatus, INT_STATUS, HDMI) == INT_STATUS_HDMI_ACTIVE)
    {
