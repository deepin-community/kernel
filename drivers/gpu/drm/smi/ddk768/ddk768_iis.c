...
)
{
    ...
    value = SMI_FIELD_GET(peekRegisterDWord(USBH_CLOCK), USBH_CLOCK, FS_IIS);
    ...
    pokeRegisterDWord(I2S_CTRL, 
          SMI_FIELD_VALUE(0, I2S_CTRL, CS, ws)
        | SMI_FIELD_VALUE(0, I2S_CTRL , CDIV, clockDivider));
    ...
    )
{
    ...
    dmaPointer = SMI_FIELD_GET(peekRegisterDWord(I2S_SRAM_DMA), I2S_SRAM_DMA, ADDRESS);

    //If DMA pointer already at the requested offset. Just set up the length.
    if (dmaPointer == offset)
    {
        pokeRegisterDWord(I2S_SRAM_DMA,
          SMI_FIELD_SET(0, I2S_SRAM_DMA, STATE, ENABLE)
        | SMI_FIELD_VALUE(0, I2S_SRAM_DMA, SIZE, len)
        | SMI_FIELD_VALUE(0, I2S_SRAM_DMA, ADDRESS, offset));
    ...
    pokeRegisterDWord(I2S_SRAM_DMA,
          SMI_FIELD_SET(0, I2S_SRAM_DMA, STATE, ENABLE)
        | SMI_FIELD_VALUE(0, I2S_SRAM_DMA, SIZE, (0x1FF - dmaPointer))
        | SMI_FIELD_VALUE(0, I2S_SRAM_DMA, ADDRESS, dmaPointer));
...
{
    return(SMI_FIELD_GET(peekRegisterDWord(I2S_SRAM_DMA), I2S_SRAM_DMA, ADDRESS));
}
...
void iisStartNoTx(void)
{
    ...
    value = SMI_FIELD_SET(peekRegisterDWord(I2S_CTRL), I2S_CTRL, MODE, MASTER);
...
void iisStart(void)
{
	...
	pokeRegisterDWord(I2S_STATUS, SMI_FIELD_SET(0, I2S_STATUS, TX, ENABLE));  //Enable Tx line out
...
void iisStop(void)
{
    ...
    value = SMI_FIELD_SET(peekRegisterDWord(I2S_CTRL), I2S_CTRL, MODE, SLAVE);
    pokeRegisterDWord(I2S_CTRL, value);

    pokeRegisterDWord(I2S_STATUS, SMI_FIELD_SET(0, I2S_STATUS, TX, DISABLE));  //Disable Tx line out
...
unsigned long iisIntMask(void)
{
    ...
    mask |= SMI_FIELD_SET(0, INT_MASK, I2S, ENABLE);
