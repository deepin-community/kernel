...
)
{
    ...
    pokeRegisterDWord(
        ...
        SMI_FIELD_VALUE(0, CHANNEL0_HWC_ADDRESS, ADDRESS, base));
    ...
    pokeRegisterDWord(
        ...
        SMI_FIELD_VALUE(0, CHANNEL0_HWC_COLOR_0, RGB888, color1));

    pokeRegisterDWord(
        (dispControl == CHANNEL0_CTRL) ? CHANNEL0_HWC_COLOR_1 : CHANNEL1_HWC_COLOR_1, 
        SMI_FIELD_VALUE(0, CHANNEL0_HWC_COLOR_1, RGB888, color2));
...
{  
    ...
    value = SMI_FIELD_VALUE(0, HWC_LOCATION, X, dx) |
            SMI_FIELD_VALUE(0, HWC_LOCATION, Y, dy);
    
    /* Set the top boundary select either partially outside the top boundary
       screen or inside */
    if (topOutside)
        value = SMI_FIELD_SET(value, HWC_LOCATION, TOP, OUTSIDE);
    else         
        value = SMI_FIELD_SET(value, HWC_LOCATION, TOP, INSIDE);

    /* Set the left boundary select either partially outside the left boundary
       screen or inside */
    if (leftOutside)
        value = SMI_FIELD_SET(value, HWC_LOCATION, LEFT, OUTSIDE);
    else        
        value = SMI_FIELD_SET(value, HWC_LOCATION, LEFT, INSIDE);
...
)
{
    ...
    value = SMI_FIELD_VALUE(value, HWC_CONTROL, MODE, mode);
