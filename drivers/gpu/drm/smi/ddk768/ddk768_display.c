   ...
   )
{
    ...
    {
       case DISP_DPMS_ON:
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DPMS, VPHP);
        break;

       case DISP_DPMS_STANDBY:
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DPMS, VPHN);
        break;

       case DISP_DPMS_SUSPEND:
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DPMS, VNHP);
        break;

       case DISP_DPMS_OFF:
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DPMS, VNHN);
        break;
    }
   ...
   )
{
   ...
   if (dispFormat == DOUBLE_PIXEL_48BIT)
   {
      if (dataPath == CHANNEL0_CTRL)
           ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, CHANNEL_OUTPUT_FORMAT, CHANNEL0_48BIT);
      else
           ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, CHANNEL_OUTPUT_FORMAT, CHANNEL1_48BIT);

        // When in 48 bit mode, enable double pixel clock
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DOUBLE_PIXEL_CLOCK, ENABLE);
   }
   else
   {
      if (dataPath == CHANNEL0_CTRL)
           ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, CHANNEL_OUTPUT_FORMAT, CHANNEL0_24BIT);
      else
           ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, CHANNEL_OUTPUT_FORMAT, CHANNEL1_24BIT);

        // When in 24 bit mode, disable double pixel.
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DOUBLE_PIXEL_CLOCK, DISABLE);
   }
...
void waitDispVerticalSync(disp_control_t dispControl, unsigned long vSyncCount)
{
    ...
    if (dispControl == CHANNEL0_CTRL)
    {
        ...
        if ((SMI_FIELD_GET(peekRegisterDWord(CLOCK_ENABLE), CLOCK_ENABLE, DC0) == CLOCK_ENABLE_DC0_OFF))
            return;
    ...
    {
        // There is no Vsync when PLL is off
        if ((SMI_FIELD_GET(peekRegisterDWord(CLOCK_ENABLE), CLOCK_ENABLE, DC1) == CLOCK_ENABLE_DC1_OFF))
            return;
    ...
    if ((SMI_FIELD_GET(peekRegisterDWord(ulDispCtrlAddr), DISPLAY_CTRL, TIMING) ==
    ...
    while (vSyncCount-- > 0)
    {
        ...
        */
        ...
        do
        {
            status = SMI_FIELD_GET(peekRegisterDWord(ulDispCtrlAddr), DISPLAY_CTRL, VSYNC);
...
void ddk768_waitVSyncLine(disp_control_t dispControl)
{
    ...
    do
    {
    	value = SMI_FIELD_GET(peekRegisterDWord(ulDispCtrlAddr), CURRENT_LINE, LINE);
    }
...
unsigned long getDisplayLine(disp_control_t dispControl)
{
    ...
    ulRegValue = SMI_FIELD_GET(peekRegisterDWord(ulRegAddr), CURRENT_LINE, LINE);
...
void ddk768_swPanelPowerSequence(disp_control_t dispControl, disp_state_t dispState, unsigned long vSyncDelay)
{
    ...
    if (dispState == DISP_ON)
    {
        ...
        /* Turn on FPVDDEN. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, FPVDDEN, HIGH);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
		waitDispVerticalSync(dispControl, vSyncDelay);

        /* Turn on FPDATA. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DATA, ENABLE);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
		waitDispVerticalSync(dispControl, vSyncDelay);

        /* Turn on FPVBIAS. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, VBIASEN, HIGH);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
		waitDispVerticalSync(dispControl, vSyncDelay);

        /* Turn on FPEN. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, FPEN, HIGH);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
    }
    else
    {
        ...
        /* Turn off FPEN. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, FPEN, LOW);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
		waitDispVerticalSync(dispControl, vSyncDelay);


        /* Turn off FPVBIASEN. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, VBIASEN, LOW);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
		waitDispVerticalSync(dispControl, vSyncDelay);

        /* Turn off FPDATA. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DATA, DISABLE);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
		waitDispVerticalSync(dispControl, vSyncDelay);

        /* Turn off FPVDDEN. */
        ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, FPVDDEN, LOW);
        pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
    }
...
)
{
    ...
    if (dispState == DISP_ON)
    {
          ...
          */
         ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, TIMING, ENABLE);
         ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, DIRECTION, INPUT); 
         pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);

		 ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, PLANE, ENABLE)|        
    ...
    else
    {
         ...
         ulDispCtrlReg = SMI_FIELD_SET(ulDispCtrlReg, DISPLAY_CTRL, TIMING, DISABLE);
         pokeRegisterDWord(ulDispCtrlAddr, ulDispCtrlReg);
...
)
{
    ...
    value = SMI_FIELD_VALUE(0, CRT_DETECT, DATA_RED, red) |
            SMI_FIELD_VALUE(0, CRT_DETECT, DATA_GREEN, green) |
            SMI_FIELD_VALUE(0, CRT_DETECT, DATA_BLUE, blue) |
            SMI_FIELD_SET(value, CRT_DETECT, ENABLE, ENABLE);
    ...
    */
    ...
    if (SMI_FIELD_GET(peekRegisterDWord(ulMonitorDetectAddr), CRT_DETECT, CRT) ==
    ...
    value = SMI_FIELD_SET(value, CRT_DETECT, ENABLE, DISABLE);
    pokeRegisterDWord(ulMonitorDetectAddr, value);
...
)
{
	...
	if (lvds == 1)
	{
		ulTmpValue &= SMI_FIELD_CLEAR(LVDS_CTRL2, PD_PLL1);
		if (!on)
			ulTmpValue |= SMI_FIELD_SET(0, LVDS_CTRL2, PD_PLL1, DOWN);
	}
	else
	{
		ulTmpValue &= SMI_FIELD_CLEAR(LVDS_CTRL2, PD_PLL2);
		if (!on)
			ulTmpValue |= SMI_FIELD_SET(0, LVDS_CTRL2, PD_PLL2, DOWN);
	}
...
void setupLVDS(unsigned short lvds)
{
    ...
    ulTmpValue = 0;
    ulTmpValue |=        
        SMI_FIELD_SET(0, LVDS_CTRL1, CLKSEL_PLL2, RE)
      | SMI_FIELD_SET(0, LVDS_CTRL1, CLKSEL_PLL1, RE)
      | SMI_FIELD_SET(0, LVDS_CTRL1, DCLK2, DEFAULT)
      | SMI_FIELD_SET(0, LVDS_CTRL1, DCLK1, DEFAULT);
	...
	ulTmpValue = 0;
    ulTmpValue |=        
        SMI_FIELD_SET(0, LVDS_CTRL2, SHTDNB2, NORMAL)
      | SMI_FIELD_SET(0, LVDS_CTRL2, SHTDNB1, NORMAL)
      | SMI_FIELD_SET(0, LVDS_CTRL2, CLK2_DS, 5MA)
      | SMI_FIELD_SET(0, LVDS_CTRL2, CLK1_DS, 5MA)
      | SMI_FIELD_SET(0, LVDS_CTRL2, DS, 5MA)
      | SMI_FIELD_SET(0, LVDS_CTRL2, CLK2_TR, 0)
      | SMI_FIELD_SET(0, LVDS_CTRL2, CLK1_TR, 0)
      | SMI_FIELD_SET(0, LVDS_CTRL2, TR, 0)
      | SMI_FIELD_SET(0, LVDS_CTRL2, CLK2_COMP, 3)
      | SMI_FIELD_SET(0, LVDS_CTRL2, CLK1_COMP, 3)
      | SMI_FIELD_SET(0, LVDS_CTRL2, PRE_COMP, 3)
      | SMI_FIELD_SET(0, LVDS_CTRL2, VCOS_PLL2, 350M)
      | SMI_FIELD_SET(0, LVDS_CTRL2, VCOS_PLL1, 350M)
      | SMI_FIELD_SET(0, LVDS_CTRL2, SHORTS_PLL2, 2048)
      | SMI_FIELD_SET(0, LVDS_CTRL2, SHORTS_PLL1, 2048)
      | SMI_FIELD_SET(0, LVDS_CTRL2, PD_PLL2, DOWN)
      | SMI_FIELD_SET(0, LVDS_CTRL2, PD_PLL1, DOWN)
      | SMI_FIELD_SET(0, LVDS_CTRL2, MODESEL2, DC1)
      | SMI_FIELD_SET(0, LVDS_CTRL2, MODESEL1, DC0);
...
long set48bitLVDS(disp_control_t dataPath)
{
    ...
    ulTmpValue = peekRegisterDWord(DISPLAY_CTRL+offset)
               & SMI_FIELD_CLEAR(DISPLAY_CTRL, LVDS_OUTPUT_FORMAT)
               & SMI_FIELD_CLEAR(DISPLAY_CTRL, PIXEL_CLOCK_SELECT)
               & SMI_FIELD_CLEAR(DISPLAY_CTRL, DOUBLE_PIXEL_CLOCK);
	...
	// bit 11 = 1
	if (offset)
    	ulTmpValue |= SMI_FIELD_SET(0, DISPLAY_CTRL, LVDS_OUTPUT_FORMAT, CHANNEL1_48BIT);
	else
    	ulTmpValue |= SMI_FIELD_SET(0, DISPLAY_CTRL, LVDS_OUTPUT_FORMAT, CHANNEL0_48BIT);

	ulTmpValue |=
        SMI_FIELD_SET(0, DISPLAY_CTRL, PIXEL_CLOCK_SELECT, HALF)
      | SMI_FIELD_SET(0, DISPLAY_CTRL, DOUBLE_PIXEL_CLOCK, ENABLE);
	...
	{
    	ulTmpValue = peekRegisterDWord(DISPLAY_CTRL) & SMI_FIELD_CLEAR(DISPLAY_CTRL, LVDS_OUTPUT_FORMAT);
	...
	{
    	ulTmpValue = peekRegisterDWord(DISPLAY_CTRL+CHANNEL_OFFSET) & SMI_FIELD_CLEAR(DISPLAY_CTRL, LVDS_OUTPUT_FORMAT);
    ...
    ulTmpValue = peekRegisterDWord(CRT_DETECT+offset)
               & SMI_FIELD_CLEAR(CRT_DETECT, LVDS_CLK);
