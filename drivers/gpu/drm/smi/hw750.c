...
{	
	...
	if(display == 0)
	{
		pokeRegisterDWord(PRIMARY_FB_WIDTH,
		          SMI_FIELD_VALUE(0, PRIMARY_FB_WIDTH, WIDTH, pitch)
		        | SMI_FIELD_VALUE(0, PRIMARY_FB_WIDTH, OFFSET, pitch));
		setDisplayBaseAddress(PRIMARY_CTRL, base_addr);
	}
	else
	{
		pokeRegisterDWord(SECONDARY_FB_WIDTH,
		          SMI_FIELD_VALUE(0, SECONDARY_FB_WIDTH, WIDTH, pitch)
		        | SMI_FIELD_VALUE(0, SECONDARY_FB_WIDTH, OFFSET, pitch));
...
int hw750_en_dis_interrupt(int status, int pipe)
{
	if(status == 0)
	{
		...
		SMI_FIELD_SET(0, INT_MASK, SECONDARY_VSYNC, DISABLE):
		SMI_FIELD_SET(0, INT_MASK, PRIMARY_VSYNC, DISABLE));
	}
	else
	{
		pokeRegisterDWord(INT_MASK, 	(pipe == SECONDARY_CTRL) ? 
		SMI_FIELD_SET(0, INT_MASK, SECONDARY_VSYNC, ENABLE):
		SMI_FIELD_SET(0, INT_MASK, PRIMARY_VSYNC, ENABLE));
	}
	return 0;
...
int hw750_en_dis_interrupt(int status)
	{
		if(status == 0)
		{
			pokeRegisterDWord(INT_MASK, SMI_FIELD_SET(0, INT_MASK, SECONDARY_VSYNC, DISABLE));
			pokeRegisterDWord(INT_MASK, SMI_FIELD_SET(0, INT_MASK, PRIMARY_VSYNC, DISABLE)); 
		}
		else
		{
			pokeRegisterDWord(INT_MASK, SMI_FIELD_SET(0, INT_MASK, SECONDARY_VSYNC, ENABLE));
			pokeRegisterDWord(INT_MASK, SMI_FIELD_SET(0, INT_MASK, PRIMARY_VSYNC, ENABLE));  
		}
		return 0;
	}
...
int hw750_check_vsync_interrupt(int path)
{
	...
	{
	    if ((SMI_FIELD_GET(value1, RAW_INT, PRIMARY_VSYNC) == RAW_INT_PRIMARY_VSYNC_ACTIVE)
			&&(SMI_FIELD_GET(value2, INT_MASK, PRIMARY_VSYNC) == INT_MASK_PRIMARY_VSYNC_ENABLE))
	    {
			return true;
		}
	}else{
		if ((SMI_FIELD_GET(value1, RAW_INT, SECONDARY_VSYNC) == RAW_INT_SECONDARY_VSYNC_ACTIVE)
			&&(SMI_FIELD_GET(value2, INT_MASK, SECONDARY_VSYNC) == INT_MASK_SECONDARY_VSYNC_ENABLE))
		{
			return true;
		}
	}
...
void hw750_clear_vsync_interrupt(int path)
{
	...
	if(path == PRIMARY_CTRL)
	{
		...
		pokeRegisterDWord(RAW_INT, SMI_FIELD_SET(value, RAW_INT, PRIMARY_VSYNC, CLEAR));

	}else{
		
		pokeRegisterDWord(RAW_INT, SMI_FIELD_SET(value, RAW_INT, SECONDARY_VSYNC, CLEAR));	
...
void hw750_setgamma(disp_control_t dispCtrl, unsigned long enable)
{
	...
	if (enable)
	    value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, GAMMA, ENABLE);
	else
	    value = SMI_FIELD_SET(value, PRIMARY_DISPLAY_CTRL, GAMMA, DISABLE);
