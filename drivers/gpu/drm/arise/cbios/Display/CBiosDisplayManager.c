                                              ...
                                              PCBIOS_DISPLAY_MANAGER pDispMgr, PCBiosSettingModeParams pSettingModeParams)
{
    ...
    CBIOS_STATUS csRet = CBIOS_OK;
    PCBIOS_MONITOR_MISC_ATTRIB pAttribute = &pDevCommon->EdidStruct.Attribute;
    CBIOS_BOOL bForceHDTV = CBIOS_FALSE;
    ...
    // patch for a handwriting screen's 800x1280 mode(HBack=20). when HBack is smaller 24, HDisEnd and HBack is not right. So use HDTV module.
    if (!cb_memcmp(pAttribute->MonitorName, "HDMI", 5)
        && !cb_memcmp(pAttribute->ManufactureName, "\xff\xff", 2)
        && (pModeParams->TargetTiming.XRes == 800)
        && (pModeParams->TargetTiming.YRes == 1280)
        && (pModeParams->TargetTiming.HorBEnd - pModeParams->TargetTiming.HorSyncEnd == 20))
    {
...
static  CBIOS_U32  cbDispMgrGetIgaCnt(PCBIOS_EXTENSION_COMMON pcbe)
{
    ...
    switch(pcbe->ChipID)
    {
        ...
        case  CHIPID_ARISE1020:
            Count = (pcbe->FeatureSwitch.bNonSimulChip)? 2 : 4;
            break;
