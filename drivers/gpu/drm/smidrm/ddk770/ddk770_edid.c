 ...
 */
__attribute__((unused)) static unsigned char ddk770_edidGetHeader(
...
)
{
    ...
    version = ddk770_edidGetVersion(pEDIDBuffer, &revision);
...
)
{
    ...
    /* Get EDID Version and revision */
    version = ddk770_edidGetVersion(pEDIDBuffer, (unsigned char *)0);
...
/* You can also use ddk770_edidGetWhitePoint to get the Gamma */
/*
 *  edidGetGamma
 ...
 */
__attribute__((unused)) static unsigned short edidGetGamma(
    unsigned char *pEDIDBuffer
)
{
    unsigned char version;
...
)
{
    ...
    for (index = 10; index > 0; index--)
    {
        ...
        result += ddk770_roundedDiv((colorBinaryValue & 0x0001) * 1000000, ddk770_twoToPowerOfx(index));
        colorBinaryValue >>= 1;
    }
    
    /* Make it accurate to 1000 place */
    return ((unsigned short)ddk770_roundedDiv(result, 1000));
}
 ...
 *      This function reads the EDID structure from attached monitor with SW I2C
 *
 *  Input:
 *      pEDIDBuffer - Buffer that contains the EDID structure of the monitor
...
)
{
    ...
    ddk770_swI2CInit(sclGpio, sdaGpio);
    ...
    for (retry = 0; retry < EDID_TOTAL_RETRY_COUNTER; retry++)
    {
        ...
        for (offset = 0; offset < TOTAL_EDID_REGISTERS; offset++)
            edidBuffer[offset] = ddk770_swI2CReadReg(EDID_DEVICE_I2C_ADDRESS, (unsigned char)offset);
            
        /* Check if the EDID is valid. */
        edidVersion = ddk770_edidGetVersion((unsigned char *)&edidBuffer, (unsigned char *)&edidRevision);
...
)
{
    ...
    for (retry = 0; retry < EDID_TOTAL_RETRY_COUNTER; retry++)
    {
        ...
        /* Read the EDID from the monitor. */
        for (offset = 0; offset < TOTAL_EDID_REGISTERS; offset++)
            edidBuffer[offset] = ddk770_hwI2CReadReg(i2cNumber, EDID_DEVICE_I2C_ADDRESS, (unsigned char)offset);
...
)
{
    ...
    return ddk770_edidReadMonitorEx(pEDIDBuffer, bufferSize, edidExtNo, i2cSCL, i2cSDA);
}
    ...
    if (version == 1)
    {
        ...
        while (1)
        {
            ...
            if ((pDetailedTiming->pixelClock != 0) && (pVDIF != (vdif_t *)0))
            {
                ...
                pVDIF->horizontalFrequency = ddk770_roundedDiv(pVDIF->pixelClock, pVDIF->horizontalTotal);
                ...
                pVDIF->verticalFrequency =
                    ddk770_roundedDiv(pVDIF->pixelClock, (pVDIF->horizontalTotal * pVDIF->verticalTotal));
...
)
{
...

    {
         ...
         */
        if (ddk770_edidGetDetailedTiming(pEDIDBuffer, &index, &vdifBuffer) == 0)
        {
            if (pWidth != (unsigned long *)0)
