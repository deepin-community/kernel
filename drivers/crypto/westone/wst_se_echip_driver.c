...
static int se_ChipInit(SECHIPDRV_CTRL *pDrvCtrl)
{
	...
	for (i = 0; i < SE_BDQUEUE_LEN; i++) {
		for (j = 0; j < SWCHANNELNUM; j++)
			(&((SE_BASIC_BD *)(pDrvCtrl->ulBDMemBase[j]))[i])
				->ucRetCode = 0x0f;
	}
...
static void SE_RESET(SECHIPDRV_CTRL *pdrvctl)
{
	...
	if ((ulreg64 & 0xf0000000000000) != 0xf0000000000000) {
		writeq(ulreg64 | 0xf0000000000000,
		       (volatile void __iomem *)uladdr);
	}
...
static int wst_init(void)
{
	...
	g_psechipDrvCtrl->pdev->coherent_dma_mask =
		(long long unsigned int)&wst_dma_mask;
...
PROG:
		...
		psendpackage =
			(SEND_PACKAGE *)wst_Popfront_Que(&g_SendQueueContainer);
		if (!psendpackage) {
			...
			while (time_before(jiffies, ultimeout)) {
				...
				if ((pdrvctl->ulCurrBdReadPtr[iChannel] ==
				     ((pdrvctl->ulCurrBdWritePtr[iChannel] + 1) &
				      (SE_BDQUEUE_LEN - 1))) ||
				    ((atomic_read(&g_sendtotallen) +
				      *pusOutlen + SE_FILL_LEN) >
				     SE_MAX_SEND_LEN))
#else
				if (pdrvctl->ulCurrBdReadPtr[iChannel] ==
				    ((pdrvctl->ulCurrBdWritePtr[iChannel] + 1) &
...
#endif
				{
					int ret = down_timeout(&(pdrvctl->sema),
							       1 * HZ);
					if (ret < 0) {
						pr_err("Failed to take stats lock event.\n");
						return;
					}
		...
		pCurBD = &((SE_BASIC_BD *)(pdrvctl->ulBDMemBase[iChannel]))
				 [ulCurrWritePtr];
		memset(pCurBD, 0x0, sizeof(SE_BASIC_BD));
		if (pcallback != NULL) {
			(pdrvctl->pcallback)[iChannel][ulCurrWritePtr] =
				pcallback;
		...
		if (pInPtr == pOutPtr) {
			if (pOutPtr) {
				len = usInlen >= *pusOutlen ? usInlen :
							      *pusOutlen;
				if (len) {
					ulCurrAddrOutput = dma_map_single(
						pdrvctl->pdev, pOutPtr, len,
						DMA_BIDIRECTIONAL);
			...
			if (usInlen && pInPtr) {
				ulCurrAddrInput =
					dma_map_single(pdrvctl->pdev, pInPtr,
						       usInlen, DMA_TO_DEVICE);
...
#ifdef CONFIG_MIPS
		atomic_add((*(pdrvctl->pusOutlen[iChannel][ulCurrWritePtr]) +
			    SE_FILL_LEN),
			   &g_sendtotallen);
#endif
...
static int se_hardtrans(SECHIPDRV_CTRL *pdrvctl, unsigned char *pInPtr,
...
static ssize_t wst_low_channel_userwrite_op(SECHIPDRV_CTRL *pdrvctl,
...
static void globalmem_do_rec_op(struct work_struct *p)
{
	...
	while (1) {
		...
		intmessage =
			(INT_MESSAGE *)wst_Popfront_Que(&g_RecQueueContainer);
	...
	for (i = 0; i <= 1; i++) {
		if (ulIntStat & istatus[i]) {
			if (i == 0) {
				...
				read_unlock_irqrestore(&(pdrvctl->mr_lock),
						       ulflags);
				iChannel = 0;
			} else {
				read_lock_irqsave(&(pdrvctl->mr_lock), ulflags);
		...
		while (1) {
			if (ulCurrReadPtr != ulReadPtr) {
				pCurBD = &((SE_BASIC_BD *)(pdrvctl->ulBDMemBase
								   [iChannel]))
						 [ulCurrReadPtr];
				if ((pCurBD->ucRetCode == 0x0f) ||
					...
					memcpy(pdrvctl->pucRetCode[iChannel]
								  [ulCurrReadPtr],
					       &ucMyRetCode, 1);
					...
					atomic_sub(
						...
						&g_sendtotallen);
#endif
					if ((pdrvctl->ikernel)[iChannel]
							      [ulCurrReadPtr] !=
					    0) {
						if (pdrvctl->pcallback
							    [iChannel]
							    [ulCurrReadPtr]) {
							INT_MESSAGE *intmessage =
								NULL;
							unsigned long ulflags1;
							intmessage = (INT_MESSAGE *)kmalloc(
								sizeof(INT_MESSAGE),
								GFP_ATOMIC);
							if (!intmessage)
								return IRQ_HANDLED;
					...
					ulCurrReadPtr = ((ulCurrReadPtr + 1) &
							 (SE_BDQUEUE_LEN - 1));
					pdrvctl->ulCurrReadPtr[iChannel] =
						ulCurrReadPtr;
					pdrvctl->ulCurrBdReadPtr[iChannel] =
						ulCurrReadPtr;
