...
static const char *
hwaddr_parse(const char *txt, u8 *addr) {
	size_t i;
 ...
 */
int
hwaddr_aton(const char *txt, u8 *addr) {
	return hwaddr_parse(txt, addr) ? 0 : -1;
}

/**
 * hwaddr_masked_aton - Convert ASCII string with optional mask to
 *  MAC address (colon-delimited format)
 * @txt: MAC address with optional mask as a string
 *  (e.g., "00:11:22:33:44:55/ff:ff:ff:ff:00:00")
 * @addr: Buffer for the MAC address (ETH_ALEN = 6 bytes)
 ...
 */
int
hwaddr_masked_aton(const char *txt, u8 *addr, u8 *mask, u8 maskable) {
#if 0
	const char *r;
...
/**
 * hwaddr_compact_aton - Convert ASCII string to MAC address
 *  (no colon delimitors format)
 * @txt: MAC address as a string (e.g., "001122334455")
 * @addr: Buffer for the MAC address (ETH_ALEN = 6 bytes)
 * Returns: 0 on success, -1 on failure (e.g., string not a MAC address)
 */
int
hwaddr_compact_aton(const char *txt, u8 *addr) {
	int i;
 ...
 */
int
hwaddr_aton2(const char *txt, u8 *addr) {
	int i;
	const char *pos = txt;
...
int
hwaddr_mask_txt(char *buf, size_t len, const u8 *addr, const u8 *mask) {
	size_t i;
	int print_mask = 0;
	int res;
	...
	if (print_mask)
		res = os_snprintf(buf, len, MACSTR "/" MACSTR, MAC2STR(addr),
				  MAC2STR(mask));
	else
 ...
 */
void
inc_byte_array(u8 *counter, size_t len) {
	int pos = len - 1;
 ...
 */
int
wpa_scnprintf(char *buf, size_t size, const char *fmt, ...) {
#if 0
	va_list ap;
	int ret;
...
int
wpa_snprintf_hex_sep(char *buf, size_t buf_size, const u8 *data, size_t len,
		     char sep) {
	size_t i;
 ...
 */
int
wpa_snprintf_hex_uppercase(char *buf, size_t buf_size, const u8 *data,
			   size_t len) {
 ...
 */
void
wpa_unicode2ascii_inplace(TCHAR *str) {
#ifdef UNICODE
	char *dst = (char *)str;
...
void
printf_encode(char *txt, size_t maxlen, const u8 *data, size_t len) {
	char *end = txt + maxlen;
	size_t i;
...
size_t
printf_decode(u8 *buf, size_t maxlen, const char *str) {
	const char *pos = str;
	size_t len = 0;
	int val;
 ...
 */
const char *
wpa_ssid_txt(const u8 *ssid, size_t ssid_len) {
...
void *
__hide_aliasing_typecast(void *foo) {
	return foo;
}

char *
wpa_config_parse_string(const char *value, size_t *len) {
#if 0
	if (*value == '"') {
		const char *pos;
		char *str;
...
int
is_hex(const u8 *data, size_t len) {
	size_t i;
...
size_t
merge_byte_arrays(u8 *res, size_t res_len, const u8 *src1, size_t src1_len,
...
char *
_dup_binstr(const void *src, size_t len) {
	char *res;
...
int
freq_range_list_parse(struct wpa_freq_range_list *res, const char *value) {
#if 0
...
int
freq_range_list_includes(const struct wpa_freq_range_list *list,
			 unsigned int freq) {
	unsigned int i;
...
char *
freq_range_list_str(const struct wpa_freq_range_list *list) {
	char *buf, *pos, *end;
	size_t maxlen;
	unsigned int i;
	int res;
...
void
int_array_concat(int **res, const int *a) {
#if 0
	int reslen, alen, i;
	int *n;
...
void
int_array_add_unique(int **res, int a) {
#if 0
	int reslen;
	int *n;
...
void
bin_clear_free(void *bin, size_t len) {
	if (bin) {
		os_memset(bin, 0, len);
		os_free(bin);
	}
 ...
 */
const char *
cstr_token(const char *str, const char *delim, const char **last) {
 ...
 */
char *
str_token(char *str, const char *delim, char **context) {
...
size_t
utf8_unescape(const char *inp, size_t in_size, char *outp, size_t out_size) {
	size_t res_size = 0;
...
size_t
utf8_escape(const char *inp, size_t in_size, char *outp, size_t out_size) {
	size_t res_size = 0;
