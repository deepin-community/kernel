...
ULONG               MPAllocCount;       // the number of outstanding allocs
    ...
    )
{
	...
	DbgPrintF(MP_LOUD,  "%s - file %s, line %d, Size %d failed!", __func__, FileNumber, LineNumber, Size);
    } else
    {
    ...
    DbgPrintF(MP_LOUD,
	      "%s - file %s, line %d, %d bytes, [0x" PTR_FORMAT "].", __func__,
