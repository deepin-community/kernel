...
static void fxgmac_free_channels(struct fxgmac_pdata *pdata)
{
	if (!pdata->channel_head)
		return;
	if (netif_msg_drv(pdata))
		DPRINTK("free_channels,tx_ring=%p ,rx_ring=%p ,channel=%p\n",
			pdata->channel_head->tx_ring,
			pdata->channel_head->rx_ring, pdata->channel_head);
#ifndef FXGMAC_USE_STATIC_ALLOC
...
static int fxgmac_alloc_channels(struct fxgmac_pdata *pdata)
{
	...
	if (netif_msg_drv(pdata))
		DPRINTK("static_alloc_channels,channel_head=%p,tx_ring=%p,rx_ring=%p\n",
...
#ifdef FXGMAC_TX_DMA_MAP_SINGLE
			skb_dma = dma_map_single(pdata->dev, addr + offset, len,
						 DMA_TO_DEVICE);
#else
