...
static int fxgmac_read_mac_addr(struct fxgmac_pdata *pdata)
{
	...
	ret = hw_ops->read_mac_subsys_from_efuse(pdata, pdata->mac_addr, NULL,
						 NULL);
	if (!ret) {
...
int fxgmac_init(struct fxgmac_pdata *pdata, bool save_private_reg)
{
	...
	if (ret) {
		ret = dma_set_mask_and_coherent(
			pdata->dev, DMA_BIT_MASK(FXGMAC_DMA_BIT_MASK32));
		if (ret) {
			dev_err(pdata->dev,
...
#else
	/* this is for test only. HW does not want to change Hash key */
	...
	for (i = 0; i < FXGMAC_RSS_MAX_TABLE_SIZE; i++) {
		pdata->rss_table[i] = FXGMAC_SET_REG_BITS(
			...
			MAC_RSSDR_DMCH_LEN, (i % 3) + 1); /* eliminate ch0 */
	}
#else
	for (i = 0; i < FXGMAC_RSS_MAX_TABLE_SIZE; i++) {
		pdata->rss_table[i] = FXGMAC_SET_REG_BITS(
			pdata->rss_table[i], MAC_RSSDR_DMCH_POS,
			MAC_RSSDR_DMCH_LEN,
			/* note, rx_ring_count should be equal to IRQ requsted
			 * for MSIx, 4
			 */
	...
	pdata->rss_options = FXGMAC_SET_REG_BITS(pdata->rss_options,
						 MAC_RSSCR_IP4TE_POS,
	...
	if (pdata->hw_feat.rss) {
		netdev->hw_features |=
			NETIF_F_RXHASH; /* it is NETIF_F_RXHASH_BIT finally */
	}
	...
	if (vectors >= FXGMAC_MAX_DMA_CHANNELS) {
		...
		if (!pdata->expansion.msix_entries) {
			...
			if (req_vectors >= FXGMAC_MAX_DMA_CHANNELS_PLUS_1TX) {
				dev_info(pdata->dev,
					 "enable MSIx ok, cpu=%d, vectors=%d.\n",
					 vectors, req_vectors);
				...
				dev_err(pdata->dev,
					"enable MSIx failure and clear msix entries.\n");
				/* roll back to msi */
				kfree(pdata->expansion.msix_entries);
				pdata->expansion.msix_entries = NULL;
				req_vectors = 0;
			 ...
			 unsigned int idx)
{
	...
	netdev_dbg(pdata->netdev,
		   ...
		   idx, le32_to_cpu(dma_desc->desc0),
		   le32_to_cpu(dma_desc->desc1), le32_to_cpu(dma_desc->desc2),
		   le32_to_cpu(dma_desc->desc3));
}

void fxgmac_dbg_pkt(struct net_device *netdev, struct sk_buff *skb, bool tx_rx)
{
...
void fxgmac_get_all_hw_features(struct fxgmac_pdata *pdata)
{
	...
	hw_feat->phyifsel = FXGMAC_GET_REG_BITS(
		mac_hfr0, MAC_HWF0R_ACTPHYIFSEL_POS, MAC_HWF0R_ACTPHYIFSEL_LEN);
	hw_feat->vlhash = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_VLHASH_POS,
					      MAC_HWF0R_VLHASH_LEN);
	hw_feat->sma = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_SMASEL_POS,
					   MAC_HWF0R_SMASEL_LEN);
	hw_feat->rwk = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_RWKSEL_POS,
					   MAC_HWF0R_RWKSEL_LEN);
	hw_feat->mgk = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_MGKSEL_POS,
					   MAC_HWF0R_MGKSEL_LEN);
	hw_feat->mmc = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_MMCSEL_POS,
					   MAC_HWF0R_MMCSEL_LEN);
	hw_feat->aoe = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_ARPOFFSEL_POS,
					   MAC_HWF0R_ARPOFFSEL_LEN);
	hw_feat->ts = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_TSSEL_POS,
					  MAC_HWF0R_TSSEL_LEN);
	hw_feat->eee = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_EEESEL_POS,
					   MAC_HWF0R_EEESEL_LEN);
	hw_feat->tx_coe = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_TXCOESEL_POS,
					      MAC_HWF0R_TXCOESEL_LEN);
	hw_feat->rx_coe = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_RXCOESEL_POS,
					      MAC_HWF0R_RXCOESEL_LEN);
	hw_feat->addn_mac = FXGMAC_GET_REG_BITS(mac_hfr0,
						MAC_HWF0R_ADDMACADRSEL_POS,
						MAC_HWF0R_ADDMACADRSEL_LEN);
	hw_feat->ts_src = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_TSSTSSEL_POS,
					      MAC_HWF0R_TSSTSSEL_LEN);
	hw_feat->sa_vlan_ins = FXGMAC_GET_REG_BITS(
		mac_hfr0, MAC_HWF0R_SAVLANINS_POS, MAC_HWF0R_SAVLANINS_LEN);

	/* Hardware feature register 1 */
	hw_feat->rx_fifo_size = FXGMAC_GET_REG_BITS(
		mac_hfr1, MAC_HWF1R_RXFIFOSIZE_POS, MAC_HWF1R_RXFIFOSIZE_LEN);
	hw_feat->tx_fifo_size = FXGMAC_GET_REG_BITS(
		mac_hfr1, MAC_HWF1R_TXFIFOSIZE_POS, MAC_HWF1R_TXFIFOSIZE_LEN);
	hw_feat->adv_ts_hi = FXGMAC_GET_REG_BITS(
		mac_hfr1, MAC_HWF1R_ADVTHWORD_POS, MAC_HWF1R_ADVTHWORD_LEN);
	hw_feat->dma_width = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_ADDR64_POS,
						 MAC_HWF1R_ADDR64_LEN);
	hw_feat->dcb = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_DCBEN_POS,
					   MAC_HWF1R_DCBEN_LEN);
	hw_feat->sph = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_SPHEN_POS,
					   MAC_HWF1R_SPHEN_LEN);
	hw_feat->tso = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_TSOEN_POS,
					   MAC_HWF1R_TSOEN_LEN);
	hw_feat->dma_debug = FXGMAC_GET_REG_BITS(
		mac_hfr1, MAC_HWF1R_DBGMEMA_POS, MAC_HWF1R_DBGMEMA_LEN);
#if (FXGMAC_RSS_FEATURE_ENABLED)
	hw_feat->rss = 1;
#else
	/* = FXGMAC_GET_REG_BITS(mac_hfr1,
	 * MAC_HWF1R_RSSEN_POS,
	 * MAC_HWF1R_RSSEN_LEN);
	 */
	hw_feat->rss = 0;
	 ...
	 */
	hw_feat->tc_cnt = 3;
	hw_feat->avsel = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_AVSEL_POS,
					     MAC_HWF1R_AVSEL_LEN);
	hw_feat->ravsel = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_RAVSEL_POS,
					      MAC_HWF1R_RAVSEL_LEN);
	hw_feat->hash_table_size = FXGMAC_GET_REG_BITS(
		mac_hfr1, MAC_HWF1R_HASHTBLSZ_POS, MAC_HWF1R_HASHTBLSZ_LEN);
	hw_feat->l3l4_filter_num = FXGMAC_GET_REG_BITS(
		mac_hfr1, MAC_HWF1R_L3L4FNUM_POS, MAC_HWF1R_L3L4FNUM_LEN);


	/* Hardware feature register 2
	 * FXGMAC_GET_REG_BITS(mac_hfr2,
	 * MAC_HWF2R_RXQCNT_POS,
	 * MAC_HWF2R_RXQCNT_LEN)
	 */
	hw_feat->rx_q_cnt = 3;
	hw_feat->tx_q_cnt = FXGMAC_GET_REG_BITS(mac_hfr2, MAC_HWF2R_TXQCNT_POS,
						MAC_HWF2R_TXQCNT_LEN);
	hw_feat->rx_ch_cnt = FXGMAC_GET_REG_BITS(
		mac_hfr2, MAC_HWF2R_RXCHCNT_POS, MAC_HWF2R_RXCHCNT_LEN);
	hw_feat->tx_ch_cnt = FXGMAC_GET_REG_BITS(
		mac_hfr2, MAC_HWF2R_TXCHCNT_POS, MAC_HWF2R_TXCHCNT_LEN);
	hw_feat->pps_out_num = FXGMAC_GET_REG_BITS(
		mac_hfr2, MAC_HWF2R_PPSOUTNUM_POS, MAC_HWF2R_PPSOUTNUM_LEN);
	hw_feat->aux_snap_num = FXGMAC_GET_REG_BITS(
		mac_hfr2, MAC_HWF2R_AUXSNAPNUM_POS, MAC_HWF2R_AUXSNAPNUM_LEN);
