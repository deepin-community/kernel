				       ...
				       struct ethtool_drvinfo *drvinfo)
{
	 ...
	 */
	sver = FXGMAC_GET_REG_BITS(ver, MAC_VR_SVER_POS, MAC_VR_SVER_LEN);
					...
					struct ethtool_channels *channel)
{
	...
	DPRINTK("fxmac rss, get channels max=(combined %d, other %d), count(combined %d, other %d)\n",
		channel->max_combined, channel->max_other,
		channel->combined_count, channel->other_count);
#endif
...
#else
	rss_m = FXGMAC_MAX_DMA_CHANNELS -
		1; /* mask for index of channel, 0-3 */
#endif
				    ...
				    struct ethtool_rxnfc *cmd)
{
	...
	case TCP_V4_FLOW:
	case UDP_V4_FLOW:
		if (((TCP_V4_FLOW == (cmd->flow_type)) &&
		     (FXGMAC_GET_REG_BITS(pdata->rss_options,
					  MAC_RSSCR_TCP4TE_POS,
					  MAC_RSSCR_TCP4TE_LEN))) ||
		    ((UDP_V4_FLOW == (cmd->flow_type)) &&
		     (FXGMAC_GET_REG_BITS(pdata->rss_options,
					  MAC_RSSCR_UDP4TE_POS,
					  MAC_RSSCR_UDP4TE_LEN)))) {
	...
	case TCP_V6_FLOW:
	case UDP_V6_FLOW:
		if (((TCP_V6_FLOW == (cmd->flow_type)) &&
		     (FXGMAC_GET_REG_BITS(pdata->rss_options,
					  MAC_RSSCR_TCP6TE_POS,
					  MAC_RSSCR_TCP6TE_LEN))) ||
		    ((UDP_V6_FLOW == (cmd->flow_type)) &&
		     (FXGMAC_GET_REG_BITS(pdata->rss_options,
					  MAC_RSSCR_UDP6TE_POS,
					  MAC_RSSCR_UDP6TE_LEN)))) {
			cmd->data |= RXH_L4_B_0_1 | RXH_L4_B_2_3;
		}
		fallthrough;
	case SCTP_V6_FLOW:
				   ...
				   struct ethtool_rxnfc *nfc)
{
	...
	case TCP_V6_FLOW:
		...
		if (TCP_V4_FLOW == (nfc->flow_type)) {
			rssopt = FXGMAC_SET_REG_BITS(rssopt,
						     MAC_RSSCR_IP4TE_POS,
	...
	case UDP_V4_FLOW:
		if (!(nfc->data & RXH_IP_SRC) || !(nfc->data & RXH_IP_DST))
			return -EINVAL;
		rssopt = FXGMAC_SET_REG_BITS(rssopt, MAC_RSSCR_IP4TE_POS,
					     MAC_RSSCR_IP4TE_LEN, 1);
	...
	case UDP_V6_FLOW:
		if (!(nfc->data & RXH_IP_SRC) || !(nfc->data & RXH_IP_DST))
			return -EINVAL;
		rssopt = FXGMAC_SET_REG_BITS(rssopt, MAC_RSSCR_IP6TE_POS,
					     MAC_RSSCR_IP6TE_LEN, 1);
			   ...
			   struct ethtool_wolinfo *wol)
{
	...
	if (!(pdata->hw_feat.rwk) ||
	    !device_can_wakeup(/*pci_dev_to_dev*/ (pdata->dev))) {
...
{
	/* enable or disable WOL. this function only set wake-up type, and
	 * power related configure will be in other place, see power management.
	 */
	struct fxgmac_hw_ops *hw_ops = &pdata->hw_ops;
			    ...
			    struct ethtool_regs *regs, void *p)
{
	...
	regs->version = regs_buff[REG_MII_PHYSID1] << 16 |
				     ...
				     struct ethtool_link_ksettings *cmd)
{
	...
	if (FXGMAC_GET_REG_BITS(regval, PHY_MII_ADVERTISE_PAUSE_POS,
				PHY_MII_ADVERTISE_PAUSE_LEN))
		ethtool_link_ksettings_add_link_mode(cmd, advertising, Pause);
	if (FXGMAC_GET_REG_BITS(regval, PHY_MII_ADVERTISE_ASYPAUSE_POS,
...
static int fxgmac_ethtool_reset(struct net_device *netdev, u32 *flag)
{
	...
	case ETH_RESET_PHY:
		...
		val = FXGMAC_SET_REG_BITS(val, PHY_CR_POWER_POS,
					  PHY_CR_POWER_LEN, PHY_POWER_DOWN);
