...
int fxgmac_phy_force_mode(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, PHY_CR_AUTOENG_POS,
				     PHY_CR_AUTOENG_LEN, pdata->phy_autoeng);
	...
	regval = FXGMAC_SET_REG_BITS(regval, PHY_CR_DUPLEX_POS,
				     PHY_CR_DUPLEX_LEN, pdata->phy_duplex);
...
void fxgmac_set_phy_link_ksettings(struct fxgmac_pdata *pdata)
{
	...
	if (pdata->phy_autoeng ||
	    (!pdata->phy_autoeng && pdata->phy_speed == SPEED_1000))
				     ...
				     int *duplex)
{
	...
	*duplex = (val & BIT(FXGMAC_EPHY_DUPLEX_BIT)) >> FXGMAC_EPHY_DUPLEX_BIT;
	speed_mode = (val & FXGMAC_EPHY_SPEED_MODE) >>
		     FXGMAC_EPHY_SPEED_MODE_BIT;
	switch (speed_mode) {
	case 0:
		if (is_utp)
			*speed = SPEED_10M;
 ...
 *  media: only valid when ret_link=1, (YT8614_SMI_SEL_SDS_SGMII + 1)
 *   for fiber; (YT8614_SMI_SEL_PHY + 1) for utp. -1 for link down.
 */
...
void fxgmac_phy_update_link(struct net_device *netdev)
{
	...
	cur_speed = FXGMAC_GET_REG_BITS(regval, MGMT_EPHY_CTRL_STA_SPEED_POS,
					MGMT_EPHY_CTRL_STA_SPEED_LEN);
	pdata->phy_speed = (cur_speed == 2) ? SPEED_1000 :
			   (cur_speed == 1) ? SPEED_100 :
						    SPEED_10;
	pdata->phy_duplex =
		FXGMAC_GET_REG_BITS(regval, MGMT_EPHY_CTRL_STA_EPHY_DUPLEX_POS,
	...
	if (pdata->expansion.phy_link != cur_link) {
		...
		if (cur_link) {
			...
			if (FXGMAC_GET_REG_BITS(
				    regval, PHY_MII_LINK_PARNTNER_10FULL_POS,
				    PHY_MII_LINK_PARNTNER_10FULL_LEN) ||
			    FXGMAC_GET_REG_BITS(
				    regval, PHY_MII_LINK_PARNTNER_10HALF_POS,
				    PHY_MII_LINK_PARNTNER_10HALF_LEN)) {
...
static void fxgmac_phy_link_poll(struct timer_list *t)
{
	struct fxgmac_pdata *pdata =
		from_timer(pdata, t, expansion.phy_poll_tm);
...
#endif
	{
		...
		DPRINTK("%s powerstate changed, %ld, netdev=%lx, tm=%lx\n",
			__func__, pdata->expansion.powerstate,
			(unsigned long)(pdata->netdev),
			(unsigned long)&pdata->expansion.phy_poll_tm);
	}
}
