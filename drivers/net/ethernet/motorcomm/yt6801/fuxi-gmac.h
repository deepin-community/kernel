...
/* note, maybe we should refer to NDIS_PM_WAKE_REASON_TYPE
 * to avoid duplication definition....
 */
typedef enum {
	WAKE_REASON_NONE = 0,
	WAKE_REASON_MAGIC,
	...
	/* for wake up method like Link-change, for that,
	 * GMAC cannot identify and need more checking.
	 */
	WAKE_REASON_TBD,
	WAKE_REASON_HW_ERR,
} WAKE_REASON;
...
struct fxgmac_hw_ops {
	...
	void (*enable_channel_rx)(struct fxgmac_pdata *pdata,
				  unsigned int queue);
	...
	int (*config_rsf_mode)(struct fxgmac_pdata *pdata, unsigned int val);
	int (*config_tsf_mode)(struct fxgmac_pdata *pdata, unsigned int val);
	...
	bool (*update_stats_counters)(struct fxgmac_pdata *pdata,
				      bool ephy_check_en);
	...
	int (*set_rss_lookup_table)(struct fxgmac_pdata *pdata,
				    const u32 *table);
	...
	/*NS offload*/
	int (*set_ns_offload)(struct fxgmac_pdata *pdata, unsigned int index,
	...
	int (*set_wake_pattern)(struct fxgmac_pdata *pdata,
				...
				u32 pattern_cnt);
	int (*enable_wake_pattern)(struct fxgmac_pdata *pdata);
...
#else
	void (*config_power_down)(struct fxgmac_pdata *pdata,
				  unsigned int offloadcount, bool magic_en,
				  bool remote_pattern_en);
#endif
	...
	int (*set_all_multicast_mode)(struct fxgmac_pdata *pdata,
				      unsigned int enable);
	void (*config_multicast_mac_hash_table)(struct fxgmac_pdata *pdata,
						unsigned char *pmc_mac,
						int b_add);

	/* for packet filter-promiscuous and broadcast */
	int (*set_promiscuous_mode)(struct fxgmac_pdata *pdata,
				    unsigned int enable);
	int (*enable_rx_broadcast)(struct fxgmac_pdata *pdata,
				   unsigned int enable);
	...
	bool (*write_mac_subsys_to_efuse)(struct fxgmac_pdata *pdata,
					  u8 *mac_addr, u32 *subsys,
					  u32 *revid);
	bool (*read_mac_addr_from_efuse)(struct fxgmac_pdata *pdata,
					 u8 *mac_addr);
	...
	void (*trigger_pcie)(struct fxgmac_pdata *pdata, u32 code);
};
...
struct fxgmac_pdata {
	...
	/* change type from int to u32 to match MSIx, p_msix_entry.vector; */
	u32 channel_irq[FXGMAC_MAX_DMA_CHANNELS_PLUS_1TX];
