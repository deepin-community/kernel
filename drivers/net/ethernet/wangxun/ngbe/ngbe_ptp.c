...
static void ngbe_ptp_setup_sdp(struct ngbe_adapter *adapter)
{
	...
	if (NGBE_1588_PPS_WIDTH * NS_PER_MSEC >= NS_PER_SEC) {
		e_dev_err("PTP pps width cannot be longer than 1s!\n");
		return;
	}

	/* disable the pin first */
	wr32(hw, NGBE_TSEC_1588_AUX_CTL, 0);
	NGBE_WRITE_FLUSH(hw);

	if (!(adapter->flags2 & NGBE_FLAG2_PTP_PPS_ENABLED)) {
		if (adapter->pps_enabled == 1) {
			adapter->pps_enabled = 0;
			if (NGBE_1588_TOD_ENABLE)
				ngbe_set_pps(hw, adapter->pps_enabled, 0, 0);
		}
		return;
	}
	...
	tssdp = NGBE_TSEC_1588_SDP_FUN_SEL_TT0;
	tssdp |= NGBE_1588_PPS_LEVEL ? NGBE_TSEC_1588_SDP_OUT_LEVEL_HIGH :
				       NGBE_TSEC_1588_SDP_OUT_LEVEL_LOW;
	tsauxc = NGBE_TSEC_1588_AUX_CTL_PLSG | NGBE_TSEC_1588_AUX_CTL_EN_TT0 |
		 NGBE_TSEC_1588_AUX_CTL_EN_TT1 | NGBE_TSEC_1588_AUX_CTL_EN_TS0;

	tssdp1 = NGBE_TSEC_1588_SDP_FUN_SEL_TS0;
	...
	if (NGBE_1588_TOD_ENABLE)
		ngbe_set_pps(hw, adapter->pps_enabled, ns + rem,
			     adapter->pps_edge_start);

	rem += NGBE_1588_PPS_WIDTH * NS_PER_MSEC;
	...
	wr32(hw, NGBE_TSEC_1588_TRGT_L(0), trgttiml0);
	wr32(hw, NGBE_TSEC_1588_TRGT_H(0), trgttimh0);
	wr32(hw, NGBE_TSEC_1588_TRGT_L(1), trgttiml1);
	wr32(hw, NGBE_TSEC_1588_TRGT_H(1), trgttimh1);
	wr32(hw, NGBE_TSEC_1588_SDP(0), tssdp);
	wr32(hw, NGBE_TSEC_1588_SDP(1), tssdp1);
	wr32(hw, NGBE_TSEC_1588_AUX_CTL, tsauxc);
	wr32(hw, NGBE_TSEC_1588_INT_EN, NGBE_TSEC_1588_INT_EN_TT1);
	NGBE_WRITE_FLUSH(hw);
 ...
 */
static int ngbe_ptp_adjfine(struct ptp_clock_info *ptp, long ppb)
{
	struct ngbe_adapter *adapter =
 ...
 */
static int ngbe_ptp_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	struct ngbe_adapter *adapter =
		container_of(ptp, struct ngbe_adapter, ptp_caps);
	unsigned long flags;
 ...
 */
static int ngbe_ptp_gettime64(struct ptp_clock_info *ptp, struct timespec64 *ts)
{
	struct ngbe_adapter *adapter =
		container_of(ptp, struct ngbe_adapter, ptp_caps);
	unsigned long flags;
...
void ngbe_ptp_check_pps_event(struct ngbe_adapter *adapter)
{
	...
	int_status = rd32(hw, NGBE_TSEC_1588_INT_ST);
	if (int_status & NGBE_TSEC_1588_INT_ST_TT1) {
		/* disable the pin first */
		wr32(hw, NGBE_TSEC_1588_AUX_CTL, 0);
		NGBE_WRITE_FLUSH(hw);

		tsauxc = NGBE_TSEC_1588_AUX_CTL_PLSG |
			 NGBE_TSEC_1588_AUX_CTL_EN_TT0 |
			 NGBE_TSEC_1588_AUX_CTL_EN_TT1 |
			 NGBE_TSEC_1588_AUX_CTL_EN_TS0;
		...
		/* Adjust the clock edge to align with the next full second. */
		adapter->pps_edge_start +=
			div_u64(((u64)rem << cc->shift), cc->mult);
		trgttiml0 = (u32)adapter->pps_edge_start;
		trgttimh0 = (u32)(adapter->pps_edge_start >> 32);

		rem += NGBE_1588_PPS_WIDTH * NS_PER_MSEC;
		adapter->pps_edge_end +=
			div_u64(((u64)rem << cc->shift), cc->mult);
		trgttiml1 = (u32)adapter->pps_edge_end;
		trgttimh1 = (u32)(adapter->pps_edge_end >> 32);
...
{
	struct ngbe_adapter *adapter =
		container_of(work, struct ngbe_adapter, ptp_tx_work);
	struct ngbe_hw *hw = &adapter->hw;
				       ...
				       struct hwtstamp_config *config)
{
	...
	if (is_l2)
		wr32(hw, NGBE_PSR_ETYPE_SWC(NGBE_PSR_ETYPE_SWC_FILTER_1588),
		     (NGBE_PSR_ETYPE_SWC_FILTER_EN | /* enable filter */
		      NGBE_PSR_ETYPE_SWC_1588 | /* enable timestamping */
		      ETH_P_1588)); /* 1588 eth protocol type */
	else
		wr32(hw, NGBE_PSR_ETYPE_SWC(NGBE_PSR_ETYPE_SWC_FILTER_1588), 0);
...
static void ngbe_ptp_link_speed_adjust(struct ngbe_adapter *adapter, u32 *shift,
				       u32 *incval)
{
	/**
...
static void ngbe_ptp_init_systime(struct ngbe_adapter *adapter)
{
	struct ngbe_hw *hw = &adapter->hw;

	wr32(hw, NGBE_TSEC_1588_SYSTIML, 0);
	wr32(hw, NGBE_TSEC_1588_SYSTIMH, 0);
	NGBE_WRITE_FLUSH(hw);
}
...
static long ngbe_ptp_create_clock(struct ngbe_adapter *adapter)
{
	...
	adapter->ptp_caps.adjfine = ngbe_ptp_adjfine;
	...
	adapter->ptp_setup_sdp = ngbe_ptp_setup_sdp;
