...
#define SPI_CLK_DIV_OFFSET \
	25 // SPI clock divide field offset in Command register
...
#define MAC_ADDR0_WORD0_OFFSET_1G \
	0x006000c // MAC Address for LAN0, stored in external FLASH
#define MAC_ADDR0_WORD1_OFFSET_1G 0x0060014
#define MAC_ADDR1_WORD0_OFFSET_1G \
	0x006800c // MAC Address for LAN1, stored in external FLASH
#define MAC_ADDR1_WORD1_OFFSET_1G 0x0068014
#define MAC_ADDR2_WORD0_OFFSET_1G \
	0x007000c // MAC Address for LAN2, stored in external FLASH
#define MAC_ADDR2_WORD1_OFFSET_1G 0x0070014
#define MAC_ADDR3_WORD0_OFFSET_1G \
	0x007800c // MAC Address for LAN3, stored in external FLASH
#define MAC_ADDR3_WORD1_OFFSET_1G 0x0078014
#define PRODUCT_SERIAL_NUM_OFFSET_1G \
	0x00f0000 // Product Serial Number, stored in external FLASH last sector
...
int ngbe_read_pba_string(struct ngbe_hw *hw, u8 *pba_num, u32 pba_num_size);
...
int ngbe_set_vlvf(struct ngbe_hw *hw, u32 vlan, u32 vind, bool vlan_on,
		  bool *vfta_changed);
...
int ngbe_set_fw_drv_ver(struct ngbe_hw *hw, u8 maj, u8 min, u8 build, u8 ver);
int ngbe_reset_hostif(struct ngbe_hw *hw);
u8 ngbe_calculate_checksum(u8 *buffer, u32 length);
int ngbe_host_interface_command(struct ngbe_hw *hw, u32 *buffer, u32 length,
...
int ngbe_setup_mac_link_multispeed_fiber(struct ngbe_hw *hw, u32 speed,
...
void ngbe_set_hard_rate_select_speed(struct ngbe_hw *hw, u32 speed);
...
int ngbe_read_flash_buffer(struct ngbe_hw *hw, u32 offset, u32 dwords,
			   u32 *data);
int ngbe_write_flash_buffer(struct ngbe_hw *hw, u32 offset, u32 dwords,
			    u32 *data);

int ngbe_read_eeprom(struct ngbe_hw *hw, u16 offset, u16 *data);
int ngbe_read_eeprom_buffer(struct ngbe_hw *hw, u16 offset, u16 words,
			    u16 *data);
...
int ngbe_upgrade_flash(struct ngbe_hw *hw, u32 region, const u8 *data,
		       u32 size);
...
int ngbe_read_ee_hostif_buffer(struct ngbe_hw *hw, u16 offset, u16 words,
			       u16 *data);
...
int ngbe_upgrade_flash_hostif(struct ngbe_hw *hw, u32 region, const u8 *data,
			      u32 size);
