			       ...
			       unsigned int num_vfs)
{
	...
	num_vf_macvlans =
		hw->mac.num_rar_entries - (NGBE_MAX_PF_MACVLANS + 1 + num_vfs);
	 ...
	 */
	adapter->vfinfo =
		kcalloc(num_vfs, sizeof(struct vf_data_storage), GFP_KERNEL);
	if (!adapter->vfinfo) {
		adapter->num_vfs = 0;
	...
	for (i = 0; i < adapter->num_vfs; i++) {
		...
		adapter->vfinfo[i].link_state = NGBE_VF_LINK_STATE_AUTO;
	...
	wr32m(hw, NGBE_CFG_PORT_CTL, NGBE_CFG_PORT_CTL_NUM_VT_MASK,
...
int ngbe_disable_sriov(struct ngbe_adapter *adapter)
{
	...
	wr32m(hw, NGBE_PSR_VM_CTL, NGBE_PSR_VM_CTL_POOL_MASK, 0);
	NGBE_WRITE_FLUSH(hw);
...
static int ngbe_set_vf_multicasts(struct ngbe_adapter *adapter, u32 *msgbuf,
				  u32 vf)
{
	...
	entries = min_t(u16, entries, (u16)NGBE_MAX_VF_MC_ENTRIES);
	...
	ngbe_write_mc_addr_list(adapter->netdev);
...
void ngbe_restore_vf_multicasts(struct ngbe_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_vfs; i++) {
		...
		for (j = 0; j < vfinfo->num_vf_mc_hashes; j++) {
			...
			wr32m(hw, NGBE_PSR_MC_TBL(vector_reg), 1 << vector_bit,
			      1 << vector_bit);
...
static int ngbe_set_vf_lpe(struct ngbe_adapter *adapter, u32 max_frame, u32 vf)
{
	struct ngbe_hw *hw = &adapter->hw;
	u32 max_frs, reg_val;
...
static void ngbe_set_vmvir(struct ngbe_adapter *adapter, u16 vid, u16 qos,
			   u16 vf, __be16 vlan_proto)
{
	struct ngbe_hw *hw = &adapter->hw;
	...
	if (vlan_proto == htons(ETH_P_8021AD))
		vmvir |= 1 << NGBE_TDM_VLAN_INS_TPID_SEL_SHIFT;
...
static inline void ngbe_vf_reset_event(struct ngbe_adapter *adapter, u16 vf)
{
	...
	if (!vfinfo->pf_vlan && !vfinfo->pf_qos && !num_tcs) {
		...
		if (vfinfo->pf_qos || !num_tcs)
			ngbe_set_vmvir(adapter, vfinfo->pf_vlan, vfinfo->pf_qos,
				       vf, vfinfo->vlan_proto);
		else
...
int ngbe_set_vf_mac(struct ngbe_adapter *adapter, u16 vf, unsigned char *mac_addr)
{
	s32 retval = 0;
...
static int ngbe_negotiate_vf_api(struct ngbe_adapter *adapter, u32 *msgbuf, u32 vf)
{
	int api = msgbuf[1];
...
static int ngbe_get_vf_queues(struct ngbe_adapter *adapter, u32 *msgbuf, u32 vf)
{
...
static int ngbe_get_vf_link_status(struct ngbe_adapter *adapter, u32 *msgbuf,
				   u32 vf)
{
	/* verify the PF is supporting the correct APIs */
	switch (adapter->vfinfo[vf].vf_api) {
...
static int ngbe_set_vf_macvlan(struct ngbe_adapter *adapter, u16 vf, int index,
	...
	if (index <= 1) {
		list_for_each(pos, &adapter->vf_mvs.l) {
			...
			if (entry->vf == vf) {
				...
				ngbe_del_mac_filter(adapter, entry->vf_macvlan,
						    vf);
			}
		}
	}
...
static inline void ngbe_write_qde(struct ngbe_adapter *adapter, u32 vf, u32 qde)
{
	struct ngbe_hw *hw = &adapter->hw;
	u32 q_per_pool = 1;
	u32 reg = 0;
...
static int ngbe_vf_reset_msg(struct ngbe_adapter *adapter, u16 vf)
{
	...
	if (!is_zero_ether_addr(vf_mac)) {
		...
		dev_warn(pci_dev_to_dev(adapter->pdev),
			 "VF %d no MAC address assigned, you may have to assign one manually\n",
			 vf);
	}
...
static int ngbe_set_vf_mac_addr(struct ngbe_adapter *adapter, u32 *msgbuf,
				u16 vf)
{
...
static int ngbe_set_vf_vlan_msg(struct ngbe_adapter *adapter, u32 *msgbuf,
				u16 vf)
{
	struct ngbe_hw *hw = &adapter->hw;
	int add = (msgbuf[0] & NGBE_VT_MSGINFO_MASK) >> NGBE_VT_MSGINFO_SHIFT;
	int vid = (msgbuf[1] & NGBE_PSR_VLAN_SWC_VLANID_MASK);
	int vlan_offload = (msgbuf[0] & NGBE_VT_MSGINFO_MASK) >>
			   NGBE_VT_MSGINFO_VLAN_OFFLOAD_SHIFT;
	int err;
...
static int ngbe_set_vf_macvlan_msg(struct ngbe_adapter *adapter, u32 *msgbuf,
				   u16 vf)
{
	u8 *new_mac = ((u8 *)(&msgbuf[1]));
...
static int ngbe_update_vf_xcast_mode(struct ngbe_adapter *adapter, u32 *msgbuf,
				     u32 vf)
{
	struct ngbe_hw *hw = &adapter->hw;
	...
	case NGBEVF_XCAST_MODE_NONE:
		disable = NGBE_PSR_VM_L2CTL_BAM | NGBE_PSR_VM_L2CTL_ROMPE |
			  NGBE_PSR_VM_L2CTL_MPE | NGBE_PSR_VM_L2CTL_UPE |
			  NGBE_PSR_VM_L2CTL_VPE;
		enable = 0;
		break;
	case NGBEVF_XCAST_MODE_MULTI:
		disable = NGBE_PSR_VM_L2CTL_MPE | NGBE_PSR_VM_L2CTL_UPE |
			  NGBE_PSR_VM_L2CTL_VPE;
		enable = NGBE_PSR_VM_L2CTL_BAM | NGBE_PSR_VM_L2CTL_ROMPE;
		break;
	case NGBEVF_XCAST_MODE_ALLMULTI:
		disable = NGBE_PSR_VM_L2CTL_UPE | NGBE_PSR_VM_L2CTL_VPE;
		enable = NGBE_PSR_VM_L2CTL_BAM | NGBE_PSR_VM_L2CTL_ROMPE |
...
static int ngbe_get_vf_link_state(struct ngbe_adapter *adapter, u32 *msgbuf,
				  u32 vf)
{
...
static int ngbe_get_fw_version(struct ngbe_adapter *adapter, u32 *msgbuf,
			       u32 vf)
{
...
static int ngbe_rcv_msg_from_vf(struct ngbe_adapter *adapter, u16 vf)
{
	...
	case NGBE_VF_SET_LPE:
		if (msgbuf[1] > NGBE_MAX_JUMBO_FRAME_SIZE) {
			e_err(drv,
			      "VF max_frame %d exceed MAX_JUMBO_FRAME_SIZE\n",
			      msgbuf[1]);
			return -EINVAL;
		}
...
void ngbe_ping_all_vfs_with_link_status(struct ngbe_adapter *adapter,
					bool link_up)
{
	struct ngbe_hw *hw = &adapter->hw;
	u32 msgbuf[2];
	...
	if (adapter->need_delay) {
		adapter->need_delay = false;
		msleep(100);
	}
...
static void ngbe_ping_vf_with_link_status(struct ngbe_adapter *adapter,
					  bool link_up, u16 vf)
{
	struct ngbe_hw *hw = &adapter->hw;
	u32 msgbuf[2] = { 0, 0 };
	u32 link_speed = adapter->speed;
	...
	//	msgbuf[1] |= NGBE_PF_NOFITY_VF_NET_NOT_RUNNING;
	if (adapter->vfinfo[vf].clear_to_send)
		msgbuf[0] |= NGBE_VT_MSGTYPE_CTS;
	ngbe_write_mbx(hw, msgbuf, 2, vf);
}
	...
	if (is_valid_ether_addr(mac)) {
		...
		if (retval >= 0) {
			...
			if (test_bit(__NGBE_DOWN, &adapter->state)) {
				dev_warn(pci_dev_to_dev(adapter->pdev),
					 "The VF MAC address  set, but the PF device not up.\n");
				dev_warn(pci_dev_to_dev(adapter->pdev),
					 "up the PF device before use the VF device.\n");
			}
		} else {
			dev_warn(pci_dev_to_dev(adapter->pdev),
				 "The VF MAC not set due to invalid or duplicate MAC address.\n");
		}
		...
		retval = ngbe_del_mac_filter(adapter, vf_mac_addr, vf);
		if (retval >= 0) {
...
static int ngbe_enable_port_vlan(struct ngbe_adapter *adapter, int vf, u16 vlan,
				 u8 qos, __be16 vlan_proto)
{
	...
	if (err)
		goto out;
	ngbe_set_vmvir(adapter, vlan, qos, vf, vlan_proto);
...
static int ngbe_disable_port_vlan(struct ngbe_adapter *adapter, int vf)
{
	...
	err = ngbe_set_vf_vlan(adapter, false, adapter->vfinfo[vf].pf_vlan, vf);
	ngbe_clear_vmvir(adapter, vf);
...
int ngbe_ndo_set_vf_vlan(struct net_device *netdev, int vf, u16 vlan, u8 qos, __be16 vlan_proto)
{
	int err = 0;
	struct ngbe_adapter *adapter = netdev_priv(netdev);
	...
	if (vlan || qos) {
		...
		if (err)
			goto out;
		err = ngbe_enable_port_vlan(adapter, vf, vlan, qos, vlan_proto);
	} else {
		err = ngbe_disable_port_vlan(adapter, vf);
	}
out:
...
int ngbe_ndo_set_vf_spoofchk(struct net_device *netdev, int vf, bool setting)
{
	...
	if (vf < 32) {
		regval = (setting << vf);
		wr32m(hw, NGBE_TDM_MAC_AS_L, regval | (1 << vf), regval);

		if (adapter->vfinfo[vf].vlan_count) {
			wr32m(hw, NGBE_TDM_VLAN_AS_L, regval | (1 << vf),
			      regval);
		}
	}
...
static void ngbe_set_vf_rx_tx(struct ngbe_adapter *adapter, int vf)
{
	...
	if (adapter->vfinfo[vf].link_state == IFLA_VF_LINK_STATE_ENABLE &&
	    !(rd32(hw, NGBE_MAC_TX_CFG) & NGBE_MAC_TX_CFG_TE)) {
		wr32m(hw, NGBE_MAC_TX_CFG, NGBE_MAC_TX_CFG_TE,
		      NGBE_MAC_TX_CFG_TE);
		NGBE_WRITE_FLUSH(hw);
		wr32m(hw, NGBE_MAC_TX_CFG, NGBE_MAC_TX_CFG_TE,
		      NGBE_MAC_TX_CFG_TE);
...
void ngbe_set_vf_link_state(struct ngbe_adapter *adapter, int vf, int state)
{
	...
	case NGBE_VF_LINK_STATE_AUTO:
		...
		break;
	case NGBE_VF_LINK_STATE_ENABLE:
		adapter->vfinfo[vf].link_enable = true;
		link_up = true;
		break;
	case NGBE_VF_LINK_STATE_DISABLE:
		adapter->vfinfo[vf].link_enable = false;
		link_up = false;
		break;
	}
	...
	ngbe_ping_vf_with_link_status(adapter, link_up, vf);
...
int ngbe_ndo_set_vf_link_state(struct net_device *netdev, int vf, int state)
{
	...
	case IFLA_VF_LINK_STATE_ENABLE:
		...
		ngbe_set_vf_link_state(adapter, vf, NGBE_VF_LINK_STATE_ENABLE);
		break;
	case IFLA_VF_LINK_STATE_DISABLE:
		dev_info(pci_dev_to_dev(adapter->pdev),
			 "NDO set VF %d link state disable\n", vf);
		ngbe_set_vf_link_state(adapter, vf, NGBE_VF_LINK_STATE_DISABLE);
		break;
	case IFLA_VF_LINK_STATE_AUTO:
		dev_info(pci_dev_to_dev(adapter->pdev),
			 "NDO set VF %d link state auto\n", vf);
		ngbe_set_vf_link_state(adapter, vf, NGBE_VF_LINK_STATE_AUTO);
		break;
	default:
		dev_err(pci_dev_to_dev(adapter->pdev),
...
static int ngbe_trans_vf_link_state(int state)
{
	switch (state) {
	case NGBE_VF_LINK_STATE_ENABLE:
...
int ngbe_ndo_get_vf_config(struct net_device *netdev, int vf,
			   struct ifla_vf_info *ivi)
{
	...
	ivi->linkstate =
		ngbe_trans_vf_link_state(adapter->vfinfo[vf].link_state);
