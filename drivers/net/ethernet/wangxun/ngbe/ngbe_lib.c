...
{
	int queues = min_t(int, MAX_XDP_QUEUES, nr_cpu_ids);
...
static bool ngbe_set_rss_queues(struct ngbe_adapter *adapter)
{
	...
	if (adapter->xdp_prog)
		adapter->num_xdp_queues = ngbe_xdp_queues(adapter);
	return true;
}
 ...
 * can be allocated if they're turned on, while smaller combinations serve as
			       ...
			       unsigned int rxr_count, unsigned int rxr_idx)
{
	...
	while (txr_count) {
		...
		if (adapter->num_vmdqs > 1)
			ring->queue_index = txr_idx % adapter->queues_per_pool;
		else
	...
	while (rxr_count) {
		...
		if (adapter->num_vmdqs > 1)
			ring->queue_index = rxr_idx % adapter->queues_per_pool;
		else
...
static int ngbe_alloc_q_vectors(struct ngbe_adapter *adapter)
{
	...
	if (q_vectors >= (rxr_remaining + txr_remaining + xdp_remaining)) {
		for (; rxr_remaining; v_idx++) {
			err = ngbe_alloc_q_vector(adapter, q_vectors, v_idx, 0,
						  0, 0, 0, 1, rxr_idx);
			if (err)
				goto err_out;
	...
	for (; v_idx < q_vectors; v_idx++) {
		...
		err = ngbe_alloc_q_vector(adapter, q_vectors, v_idx, tqpv,
