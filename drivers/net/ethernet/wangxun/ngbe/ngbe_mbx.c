...
static int ngbe_poll_for_msg(struct ngbe_hw *hw, u16 mbx_id)
{
	...
	if (countdown == 0)
		ERROR_REPORT2(NGBE_ERROR_POLLING,
			      "Polling for VF%d mailbox message timedout",
			      mbx_id);
 ...
 *  returns SUCCESS if it successfully copied message into the buffer
 **/
static int ngbe_write_mbx_vf(struct ngbe_hw *hw, u32 *msg, u16 size, u16 mbx_id)
{
	int err;
	u16 i;
 ...
 *  returns SUCCESS if it successfully read message from buffer
 **/
static int ngbe_read_mbx_vf(struct ngbe_hw *hw, u32 *msg, u16 size, u16 mbx_id)
{
	int err = 0;
	u16 i;
...
/**
 * ngbe_release_mbx_lock_pf - release mailbox lock
 * @hw: pointer to the HW structure
 * @vf_id: the VF index
 **/
static void ngbe_release_mbx_lock_pf(struct ngbe_hw *hw, u16 vf_id)
{
	hw_dbg(hw, "ngbe_release_mbx_lock_pf");

	/* Return ownership of the buffer */
	wr32(hw, NGBE_PXMAILBOX(vf_id), 0x0);
}
 ...
 *  returns SUCCESS if it successfully copied message into the buffer
 **/
static int ngbe_write_mbx_pf(struct ngbe_hw *hw, u32 *msg, u16 size, u16 vf)
{
	int err;
	u16 i;
...
out_no_write:
	ngbe_release_mbx_lock_pf(hw, vf);
	return err;
}
 ...
 **/
static int ngbe_read_mbx_pf(struct ngbe_hw *hw, u32 *msg, u16 size, u16 vf)
{
	int err;
	u16 i;
