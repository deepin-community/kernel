		   ...
		   struct sk_buff *skb)
{
	...
	WARN_ON_ONCE(skb_headlen(skb) < FCOE_HEADER_LEN);
	...
	case cpu_to_le32(TXGBE_RXD_STAT_FCSTAT_DDP):
		/* update length of DDPed data */
		ddp->len = le32_to_cpu(rx_desc->wb.lower.hi_dword.rss);
		rc = 0;
		break;
	/* unmap the sg list when FCPRSP is received */
	case cpu_to_le32(TXGBE_RXD_STAT_FCSTAT_FCPRSP):
	...
	case cpu_to_le32(TXGBE_RXD_STAT_FCSTAT_NODDP):
		/* update length of DDPed data */
		ddp->len = le32_to_cpu(rx_desc->wb.lower.hi_dword.rss);
		if (ddp->len)
			rc = ddp->len;
	...
	case cpu_to_le32(TXGBE_RXD_STAT_FCSTAT_NOMTCH):
	default:
		break;
	}
