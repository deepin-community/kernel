 ...
 **/
ssize_t
simple_read_from_pcibar(struct ngbevf_adapter *adapter, int res,
...
ssize_t
simple_read_from_flash(struct ngbevf_adapter *adapter,
		void __user *buf, size_t size, loff_t *ppos)
{
...
ssize_t
simple_write_to_flash(struct ngbevf_adapter *adapter,
...
{
	struct ngbevf_adapter *adapter = filp->private_data;
	...
	case NGBE_FUNC_DUMP_BAR: {
		...
		return simple_read_from_pcibar(adapter, bar, buffer, size, ppos);
		break;
	}
	...
	case NGBE_FUNC_DUMP_RDESC: {
		...
		u32 r_idx = NGBE_DATA_ARGS(ngbe_data_mode);

		if (r_idx >= adapter->num_rx_queues)
			return 0;
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!strncmp(pc, "dump", 4)) {
		...
		if (!strncmp(pc, "adapter", 3)) {
			pc += 7;
			if (3 != sscanf(pc, "%d=%x:%x", &offset, &value, &mask)) {
				e_dev_info("offset=value:mask\n");
				goto out;
			}
			*(u32*)((void*)adapter + offset) |= (value & mask);
		} else if (!strncmp(pc, "hardware", 5)) {
			pc += 8;
			if (3 != sscanf(pc, "%d=%x:%x", &offset, &value, &mask)) {
				e_dev_info("offset=value:mask\n");
				goto out;
			}
			*(u32*)((void*)hw + offset) |= (value & mask);
		}
	} else if (!strncmp(ngbe_dbg_reg_ops_buf, "write", 5)) {
		u32 reg, value;
		int cnt;
	...
	} else if (!strncmp(ngbe_dbg_reg_ops_buf, "read", 4)) {
		u32 reg, value;
		int cnt;
 ...
 **/
static void
ngbe_regdump(struct ngbevf_hw *hw, struct ngbe_reg_info *reg_info)
{
	int i, j, n = 0;
	u32 buffer[32*7];
...
rx_ring_summary:
	...
	for (n = 0; n < adapter->num_rx_queues; n++) {
		...
		for (i = 0; i < rx_ring->count; i++) {
			...
			staterr = le32_to_cpu(rx_desc->wb.upper.status);
			if (staterr & NGBE_RXD_STAT_DD) {
				...
				pr_info("R  [0x%03X]     %016llX "
					...
					le64_to_cpu(u0->a),
					le64_to_cpu(u0->b),
					(u64)rx_buffer_info->dma_addr,
					rx_buffer_info->skb);

				if (netif_msg_pktdata(adapter) &&
				    rx_buffer_info->dma_addr) {
					print_hex_dump(KERN_INFO, "",
					   DUMP_PREFIX_ADDRESS, 16, 1,
