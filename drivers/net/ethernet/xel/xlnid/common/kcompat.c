...
_kc_pci_request_regions(struct pci_dev *dev, char *res_name)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (pci_resource_flags(dev, i) & IORESOURCE_IO) {
			if (!request_region(pci_resource_start(dev, i), pci_resource_len(dev, i),
								res_name)) {
			...
			if (!request_mem_region(pci_resource_start(dev, i), pci_resource_len(dev, i),
									res_name)) {
				pci_release_regions(dev);
				return -EBUSY;
			}
...
int xlnid_copy_dcb_cfg(struct xlnid_adapter __always_unused *adapter,
