...
ifeq (,${SCRIPT_PATH})
  ...
  PRIV_KEY_EXISTS := $(or $(and $(wildcard xel-linux-key.key),1),)
  PUB_KEY_EXISTS := $(or $(and $(wildcard xel-linux-key.crt),1),)
ifneq ($(and $(SIGN_FILE_EXISTS),$(PRIV_KEY_EXISTS),$(PUB_KEY_EXISTS)),)
  ...
  sign_driver = $(shell ${SCRIPT_PATH}/sign-file sha256 xel-linux-key.key \
                        xel-linux-key.crt ${DRIVER}.ko)
else
...
#$(if $(shell \
    ...
    #KSRC=${KSRC} OUT=${src}/../include/kcompat_generated_defs.h CONFIG_FILE=${CONFIG_FILE} \
...
export INSTALL_AUX_DIR ?= updates/drivers/net/ethernet/xel/auxiliary
...
define auxiliary_post_install
	install -D -m 644 Module.symvers ${INSTALL_MOD_PATH}/lib/modules/${KVER}/extern-symvers/xel_auxiliary.symvers
	install -d ${INSTALL_MOD_PATH}/lib/modules/${KVER}/${INSTALL_AUX_DIR}
	mv -f ${INSTALL_MOD_PATH}/lib/modules/${KVER}/${INSTALL_MOD_DIR}/xel_auxiliary.ko* \
...
define auxiliary_post_uninstall
	rm -f ${INSTALL_MOD_PATH}/lib/modules/${KVER}/extern-symvers/xel_auxiliary.symvers
	rm -f ${INSTALL_MOD_PATH}/lib/modules/${KVER}/${INSTALL_AUX_DIR}/xel_auxiliary.ko*
