...
function gen-netdevice() {
	...
	gen HAVE_NDO_SIOCDEVPRIVATE if fun ndo_siocdevprivate in "$ndh"
...
function gen-other() {
	...
	gen HAVE_ETH_GET_HEADLEN_3_PARAMS if fun eth_get_headlen matches data in include/linux/etherdevice.h
	...
	gen HAVE_PTP_FILTER if macro PTP_FILTER in include/linux/ptp_classify.h
