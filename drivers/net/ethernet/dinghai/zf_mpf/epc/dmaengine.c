...
static void zxdh_dmaengine_debug_unregister(struct dma_device *dma_dev)
{
...
static int zxdh_dmaengine_summary_show(struct seq_file *s, void *data)
{
	...
	list_for_each_entry(dma_dev, &dma_device_list, global_node) {
		...
		if (dma_dev->dbg_summary_show)
			...
			zxdh_dmaengine_dbg_summary_show(s, dma_dev);
...
static void __init zxdh_dmaengine_debugfs_init(void)
{
 ...
 */
static struct dma_chan *zxdh_dev_to_dma_chan(struct device *dev)
{
...
static void zxdh_chan_dev_release(struct device *dev)
{
	struct dma_chan_dev *chan_dev;
...
static int __init zxdh_dma_channel_table_init(void)
{
	...
	if (err) {
		pr_err("dmaengine zxdh_dma_channel_table_init failure: %d\n", err);
		for_each_dma_cap_mask(cap, dma_cap_mask_all)
 ...
 * Must be called under dma_list_mutex.
 */
static struct dma_chan *zxdh_min_chan(enum dma_transaction_type cap, int cpu)
{
...
/**
 * zxdh_dma_channel_rebalance - redistribute the available channels
 *
 ...
 * Must be called under dma_list_mutex.
 */
static void zxdh_dma_channel_rebalance(void)
{
	struct dma_chan *chan;
	struct dma_device *device;
	int cpu;
...
static struct module *zxdh_dma_chan_to_owner(struct dma_chan *chan)
{
	return chan->device->owner;
}

/**
 * zxdh_balance_ref_count - catch up the channel reference count
 * @chan:	channel to balance ->client_count versus dmaengine_ref_count
 *
 * Must be called under dma_list_mutex.
 */
static void zxdh_balance_ref_count(struct dma_chan *chan)
{
	struct module *owner = zxdh_dma_chan_to_owner(chan);
...
static void zxdh_dma_device_release(struct kref *ref)
{
...
static void zxdh_dma_device_put(struct dma_device *device)
{
	lockdep_assert_held(&dma_list_mutex);
	kref_put(&device->ref, zxdh_dma_device_release);
}

/**
 * zxdh_dma_chan_get - try to grab a DMA channel's parent driver module
...
err_out:
	zxdh_dma_device_put(chan->device);
module_put_out:
	module_put(owner);
	return ret;
}
...
/**
 * zxdh_dma_chan_put - drop a reference to a DMA channel's parent driver module
...
static void zxdh_dma_chan_put(struct dma_chan *chan)
{
	...
	zxdh_dma_device_put(chan->device);
	module_put(zxdh_dma_chan_to_owner(chan));
}

enum dma_status zxdh_dma_sync_wait(struct dma_chan *chan, dma_cookie_t cookie)
{
 ...
 */
struct dma_chan *zxdh_dma_find_channel(enum dma_transaction_type tx_type)
{
 ...
 */
void zxdh_dma_issue_pending_all(void)
{
	struct dma_device *device;
	struct dma_chan *chan;
					  ...
					  dma_filter_fn fn, void *fn_param)
{
	...
	if (mask && !zxdh_dma_device_satisfies_mask(dev, mask)) {
...
/**
 * zxdh_dma_get_slave_channel - try to get specific channel exclusively
 * @chan:	target channel
 */
struct dma_chan *zxdh_dma_get_slave_channel(struct dma_chan *chan)
{
	...
	/* lock against __zxdh__zxdh_dma_request_channel */
	mutex_lock(&dma_list_mutex);
...
struct dma_chan *zxdh_dma_get_any_slave_channel(struct dma_device *device)
{
	...
	chan = zxdh_find_candidate(device, &mask, NULL, NULL);
				       ...
				       struct device_node *np)
{
	...
	list_for_each_entry_safe(device, _d, &dma_device_list, global_node) {
		...
		chan = zxdh_find_candidate(device, mask, fn, fn_param);
		if (!IS_ERR(chan))
			break;
...
/**
 * zxdh_dma_request_chan - try to allocate an exclusive slave channel
 * @dev:	pointer to client device structure
 * @name:	slave channel name
 *
 * Returns pointer to appropriate DMA channel on success or an error pointer.
 */
struct dma_chan *zxdh_dma_request_chan(struct device *dev, const char *name)
{
	...
	list_for_each_entry_safe(d, _d, &dma_device_list, global_node) {
		dma_cap_mask_t mask;
		const struct dma_slave_map *map = zxdh_dma_filter_match(d, name, dev);
		...
		chan = zxdh_find_candidate(d, &mask, d->filter.fn, map->param);
		if (!IS_ERR(chan))
			break;
	}
	mutex_unlock(&dma_list_mutex);
 ...
 * Returns pointer to appropriate DMA channel on success or an error pointer.
 */
struct dma_chan *zxdh_dma_request_chan_by_mask(const dma_cap_mask_t *mask)
{
	struct dma_chan *chan;
	...
	chan = __zxdh__zxdh_dma_request_channel(mask, NULL, NULL, NULL);
	if (!chan) {
		mutex_lock(&dma_list_mutex);
...
static int zxdh_get_dma_id(struct dma_device *device)
{
				      ...
				      struct dma_chan *chan)
{
	...
	rc = __zxdh_dma_async_device_channel_register(device, chan);
	if (rc < 0)
		return rc;
...
int zxdh_dma_async_device_register(struct dma_device *device)
{
	...
	DH_LOG_INFO(MODULE_MPF, "%s enter\n", __func__);
	...
	zxdh_dmaengine_debug_register(device);
 ...
 */
void zxdh_dma_async_device_unregister(struct dma_device *device)
{
	struct dma_chan *chan, *n;

	zxdh_dmaengine_debug_unregister(device);
...
static void zxdh_dmam_device_release(struct device *dev, void *res)
{
	...
	zxdh_dma_async_device_unregister(device);
}

/**
 * zxdh_dmaenginem_async_device_register - registers DMA devices found
 * @device:	pointer to &struct dma_device
 *
 * The operation is managed and will be undone on driver detach.
 */
int zxdh_dmaenginem_async_device_register(struct dma_device *device)
{
	void *p;
	int ret;

	p = devres_alloc(zxdh_dmam_device_release, sizeof(void *), GFP_KERNEL);
	if (!p)
		return -ENOMEM;
...
void zxdh_dmaengine_unmap_put(struct dmaengine_unmap_data *unmap)
{
	if (unmap)
...
static void zxdh_dmaengine_destroy_unmap_pool(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(unmap_pool); i++) {
		struct zxdh_dmaengine_unmap_pool *p = &unmap_pool[i];
...
static int __init zxdh_dmaengine_init_unmap_pool(void)
{
	int i;
	...
	zxdh_dmaengine_destroy_unmap_pool();
	return -ENOMEM;
}
...
zxdh_dmaengine_get_unmap_data(struct device *dev, int nr, gfp_t flags)
{
	...
	unmap = mempool_alloc(__zxdh_get_unmap_pool(nr)->pool, flags);
	if (!unmap)
		return NULL;
				   ...
				   void *data, size_t len)
{
	...
	ret = zxdh_desc_check_and_set_metadata_mode(desc, DESC_METADATA_CLIENT);
	if (ret)
		return ret;
				      ...
				      size_t *payload_len, size_t *max_len)
{
	...
	ret = zxdh_desc_check_and_set_metadata_mode(desc, DESC_METADATA_ENGINE);
	if (ret)
		return ERR_PTR(ret);
 ...
 */
void zxdh_dma_run_dependencies(struct dma_async_tx_descriptor *tx)
{
...
static int __init zxdh_dma_bus_init(void)
{
	...
	if (!err)
		zxdh_dmaengine_debugfs_init();

	return err;
}
arch_initcall(zxdh_dma_bus_init);
