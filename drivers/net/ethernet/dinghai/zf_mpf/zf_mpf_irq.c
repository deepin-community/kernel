...
static int32_t irq_pools_init(struct dh_core_dev *dev)
{
    ...
    mpf_irq_table->mpf_async_pool = (struct dh_irq_pool *)irq_pool_alloc(dev, irq_range.start, irq_range.size, "zxdh_mpf_msg",
    ...
    mpf_irq_table->mpf_comp_pool = (struct dh_irq_pool *)irq_pool_alloc(dev, irq_range.start,
    ...
    mpf_irq_table->mpf_gdma_pool = (struct dh_irq_pool *)irq_pool_alloc(dev, irq_range.start, irq_range.size, "zxdh_mpf_gdma",
...
int32_t dh_mpf_irq_table_create(struct dh_core_dev *dev)
{
    ...
    if (total_vec < 0)
    {
        DH_LOG_ERR_DEV(MODULE_MPF, dev, "pci_alloc_irq_vectors failed: %d\n", total_vec);
        return total_vec;
    }
...
struct dh_irq *dh_mpf_async_irq_request(struct dh_core_dev *dev)
{
    ...
    if (IS_ERR_OR_NULL(irq))
        DH_LOG_ERR_DEV(MODULE_MPF, dev, "irq=0x%llx\r\n", (unsigned long long)irq);
    DH_LOG_DEBUG_DEV(MODULE_MPF, dev, "end\r\n");
    return irq;
}
