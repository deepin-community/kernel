        ...
        uint16_t id, struct net_device *netdev, struct zxdh_lag_attrs *attr)
{
    ...
    {
        LAG_LOG_ERR_DEV(dh_dev, "%s: index[%hu] out of range. \n", netdev_name(netdev), id);
        return;
    }
...
bool lag_check_aux_netdev_ready(struct zxdh_lag *ldev)
{
    ...
    {
        if (ldev->lagfunc[ix].netdev != NULL)
        {
            lag_netdev_cnt++;
        }
    }
...
static int32_t zxdh_disable_lag(struct zxdh_lag *ldev)
{
    ...
    for (i = 0; i < ZXDH_MAX_PORTS; i++)
    {
        ...
        dpp_uplink_phy_attr_set(&pf_info, phy_port, UPLINK_PHY_PORT_BOND_LINK_UP, LAG_FLAGS_DISABLE);
    }
...
static int32_t zxdh_lag_active_backup(struct zxdh_lag *ldev)
{
    ...
    for (i = ZXDH_MAX_PORTS - 1; i >= 0; i--)
    {
        ...
        if (porten && (ldev->slaves & (1 << i)))
        {
            ...
            LAG_LOG_INFO("lag_id %u: [backup] select acitve phyport %hu pannel port %hu \n",
                          ldev->lag_id, active_phy_port, func->attrs.pannel_id);
        }

        LAG_LOG_INFO("lag_id %u: [backup] phyport %hhu pannel port %hhu tx enable %d link up %d\n",
    ...
    if (active_phy_port != ZXDH_ACTIVE_PHY_PORT_NA)
    {
        ...
        dpp_uplink_phy_attr_set(&pf_info, active_phy_port, UPLINK_PHY_PORT_BOND_LINK_UP, LAG_FLAGS_ENABLE);
    }
...
static int32_t zxdh_lag_hash(struct zxdh_lag *ldev)
{
    int i;
    bool porten = false;
    uint8_t phy_port = 0;
    uint8_t flags = 0;
    uint8_t np_hash_type = 0;
    int32_t err = 0;
    struct lag_func *func;
    struct lag_tracker *tracker = &ldev->tracker;
    DPP_PF_INFO_T pf_info = {0};
...
#else
    /* HASH模式直接写死Layer3+4 */
    dpp_lag_group_hash_factor_get(&pf_info, ldev->lag_id, &np_hash_type);
    if (np_hash_type == ZXDH_NP_HASH_TYPE_DEFAULT)
    {
        np_hash_type = ZXDH_NP_HASH_TYPE_L34;
    }
#endif

    dpp_lag_group_hash_factor_set(&pf_info, ldev->lag_id, np_hash_type);
    ...
    for (i = 0; i < ZXDH_MAX_PORTS; i++)
    {
        ...
        if (porten && (ldev->slaves & (1 << i)))
        {
            ...
            dpp_uplink_phy_attr_set(&pf_info, phy_port, UPLINK_PHY_PORT_BOND_LINK_UP, LAG_FLAGS_ENABLE);
        ...
        LAG_LOG_INFO("lag_id %u: [hash] %s, vport %hu, pannel port %hu, phy port %hu, slave mask 0x%x, tx enable %d, link up %d \n",
                      ldev->lag_id, (flags == LAG_FLAGS_ENABLE) ? "enable" : "disable", pf_info.vport,
...
static struct zxdh_lag *lag_dev_alloc(struct dh_core_dev *dev)
{
    ...
    if (!ldev)
    {
        LAG_LOG_ERR_DEV(dev, "Failed to alloc ldev\n");
        return NULL;
    }
    ...
    {
	LAG_LOG_ERR_DEV(dev, "Failed to snprintf ldev name!\n");
	kfree(ldev);
        return NULL;
    }
...
#endif
    {
        ...
        LAG_LOG_ERR_DEV(dev, "Failed to register LAG netdev notifier\n");
    }
...
static int lag_dev_create(struct dh_core_dev *dh_dev)
{
    ...
    if (!ldev)
    {
        ...
        if (!ldev)
        {
            LAG_LOG_ERR_DEV(dh_dev, "Failed to alloc lag dev.\n");
            return 0;
        }
    }
    else
...
int32_t lag_dpp_pannel_init(struct dh_core_dev *dh_dev, struct zxdh_lag_attrs *attr)
{
    ...
    dpp_uplink_phy_attr_set(&pf_info, phy_port, UPLINK_PHY_PORT_PTP_PORT_VFID, ovs_vfid);
    dpp_uplink_phy_attr_set(&pf_info, phy_port, UPLINK_PHY_PORT_PTP_TC_ENABLE, 0);
    ...
    if (ret != 0)
    {
        LOG_ERR_DEV(dh_dev, "dpp_vport_attr_set SRIOV_VPORT_HW_BOND_EN_OFF LAG_FLAGS_ENABLE failed\n");
        return ret;
    }

    dpp_lag_group_create(&pf_info, ldev->lag_id);

    LAG_LOG_INFO_DEV(dh_dev, "vport 0x%hx, pannel id %hu, phy port %hu, rx qid %hu tx qid %hu bond vfid %d, ovs vfid %d, lag_id %d\n",
                pf_info.vport, pannel_id, phy_port, attr->qid[0], attr->qid[1], bond_vfid, ovs_vfid, ldev->lag_id);
...
int32_t lag_netdev_input_check(struct dh_core_dev *dh_dev, struct zxdh_lag_attrs *attr)
{
    ...
    {
        LAG_LOG_ERR_DEV(dh_dev, "dh_core_dev type PF needs to be used\n");
        return -EINVAL;
    }
    ...
    if (!ldev)
    {
        LAG_LOG_ERR_DEV(dh_dev, "ldev can not be null\n");
        return -EINVAL;
    }

    if (attr->pannel_id >= ZXDH_MAX_PORTS)
    {
        LAG_LOG_ERR_DEV(dh_dev, "pannel id %hu out of range.\n", attr->pannel_id);
        return -EINVAL;
    }
