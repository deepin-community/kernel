...
{
	if (lock_type == DH_XA_LOCK_IRQ)
		xas_lock_irq(xas);
	else if (lock_type == DH_XA_LOCK_BH)
		xas_lock_bh(xas);
	else
		xas_lock(xas);
}
...
#ifdef CGS_V5_693
static struct kmem_cache *xa_node_cachep;  /* Separate cache for xa_node */
#endif
#else
...
static void xa_node_rcu_free(struct rcu_head *head)
{
	...
	memset(node->marks, 0, sizeof(node->marks));
...
#else
	/* CGS_V5_693 has no gfpflags_allow_blocking; allocate with lock held */
...
static void *xas_alloc(struct xa_state *xas, unsigned int shift)
{
	...
	node->array = xas->xa;
	/* Initialize slots to NULL to avoid garbage values */
	memset(node->slots, 0, sizeof(node->slots));
...
#ifdef CGS_V5_693
	/* Create separate cache for xa_node which has different structure */
	xa_node_cachep = kmem_cache_create("xa_node",
			sizeof(struct xa_node), 0,
			SLAB_PANIC | SLAB_RECLAIM_ACCOUNT,
			NULL);
#endif
	radix_tree_init_maxnodes();//5.10 不存在
}
#endif