...
static uint16_t label_read(spl_addr_t label_lock_addr, uint32_t lock_id)
{
    return readw((spl_addr_t)(label_lock_addr + lock_id * 2));
}
...
int32_t zxdh_spinlock_lock(uint32_t virt_lock_id, uint64_t lock_virt_addr, uint64_t label_addr, uint16_t masterid)
{
    ...
    {
        label = label_read((spl_addr_t)label_addr, virt_lock_id);
        BAR_LOG_WARN("Bar channel is busy, label: 0x%x, spl: 0x%x lock_id:%u.\n", label, spl_val, virt_lock_id);
