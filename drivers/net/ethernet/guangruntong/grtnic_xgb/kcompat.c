...
void _kc_free_netdev(struct net_device *netdev)
{
	...
	if (netdev->reg_state == NETREG_UNINITIALIZED) {
		...
		if (WARN_ON_ONCE(netdev->reg_state != NETREG_UNREGISTERED))
			return;
...
void __kc_netdev_rss_key_fill(void *buffer, size_t len)
{
	...
	if (WARN_ON_ONCE(len > NETDEV_RSS_KEY_LEN))
		return;
...
void __kc_netdev_rss_key_fill(void *buffer, size_t len)
{
	if (WARN_ON_ONCE(len > sizeof(__kc_netdev_rss_key)))
		return;
 ...
 * check for fields that are explicitly wanted).
 *
 * Current allowlist:
 *
