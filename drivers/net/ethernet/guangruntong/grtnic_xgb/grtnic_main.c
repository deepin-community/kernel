...
static void grtnic_update_tx_dca(struct grtnic_adapter *adapter,
	struct grtnic_ring *tx_ring, int cpu)
{
	u32 txctrl = 0;
...
static void grtnic_update_rx_dca(struct grtnic_adapter *adapter,
	struct grtnic_ring *rx_ring, int cpu)
{
	u32 rxctrl = 0;
...
//    }
	...
	err = request_irq(adapter->msix_entries[vector].vector, &grtnic_msix_other, 0,
		netdev->name, adapter);
	if (err) {
...
{
	if (WARN_ON_ONCE(!test_bit(__GRTNIC_SERVICE_SCHED, &adapter->state)))
		return;
...
{
	struct grtnic_adapter *adapter = timer_container_of(adapter, t, service_timer);
...
#if 0
//#if IS_ENABLED(CONFIG_DCA)
static int grtnic_notify_dca(struct notifier_block __always_unused *nb,
	unsigned long event, void __always_unused *p)
{
	int ret_val;
