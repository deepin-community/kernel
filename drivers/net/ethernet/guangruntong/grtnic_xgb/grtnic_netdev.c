...
static inline void grtnic_rx_hash(struct grtnic_ring *ring, union grtnic_rx_desc *rx_desc,
	struct sk_buff *skb)
{
	u16 rss_type;
...
static bool grtnic_alloc_mapped_skb(struct grtnic_ring *rx_ring,
...
static bool grtnic_alloc_mapped_page(struct grtnic_ring *rx_ring,
	struct grtnic_rx_buffer *buffer_info)
{
...
static inline bool grtnic_container_is_rx(struct grtnic_q_vector *q_vector,
	struct grtnic_ring_container *rc)
{
 ...
 **/
static void grtnic_update_itr(struct grtnic_q_vector *q_vector,
	struct grtnic_ring_container *ring_container)
{
...
adjust_for_speed:
	...
	if (q_vector->adapter->speed == 1) //10G
		itr += DIV_ROUND_UP(avg_wire_size, ITR_ADAPTIVE_MIN_INC * 256) *
			ITR_ADAPTIVE_MIN_INC;
	else //1G
		itr += DIV_ROUND_UP(avg_wire_size, ITR_ADAPTIVE_MIN_INC * 64) *
			ITR_ADAPTIVE_MIN_INC;
...
static void grtnic_process_skb_fields(struct grtnic_ring *rx_ring, union grtnic_rx_desc *rx_desc,
	struct sk_buff *skb)
{
...
static bool grtnic_is_non_eop(struct grtnic_ring *rx_ring, union grtnic_rx_desc *rx_desc,
	struct sk_buff *skb)
{
#ifdef CONFIG_DISABLE_PACKET_SPLIT
...
//		printk("rx len = %d\n", len);
		...
		if (!GRTNIC_CB(skb)->head) {
			...
			dma_unmap_single(rx_ring->dev, rx_buffer->dma, rx_ring->rx_buffer_len,
				DMA_FROM_DEVICE);
		}
		...
		dma_unmap_single(rx_ring->dev, GRTNIC_CB(skb)->dma, rx_ring->rx_buffer_len,
			DMA_FROM_DEVICE);
		GRTNIC_CB(skb)->dma = 0;
...
static void grtnic_add_rx_frag(struct grtnic_ring *rx_ring, struct grtnic_rx_buffer *rx_buffer,
...
static void grtnic_put_rx_buffer(struct grtnic_ring *rx_ring, struct grtnic_rx_buffer *rx_buffer,
	struct sk_buff *skb)
{
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
	DEFINE_DMA_ATTRS(attrs);
...
static bool grtnic_cleanup_headers(struct grtnic_ring *rx_ring, union grtnic_rx_desc *rx_desc,
	struct sk_buff *skb)
{
...
int grtnic_setup_tx_resources(struct grtnic_ring *tx_ring)
{
	...
	set_dev_node(dev, node);
	tx_ring->desc_wb = dma_alloc_coherent(dev, sizeof(struct grtnic_desc_wb),
		&tx_ring->desc_wb_dma, GFP_KERNEL);
	set_dev_node(dev, orig_node);
	if (!tx_ring->desc_wb)
		tx_ring->desc_wb = dma_alloc_coherent(dev, sizeof(struct grtnic_desc_wb),
			&tx_ring->desc_wb_dma, GFP_KERNEL);
	...
	if (!test_and_set_bit(__GRTNIC_TX_XPS_INIT_DONE, &ring->state)) {
		...
		if (q_vector)
			netif_set_xps_queue(adapter->netdev,
				get_cpu_mask(reg_idx % adapter->rss_queues), ring->queue_index);
	...
	while (i != rx_ring->next_to_alloc) {
		...
		if (rx_buffer->skb) {
			...
			if (grtnic_close_active_frag_list(skb))
				dma_unmap_single(rx_ring->dev, GRTNIC_CB(skb)->dma,
...
//		mc_addr[3],mc_addr[2],mc_addr[1],mc_addr[0]);
...
////////////////////////////////////////////////////////////////////////////
	...
	//crc error(<64)
	hwstats->rfc = grtnic_get_statistics_cnt(adapter, 0x218, hwstats->rfc);
	//crc error(>=64)
	hwstats->crcerrs = grtnic_get_statistics_cnt(adapter, 0x298, hwstats->crcerrs);
