...
static int firmware_is_old(struct grtnic_adapter *adapter);
...
//  GRTNIC_STAT("tx_timeout_count", tx_timeout_count),
	...
	GRTNIC_STAT("rx_vlan_packets", stats.rvlanc),
	GRTNIC_STAT("tx_vlan_packets", stats.tvlanc),
...
static const struct speed_duplex_info speed_duplex_priority[] = {
	{ BIT(5), SPEED_2500,  DUPLEX_FULL, "2500baseT_Full" },
	{ BIT(4), SPEED_1000,  DUPLEX_FULL, "1000baseT_Full" },
	{ BIT(3), SPEED_100,   DUPLEX_FULL, "100baseT_Full"  },
	{ BIT(2), SPEED_100,   DUPLEX_HALF, "100baseT_Half"  },
	{ BIT(1), SPEED_10,    DUPLEX_FULL, "10baseT_Full"   },
	{ BIT(0), SPEED_10,    DUPLEX_HALF, "10baseT_Half"   },
};

/* 根据本地和链路伙伴的通告能力,确定协商结果 */
static void grtnic_get_negotiated_speed_duplex(u32 local_adv, u32 lp_adv,
					       u32 *speed, u8 *duplex)
{
	u32 common_modes;
	int i;

	common_modes = local_adv & lp_adv;
	...
	/* 选择最高优先级的共同模式 */
	for (i = 0; i < ARRAY_SIZE(speed_duplex_priority); i++) {
		if (common_modes & speed_duplex_priority[i].bit_mask) {
			*speed = speed_duplex_priority[i].speed;
			*duplex = speed_duplex_priority[i].duplex;
			return;
		}
	}
...
static int grtnic_get_link_ksettings(struct net_device *netdev, struct ethtool_link_ksettings *cmd)
{
	...
	u16 phy_mii_reg0x05;
	u16 phy_mii_reg0x0A;
	u16 phy_mmd7_reg0x21;
	u32 lp_advertised = 0;
	u32 negotiated_speed;
	u8  negotiated_duplex;
	...
	if (adapter->type == 1) { //copper
		...
		if (!(adapter->autoneg_advertised & BIT(0)))
			__clear_bit(ETHTOOL_LINK_MODE_10baseT_Half_BIT,
				cmd->link_modes.advertising);
		if (!(adapter->autoneg_advertised & BIT(1)))
			__clear_bit(ETHTOOL_LINK_MODE_10baseT_Full_BIT,
				cmd->link_modes.advertising);
		if (!(adapter->autoneg_advertised & BIT(2)))
			__clear_bit(ETHTOOL_LINK_MODE_100baseT_Half_BIT,
				cmd->link_modes.advertising);
		if (!(adapter->autoneg_advertised & BIT(3)))
			__clear_bit(ETHTOOL_LINK_MODE_100baseT_Full_BIT,
				cmd->link_modes.advertising);
		if (!(adapter->autoneg_advertised & BIT(4)))
			__clear_bit(ETHTOOL_LINK_MODE_1000baseT_Full_BIT,
				cmd->link_modes.advertising);
		if (!(adapter->autoneg_advertised & BIT(5)))
			__clear_bit(ETHTOOL_LINK_MODE_2500baseT_Full_BIT,
				cmd->link_modes.advertising);

		//解析 link partner 能力
		grtnic_PhyRead(netdev, phy_addr, 0x05, &phy_mii_reg0x05);
		if (phy_mii_reg0x05 & PHY_LP_10Base_TX_Half) {
			ethtool_link_ksettings_add_link_mode(cmd, lp_advertising, 10baseT_Half);
			lp_advertised |= BIT(0);
		}
		if (phy_mii_reg0x05 & PHY_LP_10Base_TX_Full) {
			ethtool_link_ksettings_add_link_mode(cmd, lp_advertising, 10baseT_Full);
			lp_advertised |= BIT(1);
		}
		if (phy_mii_reg0x05 & PHY_LP_100Base_TX_Half) {
			ethtool_link_ksettings_add_link_mode(cmd, lp_advertising, 100baseT_Half);
			lp_advertised |= BIT(2);
		}
		if (phy_mii_reg0x05 & PHY_LP_100Base_TX_Full) {
			ethtool_link_ksettings_add_link_mode(cmd, lp_advertising, 100baseT_Full);
			lp_advertised |= BIT(3);
		}

		grtnic_PhyRead(netdev, phy_addr, 0x0A, &phy_mii_reg0x0A);
		if (phy_mii_reg0x0A & PHY_LP_1000Base_T_Full) {
			ethtool_link_ksettings_add_link_mode(cmd, lp_advertising, 1000baseT_Full);
			lp_advertised |= BIT(4);
		}

		grtnic_PhyWrite(adapter->netdev, adapter->hw.phy_addr, 0x1E, 0xA000);
		grtnic_PhyWrite(adapter->netdev, adapter->hw.phy_addr, 0x1F, 0x0);

		if (adapter->pdev->device == 0x0f42 || adapter->pdev->device == 0x0f41) {
			grtnic_PhyWrite(adapter->netdev, adapter->hw.phy_addr, 0x1E, 0xA000);
			grtnic_PhyWrite(adapter->netdev, adapter->hw.phy_addr, 0x1F, 0x0);

			grtnic_PhyWrite(netdev, phy_addr, 0x0D, 0x07);  //MMD7
			grtnic_PhyWrite(netdev, phy_addr, 0x0E, 0x21);  //MMD7-reg0x21
			grtnic_PhyWrite(netdev, phy_addr, 0x0D, 0x4007);
			grtnic_PhyRead(netdev, phy_addr, 0x0E, &phy_mmd7_reg0x21);
			if (phy_mmd7_reg0x21 & PHY_LP_2500Base_T_Full) {
		...
		if (phy_mii_reg0x05 & PHY_LP_PAUSE) {
			ethtool_link_ksettings_add_link_mode(cmd, lp_advertising, Pause);
			if (phy_mii_reg0x05 & PHY_LP_ASYM_PAUSE)
				ethtool_link_ksettings_add_link_mode(cmd, lp_advertising,
					Asym_Pause);
		}
		if (phy_mii_reg0x05 & PHY_LP_LPACK)
			ethtool_link_ksettings_add_link_mode(cmd, lp_advertising, Autoneg);

		grtnic_get_negotiated_speed_duplex(adapter->autoneg_advertised,
						   lp_advertised,
			&negotiated_speed,
			&negotiated_duplex);
		...
		if (adapter->pdev->device == 0x0f52 || adapter->pdev->device == 0x0f51) {
			ethtool_link_ksettings_add_link_mode(cmd, advertising, 2500baseX_Full);

			if (adapter->autoneg_advertised == 0x1)
				__clear_bit(ETHTOOL_LINK_MODE_2500baseX_Full_BIT,
					cmd->link_modes.advertising);
			else if (adapter->autoneg_advertised == 0x2)
				__clear_bit(ETHTOOL_LINK_MODE_1000baseX_Full_BIT,
					cmd->link_modes.advertising);
		}
	...
	if (netif_running(netdev)) {
		if (netif_carrier_ok(netdev)) {
			cmd->base.speed = (adapter->type == 0) ? fiber_speed :
			(port_speed == 0x03) ? SPEED_10000 :
			(port_speed == 0x02) ? SPEED_1000 :
			(port_speed == 0x01) ? SPEED_100 : SPEED_10;

			cmd->base.duplex = DUPLEX_FULL;

			if (adapter->type == 1) {
				cmd->base.speed = adapter->autoneg_enabled ? negotiated_speed :
					(adapter->link_speed == 0x02) ? SPEED_1000 :
					(adapter->link_speed == 0x01) ? SPEED_100  : SPEED_10;
				cmd->base.duplex = adapter->autoneg_enabled ? negotiated_duplex :
					adapter->link_duplex ? DUPLEX_FULL : DUPLEX_HALF;
			} else {
				if (adapter->pdev->device == 0x0f52 ||
						adapter->pdev->device == 0x0f51)
					cmd->base.speed = (port_speed == 0x2) ? SPEED_2500 :
						(port_speed == 0x1) ? SPEED_1000 : 0;
			}
		}
	}

	cmd->base.phy_address = hw->phy_addr;
	cmd->base.autoneg = adapter->autoneg_enabled ? AUTONEG_ENABLE : AUTONEG_DISABLE;
...
static int grtnic_set_link_ksettings(struct net_device *netdev,
	const struct ethtool_link_ksettings *cmd)
{
	struct grtnic_adapter *adapter = netdev_priv(netdev);
	u32 phy_addr = adapter->hw.phy_addr;
	u32 speed = cmd->base.speed;
	bool autoneg = cmd->base.autoneg;
	u16 temp;
	u32 advertised = 0;

	if (adapter->type == 1) { //copper
		bool adv_10_half = false;
		bool adv_10_full = false;
		bool adv_100_half = false;
		bool adv_100_full = false;
		bool adv_1000 = false;
		bool adv_2500 = false;

		if (!bitmap_empty(cmd->link_modes.advertising, __ETHTOOL_LINK_MODE_MASK_NBITS)) {
			...
			if (test_bit(ETHTOOL_LINK_MODE_2500baseT_Full_BIT,
				cmd->link_modes.advertising) ||
			    test_bit(ETHTOOL_LINK_MODE_2500baseX_Full_BIT,
			  cmd->link_modes.advertising))
				adv_2500 = true;
		}

		if (!autoneg && (adv_1000 | adv_2500))
			return -EINVAL;

		advertised |= adv_10_half ? BIT(0) : 0;
		advertised |= adv_10_full ? BIT(1) : 0;
		advertised |= adv_100_half ? BIT(2) : 0;
		advertised |= adv_100_full ? BIT(3) : 0;
		advertised |= adv_1000 ? BIT(4) : 0;
		advertised |= adv_2500 ? BIT(5) : 0;

		adapter->autoneg_advertised = advertised;

		grtnic_PhyWrite(adapter->netdev, adapter->hw.phy_addr, 0x1E, 0xA000);
		grtnic_PhyWrite(adapter->netdev, adapter->hw.phy_addr, 0x1F, 0x0);

		grtnic_PhyRead(netdev, phy_addr, 0x04, &temp);

		temp &= ~(PHY_10Base_TX_Half | PHY_10Base_TX_Full |
							PHY_100Base_TX_Half | PHY_100Base_TX_Full |
							PHY_NEXT_Page);

		if (adv_10_half)
			temp |= PHY_10Base_TX_Half;
		if (adv_10_full)
			temp |= PHY_10Base_TX_Full;
		if (adv_100_half)
			temp |= PHY_100Base_TX_Half;
		if (adv_100_full)
			temp |= PHY_100Base_TX_Full;
		if (adapter->pdev->device == 0x0f42 || adapter->pdev->device == 0x0f41) {
			if ((!adv_1000) && (!adv_2500))
				temp |= PHY_NEXT_Page;
		}

		grtnic_PhyWrite(netdev, phy_addr, 0x04, temp);

		grtnic_PhyRead(netdev, phy_addr, 0x09, &temp);

		temp &= ~(PHY_1000Base_T_Full | PHY_1000Base_T_Half);

		if (adv_1000)
			temp |= PHY_1000Base_T_Full;

		grtnic_PhyWrite(netdev, phy_addr, 0x09, temp);

		if (adapter->pdev->device == 0x0f42 || adapter->pdev->device == 0x0f41) {
			grtnic_PhyWrite(netdev, phy_addr, 0x0D, 0x07);  //MMD7
			grtnic_PhyWrite(netdev, phy_addr, 0x0E, 0x20);  //MMD7-reg0x20
			grtnic_PhyWrite(netdev, phy_addr, 0x0D, 0x4007);
			grtnic_PhyRead(netdev, phy_addr, 0x0E, &temp);
			if (adv_2500)
				temp |= PHY_2500Base_T_Full;
			else
				temp &= ~PHY_2500Base_T_Full;
			...
			grtnic_PhyWrite(netdev, phy_addr, 0x0E, 0x20);  //MMD7-reg0x20
			grtnic_PhyWrite(netdev, phy_addr, 0x0D, 0x4007);
			grtnic_PhyWrite(netdev, phy_addr, 0x0E, temp);
		}

		grtnic_PhyRead(netdev, phy_addr, 0x00, &temp);

		if (!adv_2500 && !adv_1000 && (adv_100_half))
			temp &= ~PHY_BMCR_DUPLEX;
		else if (!adv_2500 && !adv_1000 && !adv_100_half && !adv_100_full && adv_10_half)
			temp &= ~PHY_BMCR_DUPLEX;
		else
			temp |= PHY_BMCR_DUPLEX;

		temp &= ~PHY_BMCR_SPEED_MASK;
		if (adv_2500)
			temp |= PHY_BMCR_SPEED_MSB | PHY_BMCR_SPEED_LSB;
		else if (adv_1000)
			temp |= PHY_BMCR_SPEED_MSB;
		else if (adv_100_full || adv_100_half)
			temp |= PHY_BMCR_SPEED_LSB;

		if (autoneg) {
			temp |= PHY_BMCR_AUTO_NEG;
			adapter->autoneg_enabled = 1;
		} else {
			temp &= ~PHY_BMCR_AUTO_NEG;
			adapter->autoneg_enabled = 0;
		}

		grtnic_PhyWrite(netdev, phy_addr, 0x00, temp | PHY_BMCR_RESET);

	} else {
		if (adapter->pdev->device == 0x0f52 || adapter->pdev->device == 0x0f51) {
			if (autoneg) {
				u32 speed_config = 0x0;
				bool adv_1000 = false;
				bool adv_2500 = false;

				if (!bitmap_empty(cmd->link_modes.advertising,
						__ETHTOOL_LINK_MODE_MASK_NBITS)) {
					if (test_bit(ETHTOOL_LINK_MODE_1000baseX_Full_BIT,
						cmd->link_modes.advertising)) {
						speed_config = 0x1 << 1;
						adv_1000 = true;
					}

					if (test_bit(ETHTOOL_LINK_MODE_2500baseX_Full_BIT,
						cmd->link_modes.advertising)) {
						speed_config = speed_config | (0x1 << 2);
						adv_2500 = true;
					}
				...
				if (adv_1000 && adv_2500)
					e_info(drv, "autoneg = enabled, advertising speeds: 1000 Mbps, 2500 Mbps");
				else if (adv_1000)
					e_info(drv, "autoneg = enabled, advertising speeds: 1000 Mbps");
				else if (adv_2500)
					e_info(drv, "autoneg = enabled, advertising speeds: 2500 Mbps");

				//disable laser;
				GRTNIC_WRITE_REG(&adapter->hw, PHY_TX_DISABLE, 0x01, 0);
				/* 添加1秒延迟,允许激光器完全关闭 */
				//msleep(1000);
				mdelay(100);
				//enable laser;
				GRTNIC_WRITE_REG(&adapter->hw, PHY_TX_DISABLE, 0x00, 0);

				GRTNIC_WRITE_REG(&adapter->hw, AUTO_NEG_REG,
					speed_config | 0x1, 0);

				adapter->autoneg_enabled = 0x1;
				adapter->autoneg_advertised = speed_config >> 1;
			} else {
				u32 speed_config;
				...
				e_info(drv, "autoneg = disabled, forced speed = %u Mbps", speed);
				...
				GRTNIC_WRITE_REG(&adapter->hw, AUTO_NEG_REG, speed_config, 0);

				adapter->autoneg_enabled = 0x0;
				adapter->autoneg_advertised = speed_config >> 1;
			}
		}
	}
...
static int grtnic_nic_get_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	...
	if (adapter->type == 1) {   /* copper接口 */
		...
		ecmd->advertising |= (ADVERTISED_10baseT_Half | ADVERTISED_10baseT_Full |
			...
			ADVERTISED_1000baseT_Full | ADVERTISED_TP | ADVERTISED_Pause);

		if (adapter->pdev->device == 0x0f42 || adapter->pdev->device == 0x0f41)
			ecmd->advertising |= ADVERTISED_2500baseT_Full;
		...
		if (adapter->pdev->device == 0x0f42 || adapter->pdev->device == 0x0f41) {
			...
			grtnic_PhyRead(netdev, phy_addr, 0x0E, &phy_mmd7_reg0x21);
			if (phy_mmd7_reg0x21 & PHY_LP_2500Base_T_Full) {
				ecmd->lp_advertising |= ADVERTISED_2500baseT_Full;
				lp_advertised |= BIT(5);
			}
		}
		...
		ecmd->supported |= (SUPPORTED_FIBRE | SUPPORTED_Pause);

		if (max_port_speed) {
			ecmd->supported |= SUPPORTED_10000baseSR_Full;
			ecmd->supported |= SUPPORTED_10000baseLR_Full;
		} else {
			ecmd->supported |= SUPPORTED_1000baseX_Full;
		}

		if (adapter->pdev->device == 0x0f52 || adapter->pdev->device == 0x0f51)
			ecmd->supported |= SUPPORTED_2500baseX_Full;

		ecmd->advertising |= (ADVERTISED_Autoneg | ADVERTISED_FIBRE | ADVERTISED_Pause);

		if (max_port_speed) {
			ecmd->advertising |= ADVERTISED_10000baseSR_Full;
			ecmd->advertising |= ADVERTISED_10000baseLR_Full;
		} else {
			ecmd->advertising |= ADVERTISED_1000baseX_Full;
		}
	...
	if (netif_running(netdev)) {
		if (netif_carrier_ok(netdev)) {
			if (adapter->type == 0) {   /* fiber */
				ethtool_cmd_speed_set(ecmd, fiber_speed);
				/* 2500M光口 */
				if (adapter->pdev->device == 0x0f52 ||
						adapter->pdev->device == 0x0f51) {
					u32 speed = (port_speed == 0x2) ? SPEED_2500 :
						(port_speed == 0x1) ? SPEED_1000 : 0;
					ethtool_cmd_speed_set(ecmd, speed);
				}
			} else {    /* copper */
				if (adapter->autoneg_enabled) {
					ethtool_cmd_speed_set(ecmd, negotiated_speed);
					ecmd->duplex = negotiated_duplex;
				} else {
					u32 speed = (adapter->link_speed == 0x02) ? SPEED_1000 :
						(adapter->link_speed == 0x01) ? SPEED_100 :
						SPEED_10;
					ethtool_cmd_speed_set(ecmd, speed);
	...
	ecmd->autoneg = adapter->autoneg_enabled ? AUTONEG_ENABLE : AUTONEG_DISABLE;
	ecmd->transceiver = XCVR_INTERNAL;
	ecmd->mdio_support = ETH_MDIO_SUPPORTS_C22 | ETH_MDIO_SUPPORTS_C45;
...
static int grtnic_nic_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	...
	bool autoneg = (ecmd->autoneg == AUTONEG_ENABLE);
	...
	if (adapter->type == 1) {   /* copper接口 */
		...
		if (ecmd->advertising) {
			...
			if (ecmd->advertising & ADVERTISED_1000baseT_Full)
				adv_1000 = true;
			if (ecmd->advertising & ADVERTISED_2500baseT_Full)
				adv_2500 = true;
		} else if (!autoneg) {
			switch (speed) {
			case SPEED_10:
			...
			case SPEED_2500:
				if (adapter->pdev->device == 0x0f42 ||
						adapter->pdev->device == 0x0f41)
					adv_2500 = true;
				else
					return -EINVAL;
		...
		temp &= ~(PHY_10Base_TX_Half | PHY_10Base_TX_Full |
			PHY_100Base_TX_Half | PHY_100Base_TX_Full | PHY_NEXT_Page);
		...
		if (adapter->pdev->device == 0x0f52 || adapter->pdev->device == 0x0f51) {
			if (autoneg) {
				...
				if (ecmd->advertising) {
					if (ecmd->advertising & ADVERTISED_1000baseX_Full) {
						speed_config = 0x1 << 1;
						adv_1000 = true;
					}
					if (ecmd->advertising & ADVERTISED_2500baseX_Full) {
						speed_config = speed_config | (0x1 << 2);
						adv_2500 = true;
					}
				} else if (speed == SPEED_1000) {
				...
				GRTNIC_WRITE_REG(&adapter->hw, AUTO_NEG_REG, speed_config | 0x1, 0);
...
//  u32 chip_temp;
	...
	strscpy(drvinfo->driver, DRIVER_NAME, 32);
	strscpy(drvinfo->version, DRIVER_VERSION, 32);
...
//  printk("sn = %s\n", sn_s);
	...
	adapter->firmware_version = hw_version[0];

	if (hw_version[0] < 0x200) {  //maybe old firmware //0x101
		read_flash_buffer(adapter, (VPD_OFFSET - (offset * 0x100000)) + VERSION_OFFSET,
			2, (u32 *)&hw_version);
	...
	if (kstrtol(firmware_version, 10, &value_h))
		adapter->firmware_version = 0;
	else
		adapter->firmware_version = value_h;

	strscpy(drvinfo->fw_version, firmware_version, 32);
#if (KERNEL_VERSION(4, 0, 0) < LINUX_VERSION_CODE)
	strscpy(drvinfo->erom_version, ipxe_version, 32);
#endif
...
static int grtnic_set_pauseparam(struct net_device *netdev, struct ethtool_pauseparam *pause)
{
	...
	if (mac->fc.requested_mode == fc_full)
		flowctl = 3;
	else if (mac->fc.requested_mode == fc_tx_pause)
		flowctl = 2;
	else if (mac->fc.requested_mode == fc_rx_pause)
		flowctl = 1;
	else
		flowctl = 0;
...
static void
grtnic_get_ethtool_stats(struct net_device *netdev,
			 struct ethtool_stats __always_unused *stats, u64 *data)
{
	...
	for (i = 0; i < GRTNIC_NETDEV_STATS_LEN; i++) {
		...
		data[i] = (grtnic_gstrings_net_stats[i].sizeof_stat == sizeof(u64)) ?
			*(u64 *)p : *(u32 *)p;
	}
	for (j = 0; j < GRTNIC_GLOBAL_STATS_LEN; j++, i++) {
		p = (char *)adapter + grtnic_gstrings_stats[j].stat_offset;
		data[i] = (grtnic_gstrings_stats[j].sizeof_stat == sizeof(u64)) ?
			*(u64 *)p : *(u32 *)p;
	}
...
static void grtnic_get_wol(struct net_device *netdev, struct ethtool_wolinfo *wol)
{
	struct grtnic_adapter *adapter = netdev_priv(netdev);
	...
	wol->wolopts = adapter->wol_enabled ? WAKE_MAGIC : 0;
}

static int grtnic_set_wol(struct net_device *netdev, struct ethtool_wolinfo *wol)
{
	struct grtnic_adapter *adapter = netdev_priv(netdev);
	struct grtnic_hw *hw = &adapter->hw;
	...
	if (wol->wolopts & (WAKE_UCAST | WAKE_MCAST | WAKE_BCAST | WAKE_PHY |
			WAKE_ARP | WAKE_MAGICSECURE | WAKE_FILTER))
		return -EOPNOTSUPP;
	...
	adapter->wol_enabled = wol->wolopts & WAKE_MAGIC ? 1 : 0;
	...
	if (adapter->wol_enabled) {
		...
		GRTNIC_WRITE_REG(hw, XPHY_STATUS, (u32)(1 << 31), 0); //wol_enable
	} else {
		if (adapter->type == 1) {
			grtnic_PhyWrite(adapter->netdev, phy_addr, 0x1E, 0xA00A);
			grtnic_PhyWrite(adapter->netdev, phy_addr, 0x1F, 0x0000); //PME_N+WOL DIS
		}
		GRTNIC_WRITE_REG(hw, XPHY_STATUS, (u32)(0 << 31), 0); //wol_disable
	}

	device_set_wakeup_enable(&adapter->pdev->dev, adapter->wol_enabled);
...
static int grtnic_flash_device(struct net_device *netdev, struct ethtool_flash *flash)
{
	...
	if (rc != 0) {
		...
		netdev_err(netdev, "You MUST copy image file to /lib/firmware directory!!!");
		return rc;
	}
...
//    version = strtol(ipxe_ver_s,NULL,0);
	...
	if (image_type != 2) {  //vpd no vid&pid
		if (vid != adapter->pdev->vendor || pid != adapter->pdev->device) {
			netdev_err(netdev, "Wrong image!\n\n");
			return 0;
		}
	}

	netdev_info(netdev, "Found %s image File!!! ", (image_type == 1) ? "pxe" :
			(image_type == 2) ? "vpd" : "firmware");
	if (image_type == 0 || image_type == 1)
		netdev_info(netdev, "and version = %08d", version ^ 0xFFFFFFFF);
	...
	if (image_type == 2) {  //vpd image no needed vpd & pid
		...
		while (read_filesize > 0) {
			...
			if (once_size & 0x04)
				write_flash_buffer(adapter, offset, (once_size >> 2) + 1,
					(u32 *)(fw->data + copied));
			else
				write_flash_buffer(adapter, offset, once_size >> 2,
					(u32 *)(fw->data + copied));

			schedule = (i + 1) * 100 / cycle;

			if (i < cycle - 1)
				netdev_info(netdev, "\rUpgrading--->[%d%%]", schedule);
			else
				netdev_info(netdev, "\rUpgrading--->[%s]\n\n", "Done");
	...
	netdev_info(netdev, "firmware Update Complete\n");
	//  printk("Triggering IPROG to reload ASIC...\n");
	//  write_register(0xFEE1DEAD, adapter->user_bar + 0x0054);
	netdev_info(netdev, "YOU MUST REBOOT COMPUTER TO LET NEW FIRMWARE BEGIN WORKS!\n");
	return rc;
}
...
static int grtnic_get_rxfh(struct net_device *netdev, struct ethtool_rxfh_param *rxfh)
#else
...
#ifdef HAVE_RXFH_HASHFUNC
#ifdef HAVE_ETHTOOL_RXFH_PARAM
static int grtnic_set_rxfh(struct net_device *netdev, struct ethtool_rxfh_param *rxfh,
...
#endif /* HAVE_RXFH_HASHFUNC */
{
	...
	if (indir) {
		int max_queues = min_t(int, adapter->num_rx_queues,
			grtnic_rss_indir_tbl_max(adapter));
...
static irqreturn_t grtnic_test_intr(int __always_unused irq, void *data)
{
	...
	adapter->test_icr = GRTNIC_READ_REG(&adapter->hw,
		((TARGET_IRQ << 12) + ADDR_INTR_VECTOR * 4), 1);
	...
	*data = 0;
	...
	for (; i < 2; i++) {
		...
		if (!shared_int) {
			 ...
			 */
			adapter->test_icr = 0;
			GRTNIC_WRITE_REG(&adapter->hw, ((TARGET_IRQ << 12) + ADDR_INTR_IMC * 4),
				mask, 1);
			GRTNIC_WRITE_REG(&adapter->hw, ((TARGET_IRQ << 12) + ADDR_INTR_ICS * 4),
				mask, 1); //trigger interrupt
			GRTNIC_WRITE_FLUSH(&adapter->hw);
			usleep_range(10000, 20000);
		 ...
		 */
		adapter->test_icr = 0;
		GRTNIC_WRITE_REG(&adapter->hw, ((TARGET_IRQ << 12) + ADDR_INTR_IMS * 4),
			mask, 1);
		GRTNIC_WRITE_REG(&adapter->hw, ((TARGET_IRQ << 12) + ADDR_INTR_ICS * 4),
			mask, 1); //trigger interrupt
		GRTNIC_WRITE_FLUSH(&adapter->hw);
...
static int grtnic_setup_desc_rings(struct grtnic_adapter *adapter)
{
	...
	// GRTNIC_WRITE_REG(&adapter->hw, PG_PCSRESET, 0xff, 0);  //reset pcs
...
static int grtnic_setup_loopback_test(struct grtnic_adapter *adapter)
{
	...
	if (adapter->ei->type == board_1002E_GRT_FF || adapter->ei->type == board_1005E_GRT_FX) {
		...
		if (adapter->ei->type == board_1002E_GRT_FF && hw->revision == 1) {
			GRTNIC_WRITE_REG(hw, PG_LOOPBACK, 0x01, 0);
			reg_data = grtnic_mdio_read(netdev, hw->phy_addr, 3, 0);
			pr_info("%s: mdio3.0 = 0x%04x", __func__, reg_data);
			reg_data |= 0x4000;
			for (retry = 0; retry < 5; retry++) {
				grtnic_mdio_write(netdev, hw->phy_addr, 3, 0, reg_data);
				reg_data = grtnic_mdio_read(netdev, hw->phy_addr, 3, 0);
				pr_info("%s: mdio3.0 = 0x%04x (retry %d)", __func__,
					reg_data, retry);
				if (reg_data & 0x4000)
					break;
			}
		} else {
			//prtad_devad_reg //mdio reg:1.0
			grtnic_SetPhyAddr(netdev, phy_addr, 0x01, 0x00);
			grtnic_PhyRead(netdev, phy_addr, 0x01, &reg_data);
			reg_data |= 0x01; //loopback 1.0.0
			grtnic_PhyWrite(netdev, phy_addr, 0x01, reg_data);
		}
	} else {
		...
		/* Setup PHY loopback */
		grtnic_PhyRead(netdev, phy_addr, 0x00, &reg_data);
		...
		if (adapter->type == 1) {//copper
			...
			if (adapter->pdev->device == 0x0f42 || adapter->pdev->device == 0x0f41)
				GRTNIC_WRITE_REG(&adapter->hw, AUTO_NEG_REG, (0x1 << 1), 0);
		}

		grtnic_PhyWrite(netdev, phy_addr, 0x00, reg_data);

		if (adapter->type == 1) { //copper
			/* Setup mac speed */
			//这里用adapter->netdev,因为之前的已经为了qsgmii更改了netdev
...
static void grtnic_loopback_cleanup(struct grtnic_adapter *adapter)
{
	...
	if (adapter->ei->type == board_1002E_GRT_FF || adapter->ei->type == board_1005E_GRT_FX) {
		...
		if (adapter->ei->type == board_1002E_GRT_FF && hw->revision == 1) {
			...
			reg_data &= ~0x4000;
			grtnic_mdio_write(netdev, hw->phy_addr, 3, 0, reg_data);
			GRTNIC_WRITE_REG(hw, PG_LOOPBACK, 0x00, 0);
			reg_data = grtnic_mdio_read(netdev, hw->phy_addr, 3, 0);
			pr_info("%s: mdio3.0 = 0x%04x", __func__, reg_data);
			...
			//prtad_devad_reg //mdio reg:1.0
			grtnic_SetPhyAddr(adapter->netdev, phy_addr, 0x01, 0x00);
		...
		if (adapter->type == 1) {//copper
			reg_data |= PHY_POWER_DOWN;
			if (adapter->pdev->device == 0x0f42 || adapter->pdev->device == 0x0f41)
				GRTNIC_WRITE_REG(&adapter->hw, AUTO_NEG_REG, 0x7, 0);
		}
		grtnic_PhyWrite(netdev, phy_addr, 0x00, reg_data);
	}
...
static u16 grtnic_clean_test_rings(struct grtnic_ring *rx_ring, struct grtnic_ring *tx_ring,
	unsigned int size)
{
			     ...
			     struct ethtool_test *eth_test, u64 *data)
{
	...
	if (eth_test->flags == ETH_TEST_FL_OFFLINE) {
		 ...
		 *doesn't interfere with test result
		 */
				  ...
				  struct ethtool_modinfo *modinfo)
{
	...
	if (adapter->pdev->device == 0x0f52 || adapter->pdev->device == 0x0f51) {
		autoneg_status = GRTNIC_READ_REG(hw, AUTO_NEG_REG, 0);
		GRTNIC_WRITE_REG(&adapter->hw, AUTO_NEG_REG, autoneg_status | BIT(15), 0);
		usleep_range(10000, 20000);
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////
static const struct ethtool_ops grtnic_nic_ethtool_ops = {
#ifdef ETHTOOL_GLINKSETTINGS
	...
	.get_wol          = grtnic_get_wol,
	.set_wol          = grtnic_set_wol,
