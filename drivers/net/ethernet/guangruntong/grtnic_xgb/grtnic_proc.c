...
static struct proc_dir_entry *grtnic_top_dir;
...
static int grtnic_driver_pktcnt_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "tx_count0 = %u,  tx_count1 = %u, rx_count = %u\n", adapter->tx_count0,
		adapter->tx_count1, adapter->rx_count);
	return 0;
}
...
static int grtnic_driver_pktcnt_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
...
static int grtnic_hardware_pktcnt_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	u32 h2c_cnt, c2h_cnt, rx_cnt;
	struct grtnic_adapter *adapter = (struct grtnic_adapter *)data;
...
static int grtnic_hardware_error(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	u32 var;
	int read_count_error, sgtxfifo_error, sgrxfifo_error, mainfifo_error;
	...
	return snprintf(page, count, "read_count_error = %d,  sgtxfifo_error = %d,"
		"sgrxfifo_error = %d, mainfifo_error = %d\n", read_count_error,
		sgtxfifo_error, sgrxfifo_error, mainfifo_error);
}
...
#if (KERNEL_VERSION(3, 9, 0) <= LINUX_VERSION_CODE)
		proc_create_data(grtnic_proc_entries[index].name, 0444,
