...
static struct cpufreq_frequency_table gs464_cpufreq_table[] = {
...
static int gs464_cpufreq_set(struct cpufreq_policy *policy, int freq_level)
{
				     ...
				     unsigned int index)
{
	...
	if (new_method) {
		...
		gs464_cpufreq_set(policy, index);
...
static int gs464_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	...
	if (new_method) {
		...
		policy->freq_table = gs464_cpufreq_table;
	} else {
...
static int gs464_cpufreq_exit(struct cpufreq_policy *policy)
{
	return 0;
}
...
static int configure_cpufreq_info(void)
{
	...
	if (cpu_has_constant_timer)
		gs464_cpufreq_driver.flags |= CPUFREQ_CONST_LOOPS;
	...
	for (i = args2.min_level; i <= max_level; i++) {
		...
		gs464_cpufreq_table[i].frequency = args3.info * 1000;
		if (i > args2.max_normal_level)
			gs464_cpufreq_table[i].flags = CPUFREQ_BOOST_FREQ;
	}
