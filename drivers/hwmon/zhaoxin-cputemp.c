...
static ssize_t crit_show(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	err = rdmsrq_safe_on_cpu(data->id, data->msr_crit, &val);
	if (err)
		return -EAGAIN;

	return sprintf(buf, "%lu\n", ((unsigned long)val & 0xff) * 1000);
}
...
static ssize_t max_show(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	err = rdmsrq_safe_on_cpu(data->id, data->msr_max, &val);
	if (err)
		return -EAGAIN;
...
static int temperature_access(struct zhaoxin_cputemp_data *data, u32 *l, u32 *h)
{
	...
	*l = 0x19;
	...
	if (data->msr_index) {
		err = wrmsrq_safe_on_cpu(data->id, data->msr_index,
					 (u64)*h << 32 | *l);
		if (err) {
