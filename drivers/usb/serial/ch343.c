			     ...
			     u16 index)
{
	...
	retval = usb_control_msg(
		...
		USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_DIR_OUT, value, index,
				 ...
				 void *buf, unsigned bufsize)
{
	...
	retval = usb_control_msg(ch343->dev, usb_sndctrlpipe(ch343->dev, 0),
				 request, requesttype, value, index, buffer,
...
static int ch343_control_msg_in(struct ch343 *ch343, u8 request, u8 requesttype,
				u16 value, u16 index, void *buf,
				unsigned bufsize)
{
	int retval;
	char *buffer;
	...
	if (retval)
		goto out;
	retval = usb_control_msg(ch343->dev, usb_rcvctrlpipe(ch343->dev, 0),
				 request, requesttype, value, index, buffer,
				 bufsize, DEFAULT_TIMEOUT);
	if (retval > 0) {
...
{
	if (ch343->iface <= 1)
		return ch343_control_out(ch343, CMD_C2 + ch343->iface, ~control,
					 0x0000);
	else if (ch343->iface <= 3)
		return ch343_control_out(ch343,
					 CMD_C2 + 0x10 + (ch343->iface - 2),
					 ~control, 0x0000);
	else
		return -1;
}
...
static int ch343_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	if (ch343->chiptype == CHIP_CH9114L ||
	    ...
	    ch343->chiptype == CHIP_CH9114W ||
	    ch343->chiptype == CHIP_CH346C_M2) {
		retval = ch343_control_out(ch343, CMD_C8,
					   0x01 | (ch343->iface << 8), 0x00);
		if (retval) {
			goto error_submit_read_urbs;
		}
	}
...
static void ch343_port_shutdown(struct tty_port *port)
{
	...
	if (ch343->chiptype == CHIP_CH9114L ||
	    ...
	    ch343->chiptype == CHIP_CH9114W ||
	    ch343->chiptype == CHIP_CH346C_M2) {
		r = ch343_control_out(ch343, CMD_C8, 0x02 | (ch343->iface << 8),
				      0x00);
		if (r)
...
static int ch343_tty_break_ctl(struct tty_struct *tty, int state)
{
	...
	if (ch343->chiptype == CHIP_CH346C_M0 ||
	    ch343->chiptype == CHIP_CH346C_M1 ||
	    ch343->chiptype == CHIP_CH346C_M2 ||
	    ch343->chiptype == CHIP_CH339W || ch343->chiptype == CHIP_CH347TF ||
			   ...
			   unsigned long arg)
{
	...
	case TIOCGSERIAL: /* gets serial port data */
		rv = ch343_get_serial_info(ch343,
					   (struct serial_struct __user *)arg);
		break;
	case TIOCSSERIAL:
		rv = ch343_set_serial_info(ch343,
					   (struct serial_struct __user *)arg);
		break;
	case TIOCMIWAIT:
	...
	case TIOCGICOUNT:
		rv = ch343_get_serial_usage(
			ch343, (struct serial_icounter_struct __user *)arg);
		break;
...
static int ch343_get(struct ch343 *ch343, enum CHIPTYPE chiptype,
		     unsigned int bval, unsigned char *fct, unsigned char *dvs)
{
	unsigned char a;
	unsigned char b;
	unsigned long c;
...

{
	...
	r = ch343_get(ch343, ch343->chiptype, newline.dwDTERate, &fct, &dvs);
	if (r) {
		dev_err(&ch343->control->dev,
	...
	if (ch343->iface <= 1)
		...
		ch343_control_out(ch343, CMD_C1 + 0x10 + (ch343->iface - 2),
				  value, index);
...
static int ch343_release(struct inode *inode, struct file *file)
{
	...
	mutex_unlock(&ch343->mutex);
	if (ch343) {
		tty_port_put(&ch343->port); /* release the tty */
	}
	return 0;
}
		       ...
		       const struct usb_device_id *id)
{
	...
	if (intf->altsetting->desc.bInterfaceClass == USB_CLASS_VENDOR_SPEC) {
	...
	while (buflen > 0) {
		...
		if (elength > buflen) {
			dev_err(&intf->dev,
				"descriptor length exceeds buffer\n");
			return -EINVAL;
		}
...
found_dev:
	...
	ch343->iface =
		control_interface->cur_altsetting->desc.bInterfaceNumber / 2;
	...
	if (minor < 0) {
		...
		usb_put_intf(control_interface);
		kfree(ch343);
		return minor;
	}
	...
	for (i = 0; i < num_rx_buf; i++) {
		...
		usb_fill_bulk_urb(urb, ch343->dev, ch343->rx_endpoint, rb->base,
				  ch343->readsize, ch343_read_bulk_callback,
				  rb);
	...
	if (data_interface != intf) {
		rv = usb_driver_claim_interface(&ch343_driver, data_interface,
						ch343);
		if (rv)
...
static int __init ch343_init(void)
{
	...
	ch343_tty_driver = tty_alloc_driver(
		CH343_TTY_MINORS, TTY_DRIVER_REAL_RAW | TTY_DRIVER_DYNAMIC_DEV);
	...
	ch343_tty_driver->init_termios.c_cflag = B115200 | CS8 | CREAD | HUPCL |
						 CLOCAL;
