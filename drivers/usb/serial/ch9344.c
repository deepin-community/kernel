...
static struct ch9344 *ch9344_get_by_index(unsigned int index)
{
	...
	if (ch9344) {
		...
		if (ch9344->disconnected) {
			...
			tty_port_get(&ch9344->ttyport[ch9344_get_portnum(index)]
					      .port);
			mutex_unlock(&ch9344->mutex);
		}
	}
 ...
 */
static int ch9344_control_out(struct ch9344 *ch9344, u8 request, u8 requesttype,
...
static int ch9344_control_in(struct ch9344 *ch9344, u8 request, u8 requesttype,
			     ...
			     unsigned int bufsize)
{
	...
	if (retval)
		goto out;
	retval = usb_control_msg(ch9344->dev, usb_rcvctrlpipe(ch9344->dev, 0),
				 request, requesttype, value, index, buf,
				 bufsize, DEFAULT_TIMEOUT);
...
static int ch9344_cmd_out(struct ch9344 *ch9344, u8 *buf, int count)
{
	...
	retval = usb_bulk_msg(ch9344->dev, ch9344->cmdtx_endpoint, buf, count,
	...
	case 0:
		...
		for (i = 0; i < len;) {
			...
			if (reg_iir == R_INIT) {
				...
				if (reg_iir == R_IO_CI) {
					spin_lock_irqsave(&ch9344->write_lock,
							  flags);
					ch9344->gpio_recv = true;
					memcpy(&gpiovalins, data + i + 2, 0x08);
					ch9344->gpiovalins =
						le64_to_cpu(gpiovalins);
					wake_up_interruptible(
						&ch9344->wgpioioctl);
					spin_unlock_irqrestore(
						&ch9344->write_lock, flags);
					if (i + 10 > len)
						break;
					i += 10;
				} else {
				...
				if ((portnum >= left) && (portnum < right)) {
					...
					ch9344->ttyport[portnum].oldcount =
						ch9344->ttyport[portnum].iocount;
				...
				dev_dbg(&ch9344->data->dev,
					"%s - wrong status received", __func__);
				break;
			}
			if (i + 3 > len)
				break;
...
static void ch9344_process_read_urb(struct ch9344 *ch9344, struct urb *urb)
{
	...
	if (urb->actual_length > sizeof(buffer))
		return;
...
static int ch9344_tty_open(struct tty_struct *tty, struct file *filp)
{
	...
	rv = tty_port_open(
		...
		filp);
	if (!rv) {
		ch9344->ttyport[ch9344_get_portnum(tty->index)].write_empty =
			true;
	}
...

{
	...
	if (raise) {
		ch9344->ttyport[portnum].ctrlout = CH9344_CTO_D | CH9344_CTO_R;
		ch9344_set_control(ch9344, portnum, 0x01);
		ch9344_set_control(ch9344, portnum, 0x11);
	} else {
		ch9344->ttyport[portnum].ctrlout = 0;
	...
	*(wb->buf + 2) = packlen >> 8;
	...
	if (!ch9344->ttyport[portnum].write_empty) {
		...
		if (timeout <= 0) {
			...
			usb_autopm_put_interface_async(ch9344->data);
			dev_err(&ch9344->data->dev, "%s - write timeout\n",
				__func__);
...
static int ch9344_tty_tiocmget(struct tty_struct *tty)
{
	...
	result = (ch9344->ttyport[portnum].ctrlout & CH9344_CTO_D ? TIOCM_DTR :
								    0) |
		 (ch9344->ttyport[portnum].ctrlout & CH9344_CTO_R ? TIOCM_RTS :
								    0) |
		 (ch9344->ttyport[portnum].ctrlin & CH9344_CTI_C ? TIOCM_CTS :
								   0) |
		 (ch9344->ttyport[portnum].ctrlin & CH9344_CTI_DS ? TIOCM_DSR :
								    0) |
		 (ch9344->ttyport[portnum].ctrlin & CH9344_CTI_R ? TIOCM_RI :
								   0) |
		 (ch9344->ttyport[portnum].ctrlin & CH9344_CTI_DC ? TIOCM_CD :
								    0);
...
static int ch9344_get_serial_usage(struct ch9344 *ch9344, int portnum,
			    ...
			    unsigned long arg)
{
	...
	case TIOCGSERIAL: /* gets serial port data */
		rv = ch9344_get_serial_info(ch9344, portnum,
					    (struct serial_struct __user *)arg);
		break;
	case TIOCSSERIAL:
		rv = ch9344_set_serial_info(ch9344, portnum,
					    (struct serial_struct __user *)arg);
		break;
	case TIOCMIWAIT:
	...
	case IOCTL_CMD_CTRLIN:
		...
		if (rv)
			goto out;
		rv = ch9344_control_in(ch9344, (u8)arg1, (u8)arg2, (u16)arg3,
				       (u16)arg4, (u8 __user *)buffer,
				       (u16)arg5);
		if (rv < 0) {
			goto out;
		}
				   ...
				   const struct ktermios *termios_old)
{
	...
	if ((ch9344->chiptype == CHIP_CH348Q) && (portnum > 3)) {
		...
		unsigned int xorval = newctrl ^
				      ch9344->ttyport[portnum].ctrlout;
...
static void ch9344_write_buffers_free(struct ch9344 *ch9344)
{
	...
	for (index = 0; index < ch9344->num_ports; index++) {
		for (i = 0; i < CH9344_NW; i++) {
			...
			usb_free_coherent(usb_dev, ch9344->writesize, wb->buf,
					  wb->dmah);
		}
	}
}
	...
	for (index = 0; index < ch9344->num_ports; index++) {
		for (i = 0; i < CH9344_NW; i++) {
			...
			if (!wb->buf) {
				int j, k;
				for (j = 0; j <= index; j++) {
					int end = (j == index) ? i : CH9344_NW;

					for (k = 0; k < end; k++) {
						struct ch9344_wb *tmp =
							&ch9344->wb[j][k];
						if (tmp->buf) {
							usb_free_coherent(
								ch9344->dev,
...
static long ch9344_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_CMD_GPIOENABLE:
		...
		if (ch9344->chiptype == CHIP_CH9344L ||
			...
			if (inargL)
				...
				uartmode = M_NOR;
			if ((portindex > 3) ||
			    (ch9344->ttyport[portindex].uartmode == M_HF)) {
				rv = -EINVAL;
				goto out;
			}
			mutex_lock(&ch9344->gpiomutex);
			rv = ch9344_set_uartmode(ch9344, portindex, portindex,
						 uartmode);
			if (!rv) {
	...
	case IOCTL_CMD_GPIODIR:
		...
		if (ch9344->chiptype == CHIP_CH9344L ||
			...
			if ((gpionumber > MAXGPIO) ||
			    (ch9344->ttyport[gpiogroup].uartmode != M_IO)) {
				rv = -EINVAL;
				goto out;
			}
			mutex_lock(&ch9344->gpiomutex);
			rv = ch9344_set_gpiodir(ch9344, gpiogroup, gpionumber,
						gpiodir);
			if (!rv) {
	...
	case IOCTL_CMD_GPIOSET:
		...
		if (ch9344->chiptype == CHIP_CH9344L ||
			...
			mutex_lock(&ch9344->gpiomutex);
			rv = ch9344_set_gpioval(ch9344, gpiogroup, gpionumber,
						gpioval);
			if (!rv) {
			...
			const struct usb_device_id *id)
{
	...
	if (data_interface->cur_altsetting->desc.bNumEndpoints < 4 ||
	...
	usb_fill_bulk_urb(ch9344->cmdreadurb, usb_dev,
			  usb_rcvbulkpipe(usb_dev, epcmdread->bEndpointAddress),
			  ch9344->cmdread_buffer, cmdsize, ch9344_cmd_irq,
			  ch9344);
	...
	for (i = 0; i < portnum; i++) {
		...
		tty_dev = tty_port_register_device(&ch9344->ttyport[i].port,
						   ch9344_tty_driver,
						   NUMSTEP * minor + i,
						   &data_interface->dev);
		if (IS_ERR(tty_dev)) {
		...
		rv = kfifo_alloc(&ch9344->ttyport[i].rfifo, FIFOSIZE,
				 GFP_KERNEL);
		if (rv) {
			dev_err(&ch9344->data->dev, "%s - kfifo_alloc failed\n",
				__func__);
			goto post_claim_fail;
		}
#endif
...
static int ch9344_reset_resume(struct usb_interface *intf)
{
	...
	for (i = 0; i < ch9344->num_ports; i++) {
		...
		retval = ch9344_port_activate(&ch9344->ttyport[i].port, tty);
		tty_kref_put(tty);
		if (retval)
			goto error_activate;
	}
...
static int __init ch9344_init(void)
{
	...
	ch9344_tty_driver =
		tty_alloc_driver(CH9344_TTY_MINORS,
