...
static int scmi_perf_domain_probe(struct scmi_device *sdev)
{
	...
	struct fwnode_genpd_onecell_data *scmi_pd_data;
	...
	for (i = 0; i < num_domains; i++, scmi_pd++) {
		...
		ret = fwnode_pm_genpd_init(&scmi_pd->genpd, NULL, false);
		if (ret)
			goto err;
...
{
	struct device *dev = &sdev->dev;
	struct fwnode_genpd_onecell_data *scmi_pd_data = dev_get_drvdata(dev);
	int i;
...
{
	return cix_scmi_driver_register(&scmi_perf_domain_driver, THIS_MODULE, KBUILD_MODNAME);
}

//subsys_initcall_sync(scmi_perf_init);
module_init(scmi_perf_init);
//core_initcall(scmi_perf_init);
