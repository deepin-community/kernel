...
static irqreturn_t sky1_pcie_local_irq_handler(int irq, void *arg)
{
	...
	for (i = 0; i < 32; i++) {
		if (sta & BIT(i)) {
			dev_err(pcie->dev, "local err0 int: %s happened\n",
	...
	for (i = 0; i < 32; i++) {
		if (sta & BIT(i)) {
			dev_err(pcie->dev, "local err1 int: %s happened\n",
...
static void sky1_pcie_handle_rp_aer_irq(struct sky1_pcie *pcie)
{
	...
	if (uncorr_val) {
		...
		if (!pcie->aer_uncor_dump) {
			/* if happened uncorrectable error, dump once phy registers */
			pcie->aer_uncor_dump = true;
		}
		if (pcie->is_aer_uncor_panic)
			panic("sky1: PCIe occurred uncorrectable error\n");
	}
	if (corr_val)
...
static int sky1_pcie_set_pwr_en(struct sky1_pcie *pcie, bool en)
{
	...
	dev_dbg(dev, "Set STRAP_REG(0x%p) as :0x%x\n", strap_addr, readl(strap_addr));
...
static void sky1_pcie_init_bases(struct sky1_pcie *pcie)
{
	...
	dev_dbg(dev, "pcie->strap_base vaddr is: 0x%p\n", pcie->strap_base);
	dev_dbg(dev, "pcie->status_base vaddr is: 0x%p\n", pcie->status_base);
}
...
static int sky1_pcie_parse_mem(struct sky1_pcie *pcie)
{
	...
	dev_info(dev, "ioremap %s, paddr:%pR, vaddr:%p\n", "rcsu", res,
		 pcie->rcsu_base);
...
static int sky1_pcie_parse_reset_gpio(struct sky1_pcie *pcie)
{
	...
	if (IS_ERR(gpiodesc)) {
		dev_err(dev, "Failed to get optional reset gpio, err=%ld .\n", PTR_ERR(gpiodesc));
...
static int sky1_pcie_parse_reset_ctrl(struct sky1_pcie *pcie)
{
	...
	if (IS_ERR(rst)) {
		dev_err(dev, "Failed to get exclusive_reset gpio\n");
		return PTR_ERR(rst);
	}
	pcie->rst = rst;
...
static int sky1_pcie_really_probe(void *p)
{
	...
	ret = cix_cdns_pcie_host_setup(rc);
	if (ret < 0)
		goto err_ecam_free;
