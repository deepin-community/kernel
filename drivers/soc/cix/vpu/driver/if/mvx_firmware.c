	...
	if (fw->fw_bin->securevideo == false) {
		...
		for (i = 0; i < fw->ncores; i++) {
			if (mvx_test_bit(i, &mask)) {
				ret =
				    fw->ops.get_region(MVX_FW_REGION_CORE_0 + i,
						       &begin, &end);
				if (ret == 0)
...
static int fw_map_core(struct mvx_fw *fw, unsigned int core)
{
	...
	ret = mvx_mmu_map_pages(fw->mmu,
				...
				fw->text,
				MVX_ATTR_PRIVATE, MVX_ACCESS_EXECUTABLE, NULL);
	if (ret != 0)
		return ret;
...
static int fw_map(struct mvx_fw *fw)
{
	...
	if (fw->fw_bin->securevideo != false) {
		...
		for (i = 0; i < fw->ncores; i++) {
			if (mvx_test_bit(i, &mask)) {
				phys_addr_t l2 =
				    fw->fw_bin->secure.securefw->l2pages +
				    i * MVE_PAGE_SIZE;
		...
		fw->text =
		    mvx_mmu_alloc_pages(fw->dev, fw_bin->nonsecure.text_cnt, 0,
					GFP_KERNEL);
		if (IS_ERR(fw->text))
		...
		fw->bss =
		    mvx_mmu_alloc_pages(fw->dev,
					fw_bin->nonsecure.bss_cnt * fw->ncores,
		...
		fw->bss_shared =
		    mvx_mmu_alloc_pages(fw->dev, fw_bin->nonsecure.sbss_cnt, 0,
		...
		ret =
		    fw->ops.get_region(MVX_FW_REGION_CORE_0 + __ffs(mask),
				       &fw_base, &end);
		if (ret != 0)
			goto unmap_fw;
...
static int fw_stat_show(struct seq_file *s, void *v)
{
	...
	mvx_seq_printf(s, "fw_bin.dir", 0, "%s\n",
		       ...
		       (fw_bin->dir == MVX_DIR_OUTPUT) ? "out" : "invalid");
...
int mvx_fw_factory(struct mvx_fw *fw,
		   ...
		   unsigned int core_mask, struct dentry *parent)
{
	unsigned int major;
	unsigned int minor;
	unsigned int ncores;
		     ...
		     unsigned int core_mask)
{
     ...
     * 0x0000_0000 - 0x0000_00018) at the beginning. Therefore, must do map for
