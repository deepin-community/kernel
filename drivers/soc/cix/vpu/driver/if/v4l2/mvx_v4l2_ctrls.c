...
static int set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_VIDEO_HEVC_TIER:
		...
		ret = mvx_session_set_tier(session, MVX_FORMAT_HEVC, mvx_tier);
		break;
	...
	case V4L2_CID_MVE_VIDEO_ENABLE_ADAPTIVE_INTRA_BLOCK:
		...
		if (i32_val != 0)
			ret =
			    mvx_session_set_adaptive_intra_block(session,
								 i32_val);
		break;
	...
	case V4L2_CID_MVE_VIDEO_JPEG_QUALITY_LUMA:
		...
		if (i32_val != 0)
			ret =
			    mvx_session_set_jpeg_quality_luma(session, i32_val);
		break;
	case V4L2_CID_MVE_VIDEO_JPEG_QUALITY_CHROMA:
		i32_val = *ctrl->p_new.p_s32;
		if (i32_val != 0)
			ret =
			    mvx_session_set_jpeg_quality_chroma(session,
								i32_val);
		break;
	...
	case V4L2_CID_MVE_VIDEO_ENC_INTRA_IPENALTY_ANGULAR:
		i32_val = *ctrl->p_new.p_s32;
		ret =
		    mvx_session_set_enc_intra_ipenalty_angular(session,
							       i32_val);
		break;
	case V4L2_CID_MVE_VIDEO_ENC_INTRA_IPENALTY_PLANAR:
		i32_val = *ctrl->p_new.p_s32;
		ret =
		    mvx_session_set_enc_intra_ipenalty_planar(session, i32_val);
		break;
	...
	case V4L2_CID_MVE_VIDEO_ENC_INTER_IPENALTY_ANGULAR:
		i32_val = *ctrl->p_new.p_s32;
		ret =
		    mvx_session_set_enc_inter_ipenalty_angular(session,
							       i32_val);
		break;
	case V4L2_CID_MVE_VIDEO_ENC_INTER_IPENALTY_PLANAR:
		i32_val = *ctrl->p_new.p_s32;
		ret =
		    mvx_session_set_enc_inter_ipenalty_planar(session, i32_val);
		break;
 ...
 */
static struct v4l2_ctrl *mvx_v4l2_ctrl_new_custom_int(struct v4l2_ctrl_handler
						      *hnd, int id,
						      const char *name,
 ...
 *         or NULL in case of failure.
 */
static struct v4l2_ctrl *mvx_v4l2_ctrl_new_custom_tristate(struct
							   v4l2_ctrl_handler
							   *hnd, int id,
							   const char *name,
							   enum mvx_tristate
				     ...
				     unsigned long flags)
{
	...
	ctrl =
	    v4l2_ctrl_new_std_compound(hnd, &ctrl_ops,
...
int mvx_v4l2_ctrls_init_dec(struct v4l2_ctrl_handler *hnd)
{
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_SECURE_VIDEO,
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_tristate(hnd,
	...
	ctrl =
	    v4l2_ctrl_new_std_menu(hnd, &ctrl_ops,
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 V4L2_CID_MVE_VIDEO_FORCE_CHROMA_FORMAT,
					 "force chroma format", -1, INT_MAX, 0,
					 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MIN_BUFFERS_FOR_OUTPUT,
			      1, 32, 1, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MIN_BUFFERS_FOR_CAPTURE,
			      1, 32, 1, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "max buffer count to allocate", 1,
					 VIDEO_MAX_FRAME, VIDEO_MAX_FRAME, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "yuv2rgb color conversion", 0, 0x1f, 0,
					 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "dsl interp mode", 0, 0xff, 0xff, 1);
	if (ctrl == NULL)
		goto handler_free;
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 V4L2_CID_MVE_VIDEO_DISABLED_FEATURES,
					 "disabled features", 0, INT_MAX, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_JOB_FRAMES,
					    "job frames", 0, INT_MAX, 1, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_AV1_FSF,
					    "av1 fsf mode",
					    MVX_INVALID_VAL, 1, MVX_INVALID_VAL,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_PRIORITY,
					    "priority", 0, 3, 2, 1);
	if (ctrl == NULL)
		goto handler_free;
...
int mvx_v4l2_ctrls_init_enc(struct v4l2_ctrl_handler *hnd)
{
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_BITRATE_PEAK,
	...
	ctrl =
	    v4l2_ctrl_new_std_menu(hnd, &ctrl_ops,
				   ...
				   V4L2_MPEG_VIDEO_H264_LEVEL_6_1,
				   get_skip_mask(h264_level_list,
						 ARRAY_SIZE(h264_level_list)),
				   V4L2_MPEG_VIDEO_H264_LEVEL_1_0);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    v4l2_ctrl_new_std_menu(hnd, &ctrl_ops,
				   ...
				   V4L2_MPEG_VIDEO_HEVC_LEVEL_6_1,
				   get_skip_mask(h265_level_list,
						 ARRAY_SIZE(h265_level_list)),
				   V4L2_MPEG_VIDEO_HEVC_LEVEL_1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    v4l2_ctrl_new_std_menu(hnd, &ctrl_ops,
				   ...
				   V4L2_MPEG_VIDEO_HEVC_TIER_HIGH,
				   get_skip_mask(h265_tier_list,
						 ARRAY_SIZE(h265_tier_list)),
				   V4L2_MPEG_VIDEO_HEVC_TIER_MAIN);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_MV_H_SEARCH_RANGE, 0, INT_MAX,
			      1, 0);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_MV_V_SEARCH_RANGE, 0, INT_MAX,
			      1, 0);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_BITDEPTH_LUMA,
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_CABAC_INIT_IDC,
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_H263_I_FRAME_QP, 0, 31, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_H263_P_FRAME_QP, 0, 31, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_H263_B_FRAME_QP, 0, 31, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H263_MIN_QP,
			      1, 31, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H263_MAX_QP,
			      1, 31, 1, 31);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_H264_I_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_H264_P_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_H264_B_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_MIN_QP,
			      1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_MAX_QP,
			      1, 51, 1, 51);
	if (ctrl == NULL)
		goto handler_free;
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_HEVC_I_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_HEVC_P_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_HEVC_B_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP,
			      1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP,
			      1, 51, 1, 51);
	if (ctrl == NULL)
		goto handler_free;
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_VPX_I_FRAME_QP, 0, 63, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops,
			      V4L2_CID_MPEG_VIDEO_VPX_P_FRAME_QP, 0, 63, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_FORCE_KEY_FRAME,
			  0, 0, 0, 0);

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_VPX_B_FRAME_QP,
					 "VPx B frame QP value", 0, 63, 0, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_JPEG_COMPRESSION_QUALITY,
			      0, 100, 1, 0);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "JPEG compress luma quality", 0, 100,
					 0, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "JPEG compress chroma quality", 0, 100,
					 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_INIT_QP_I,
					    "init qp of I frame", 0, 51, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_INIT_QP_P,
					    "init qp of P frame", 0, 51, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_SAO_LUMA,
					    "sao luma", 0, 0xffff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_SAO_CHROMA,
					    "sao chroma", 0, 0xffff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_QP_DELTA_I_P,
					 "delta qp of I and P frame", 0, 51, 0,
					 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_QP_REF_RB_EN,
					 "qp ref rb en", 0, 51, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_RC_CLIP_TOP,
					    "rc clip top", 0, 0xff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_RC_CLIP_BOT,
					    "rc clip bot", 0, 0xff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "qp map clip bot", 0, 0xff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_MAX_QP_I,
					    "qp max for I frame", 0, 63, 63, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_MIN_QP_I,
					    "qp min for I frame", 0, 63, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_VISIBLE_WIDTH,
					 "visible width for encode", 0, 0xffff,
					 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_VISIBLE_HEIGHT,
					 "visible height for encode", 0, 0xffff,
					 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_GDR_NUMBER,
					    "gdr number", 0, 0xffff, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_GDR_PERIOD,
					    "gdr period", 0, 0xffff, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_SCD_ENABLE,
					    "scd enable", 0, 1, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_SCD_PERCENT,
					    "scd percent", 0, 10, 3, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_SCD_THRESHOLD,
					 "scd  threshold", 0, 2047, 100, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_AQ_SSIM_EN,
					    "aq ssim enable", 0, 1, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_AQ_NEG_RATIO,
					 "aq neg ratio", 0, 63, 24, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_AQ_POS_RATIO,
					 "aq pos ratio", 0, 63, 24, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_AQ_QPDELTA_LMT,
					 "aq qpdelta lmt", 0, 7, 7, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "aq init frm avg svar", 0, 15, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_MULIT_SPS_PPS,
					 "sps pps", 0, 0xffff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_ENABLE_VISUAL,
					 "enable visual", 0, 0x1f, 0, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "enable adaptive intra block", 0, 1, 0,
					 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "forced uv value", 0, 0x400, 0x400, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 MVX_INVALID_VAL, 31, MVX_INVALID_VAL,
					 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "intra ipenalty dc", MVX_INVALID_VAL,
					 31, MVX_INVALID_VAL, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl =
	    mvx_v4l2_ctrl_new_custom_int(hnd,
					 ...
					 "inter ipenalty dc", MVX_INVALID_VAL,
					 31, MVX_INVALID_VAL, 1);
	if (ctrl == NULL)
		goto handler_free;
