...
static int update_mvx_flags(struct mvx_buffer *buf, struct vb2_buffer *b)
{
	...
	if (mvx_is_afbc(buf->format)) {
		...
		if (buf->dir == MVX_DIR_INPUT) {
			...
			if ((flags & V4L2_BUF_FLAG_MVX_BUFFER_RESET_RC) ==
				V4L2_BUF_FLAG_MVX_BUFFER_RESET_RC)
...
static int update_vb2_flags(struct mvx_v4l2_buffer *vbuf)
{
	...
	b->flags &= ~(V4L2_BUF_FLAG_ERROR |
		      ...
		      V4L2_BUF_FLAG_LAST | V4L2_BUF_FLAG_END_OF_SUB_FRAME);
