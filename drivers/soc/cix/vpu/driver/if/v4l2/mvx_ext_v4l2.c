...
int mvx_ext_if_register_device(struct mvx_ext_if *ext,
			       const char *name, bool is_encoder)
{
			 ...
			 struct dentry *parent)
{
	...
	ret = mvx_ext_if_register_device(&ext[MVX_EXT_IF_DECODER], "mvxdec", 0);
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR,
	...
	ret = mvx_ext_if_register_device(&ext[MVX_EXT_IF_ENCODER], "mvxenc", 1);
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR,
