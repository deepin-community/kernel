...
static int append_sg_table(struct mvx_mmu_pages *pages, struct sg_table *sgt)
{
	...
	for_each_sg_page(sgt->sgl, &piter, sgt->nents, 0) {
		...
		base = (phys_addr_t)
		    sg_page_iter_dma_address((struct sg_dma_page_iter
					      *)(&piter)) & PAGE_MASK;
...
void mvx_mmu_free_noncontiguous(struct device *dev,
				...
				struct sg_table *sgt, void **data, size_t size)
{
...
struct mvx_mmu_pages *mvx_mmu_alloc_pages(struct device *dev,
					  size_t count,
					  size_t capacity, gfp_t gfp_mask)
{
	struct mvx_mmu_pages *pages;
	int ret;
...
int mvx_mmu_map_pages(struct mvx_mmu *mmu,
		      ...
		      enum mvx_mmu_access access, mvx_mmu_va *tried_size)
{
	size_t i;
	int ret;
...
int mvx_mmu_write(struct mvx_mmu *mmu,
		  mvx_mmu_va va, const void *data, size_t size)
{
	mvx_mmu_va end = va + size;
...
mvx_mmu_pte mvx_mmu_set_pte(enum mvx_mmu_attr attr,
			    phys_addr_t pa, enum mvx_mmu_access access)
{
