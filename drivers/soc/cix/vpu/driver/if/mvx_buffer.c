...
static int map_plane(struct mvx_buffer *buf,
		     mvx_mmu_va begin,
		     mvx_mmu_va end, mvx_mmu_va *next_va, unsigned int plane)
{
	...
	do {
		...
		ret =
		    mvx_mmu_map_pages(buf->mmu, va, p->pages,
		...
		tried_size =
		    (tried_size + MVE_PAGE_SIZE - 1) & ~(MVE_PAGE_SIZE - 1);
		va += tried_size;
	} while (va < stop_va);
...
static int mvx_buffer_map_discrete_planes(struct mvx_buffer *buf,
					  mvx_mmu_va begin,
					  mvx_mmu_va end, mvx_mmu_va *next_va)
{
	int i;
	int ret;
		   ...
		   mvx_mmu_va end, mvx_mmu_va *next_va, unsigned int *size)
{
	...
	if (buf->is_contiguous)
		return mvx_buffer_map_contiguous_planes(buf, begin, end,
							next_va, size);
	else
	...
	for (i = 0; i < *nplanes; i++) {
		...
		if (i >= __nplanes || stride[i] == 0) {
			if (setting_stride[i] == stride[i]) {
				stride[i] =
				    max(stride[i], round_up(tmp, stride_align));
			} else {
			       ...
			       unsigned int *max_height)
{
	...
	*max_width =
	    DIV_ROUND_UP(buf->planes[0].stride * SUBSAMPLE_PIXELS, s[0][0]);
