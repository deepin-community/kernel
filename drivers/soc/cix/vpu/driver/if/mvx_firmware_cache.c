...
static int get_fw_name(char *filename,
		       ...
		       enum mvx_direction dir, struct mvx_hw_ver *hw_ver)
{
...
static void request_firmware_done(const struct firmware *fw, void *arg)
{
	...
	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO,
		      ...
		      fw_bin->nonsecure.bss_cnt,
		      fw_bin->nonsecure.sbss_cnt,
		      header->text_length, header->bss_start_address);
					...
					bool securevideo)
{
	...
	if (securevideo != false)
		ret =
		    mvx_secure_request_firmware_nowait(cache->secure,
						       fw_bin->filename,
						       MVX_SECURE_NUMCORES,
						       fw_bin,
