...
#endif
	clk_handle = (struct clk *)gpu_clk_handle;
	if (!__clk_is_enabled(clk_handle))
		WARN_ON(clk_prepare_enable(clk_handle));

	clk_rate = clk_get_rate(clk_handle);

	if (__clk_is_enabled(clk_handle)) {
		clk_disable_unprepare(clk_handle);
		WARN_ON(__clk_is_enabled(clk_handle));
	}
