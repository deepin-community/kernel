 ...
 * @np: Pointer to acpi_device.fwnode_handle
 *
...
{
	int err = 0;
	struct fwnode_handle *pma_node;
	struct platform_device *pdev;
	struct fwnode_handle *fwnode = kbdev->dev->fwnode;

	pma_node = fwnode_find_reference(fwnode, "protected-memory-allocator", 0);
	if (IS_ERR(pma_node))
		dev_info(kbdev->dev, "Protected memory allocator not available\n");
	else {
		if (has_acpi_companion(kbdev->dev))
			pdev = acpi_find_device_by_node(pma_node);
		else
			pdev = of_find_device_by_node(to_of_node(pma_node));
