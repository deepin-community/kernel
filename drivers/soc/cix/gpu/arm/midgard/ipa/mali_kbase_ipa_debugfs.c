...
static int kbasep_ipa_current_power_debugfs_show(struct seq_file *file, void *data)
{
	...
	kbase_get_real_power(df, &total_power, kbdev->current_nominal_freq,
			     (kbdev->current_voltages[0] / 1000));
	kbase_pm_context_idle(kbdev);

	seq_printf(file, "total_power: %u static_power: %u dynamic_power: %u\n",
		   total_power, static_power, total_power - static_power);

	return 0;
}

static int kbasep_ipa_current_power_debugfs_open(struct inode *in, struct file *file)
{
	return single_open(file, kbasep_ipa_current_power_debugfs_show, in->i_private);
}

static const struct file_operations kbasep_ipa_current_power_debugfs_ops = {
	.open = kbasep_ipa_current_power_debugfs_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
void kbase_ipa_debugfs_init(struct kbase_device *kbdev)
{
	...
	debugfs_create_file("ipa_current_power", 0444, kbdev->mali_debugfs_directory, kbdev,
			    &kbasep_ipa_current_power_debugfs_ops);
