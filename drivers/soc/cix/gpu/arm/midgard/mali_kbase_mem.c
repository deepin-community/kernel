...
/*
 * acpi_find_device_by_node - Find the platform_device associated with a node
 * @np: Pointer to acpi_device.fwnode
 *
 ...
 */
static inline struct platform_device *acpi_find_device_by_node(struct fwnode_handle *np)
{
	struct device *dev;

	dev = bus_find_device_by_fwnode(&platform_bus_type, np);
...
int kbase_mem_init(struct kbase_device *kbdev)
{
	...
	struct fwnode_handle *mgm_node = NULL;
	struct fwnode_handle *fwnode = kbdev->dev->fwnode;
	 ...
	 */
	mgm_node = fwnode_find_reference(fwnode, "physical-memory-group-manager", 0);
	if (IS_ERR(mgm_node))
		dev_info(kbdev->dev, "No memory group manager is configured\n");
	else {
		if (has_acpi_companion(kbdev->dev))
			pdev = acpi_find_device_by_node(mgm_node);
		else
			pdev = of_find_device_by_node(to_of_node(mgm_node));
		...
		if (!pdev)
			...
			if (!kbdev->mgm_dev) {
				...
				dev_err(kbdev->dev, "Failed to get memory group manager module\n");
				err = -ENODEV;
				kbdev->mgm_dev = NULL;
			} else
				dev_info(kbdev->dev, "Memory group manager successfully loaded\n");
		}
		fwnode_handle_put(mgm_node);
	}
