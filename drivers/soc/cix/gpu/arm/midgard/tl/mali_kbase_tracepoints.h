 ...
 * @j_id: Unique ID provided by the caller, this is used to pair allocation and free requests.
 * @bin_id: The JIT allocation bin, used in conjunction with max_allocations to limit
 * the number of each type of JIT allocation.
 ...
 * @kbdev: Kbase device
 * @kbase_device_id: The ID of the physical hardware
 * @kbase_device_csg_slot_index: The index of the slot in the scheduler
 * whose CSG for which we are receiving an idle notification
 */
#if MALI_USE_CSF
...
/**
 * KBASE_TLSTREAM_TL_KBASE_ARRAY_ITEM_KCPUQUEUE_ENQUEUE_JIT_ALLOC - Array item of KCPU Queue enqueues
 * JIT Alloc
 *
 * @kbdev: Kbase device
 * @kcpu_queue: KCPU queue
 * @jit_alloc_gpu_alloc_addr_dest: The GPU virtual address to write the JIT allocated GPU
 * virtual address to
 ...
 * @jit_alloc_jit_id: Unique ID provided by the caller, this is used to pair allocation
 * and free requests. Zero is not a valid value
 * @jit_alloc_bin_id: The JIT allocation bin, used in conjunction with max_allocations
 * to limit the number of each type of JIT allocation
 * @jit_alloc_max_allocations: The maximum number of allocations allowed within the
 * bin specified by bin_id. Should be the same for all JIT allocations within the same bin.
 * @jit_alloc_flags: Flags specifying the special requirements for the JIT allocation
 * @jit_alloc_usage_id: A hint about which allocation should be reused.
 * The kernel should attempt to use a previous allocation with the same usage_id
 */
#if MALI_USE_CSF
 ...
 * @kbdev: Kbase device
 * @kcpu_queue: KCPU queue
 * @jit_alloc_jit_id: Unique ID provided by the caller, this is used to
 * pair allocation and free requests. Zero is not a valid value
 */
#if MALI_USE_CSF
 ...
 * @target_in_flight: Number of render-passes that the driver should attempt to keep
 * in flight for which allocation of new chunks is allowed
 ...
 * @mmu_synchronicity: Indicates whether the command is related to current
 * running job that needs to be resolved to make it progress (synchronous,
 * e.g. grow on page fault, JIT) or not (asynchronous, e.g. IOCTL calls from
 * user-space). This param will be 0 if it is an asynchronous operation.
