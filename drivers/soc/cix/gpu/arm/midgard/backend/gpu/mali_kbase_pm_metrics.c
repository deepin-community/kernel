...
#else
				IPA_CONTROL_TIMER_DEFAULT_VALUE_MS * 2 *  NSEC_PER_MSEC * 3 / 2;
#endif
			       ...
			       struct kbasep_pm_metrics *diff)
{
	...
	kbdev->pm.backend.metrics.values_diff.time_busy = diff->time_busy;
	kbdev->pm.backend.metrics.values_diff.time_idle = diff->time_idle;

#if MALI_USE_CSF
	diff->time_in_protm = cur->time_in_protm - last->time_in_protm;
	kbdev->pm.backend.metrics.values_diff.time_in_protm = diff->time_in_protm;
#else
