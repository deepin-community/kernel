...
{
	return ida_alloc_max(&its_vpeid_ida, ITS_MAX_VPEID, GFP_KERNEL);
}
...
static int its_save_disable(void *data __maybe_unused)
{
	struct its_node *its;
	int err = 0;
...
static void its_restore_enable(void *data __maybe_unused)
{
	struct its_node *its;
	int ret;
