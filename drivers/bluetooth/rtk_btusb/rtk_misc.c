...
int patch_add(struct usb_interface *intf)
{
	...
	RTKBT_DBG("patch_add");
	dev_entry = check_dev_entry(intf);
	if (NULL != dev_entry) {
		RTKBT_DBG("dev entry != NULL");
		return -1;
	}
...
#endif
	memset(&g_dev_entry, 0, sizeof(dev_data));
...
int download_patch(struct usb_interface *intf)
{
	...
	RTKBT_DBG("download_patch start jackie");
...
dev_data *dev_data_find(struct usb_interface * intf)
{
	...
	list_for_each_entry(dev_entry, &dev_data_list, list_node) {
		if (dev_entry->intf == intf) {
			...
			RTKBT_INFO("chip type value: 0x%02x", patch->chip_type);
			memcpy(&g_dev_entry, dev_entry, sizeof(dev_data));
			spin_unlock_irq(&entry_lock);
			return &g_dev_entry;
		}
	}
