...
IMPL_HOOK_FUNC_INT(inode_getsecurity, -EOPNOTSUPP, UOS_INODE_GETSECURITY,
		   ((int (*)(struct user_namespace *mnt_userns, struct inode *, const char *,
			     void **, bool))(uos_entry->cb->cb_addr)) (mnt_userns,
				 					   inode,
								       name,
								       buffer,
								       alloc),
		   struct user_namespace *mnt_userns, struct inode *inode, const char *name, void **buffer,
		   bool alloc);
...
IMPL_HOOK_FUNC_INT(sctp_assoc_request, 0, UOS_SCTP_ASSOC_REQUEST,
		   ((int (*)(struct sctp_association *,
			     struct sk_buff *))(uos_entry->cb->cb_addr)) (ep,
									  skb),
		   struct sctp_association *ep, struct sk_buff *skb);
